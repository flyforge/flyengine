...
if ($NoUnityBuild) {
    $CMAKE_ARGS += "-DPLASMA_ENABLE_FOLDER_UNITY_FILES:BOOL=OFF"
}
else {
    $CMAKE_ARGS += "-DPLASMA_ENABLE_FOLDER_UNITY_FILES:BOOL=ON"
}

if ($NoVulkan){
    $CMAKE_ARGS += "-DPLASMA_BUILD_VULKAN=OFF"
}
else{
    $CMAKE_ARGS += "-DPLASMA_BUILD_VULKAN=ON"
}
