  ...
  namespace tidy
  {
    namespace pl
    {
      ...
      std::string AddPrefixForType(clang::StringRef typeName, std::string newName, bool* prefixAdded = nullptr)
      {
        ...
        else if (typeName == "plSharedPtr" || typeName == "plUniquePtr" ||
                 typeName == "plPointerWithFlags" || typeName == "QPointer" ||
        ...
        else if (typeName == "plStringView" || typeName == "plHybridString" ||
                 typeName == "plHashedString" || typeName == "plTempHashedString" ||
                 typeName == "basic_string" || typeName == "plStringBuilder" ||
                 typeName == "QString")
        {
          if (prefixAdded)
            *prefixAdded = true;
        ...
        else if (typeName == "plAtomicBool")
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "b");
        }
        else if (typeName == "plAtomicInteger")
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "i");
        }
        else if (typeName.startswith("plVec") || typeName.startswith("plSimdVec"))
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "v");
        }
        else if (typeName.startswith("plMat") || typeName.startswith("plSimdMat"))
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "m");
        }
        else if (typeName.startswith("plQuat") || typeName == "plSimdQuat")
        {
          if (prefixAdded)
            *prefixAdded = true;
          return newName.insert(0, "q");
        }
        else if (typeName == "plSimdFloat")
        {
          if (prefixAdded)
            *prefixAdded = true;
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              {
                auto typedefName = typedefType->getDecl()->getName();
                if (typedefName == "plUInt8" || typedefName == "plUInt16" ||
                    typedefName == "plUInt32" || typedefName == "plUInt64" ||
                    typedefName == "size_t" || typedefName == "DWORD")
                {
                  return newName.insert(0, "ui");
                }
                else if (typedefName == "plInt8" || typedefName == "plInt16" ||
                         typedefName == "plInt32" || typedefName == "plInt64" ||
...
// m_bIsUsed[3]; plDynmicArray<bool> m_bVar? what about plUInt8 m_uiData[128];
// -> On fixed size arrays keep the already existing prefix
// * static plWorldModuleTypeId TYPE_ID. This is not a constant. Why all upper
