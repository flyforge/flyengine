...
if [ "$RunCMake" = true ]; then
  ...
  cmake -B $BuildDir -S . -G Ninja -DCMAKE_CXX_COMPILER=$cxx_compiler -DCMAKE_C_COMPILER=$c_compiler -DPLASMA_EXPERIMENTAL_EDITOR_ON_LINUX=ON -DPLASMA_BUILD_VULKAN=ON -DCMAKE_BUILD_TYPE=$BuildType -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
