...
struct ImGuiTableColumn
{
    ...
    float                   ContentMaxXHeadersUsed;         // Contents maximum position for headers rows (regardless of frepling). TableHeader() automatically softclip itself + report ideal desired size, to avoid creating extraneous draw calls
...
struct ImGuiTable
{
    ...
    ImGuiTableColumnIdx         FrepleRowsRequest;          // Requested frozen rows count
    ImGuiTableColumnIdx         FrepleRowsCount;            // Actual frozen row count (== FrepleRowsRequest, or == 0 when no scrolling offset)
    ImGuiTableColumnIdx         FrepleColumnsRequest;       // Requested frozen columns count
    ImGuiTableColumnIdx         FrepleColumnsCount;         // Actual frozen columns count (== FrepleColumnsRequest, or == 0 when no scrolling offset)
    ImGuiTableColumnIdx         RowCellDataCurrent;         // Index of current RowCellData[] entry in current row
    ImGuiTableDrawChannelIdx    DummyDrawChannel;           // Redirect non-visible columns here.
    ImGuiTableDrawChannelIdx    Bg2DrawChannelCurrent;      // For Selectable() and other widgets drawing across columns after the frepling line. Index within DrawSplitter.Channels[]
