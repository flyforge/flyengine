...
// - TableSetupScrollFreple()                   user submit scroll freple information (optional)
//-----------------------------------------------------------------------------
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    table->FrepleRowsRequest = table->FrepleRowsCount = 0; // This will be setup by TableSetupScrollFreple(), if any
    table->FrepleColumnsRequest = table->FrepleColumnsCount = 0;
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    int visible_n = 0;
    bool offset_x_frozen = (table->FrepleColumnsCount > 0);
    float offset_x = ((table->FrepleColumnsCount > 0) ? table->OuterRect.Min.x : work_rect.Min.x) + table->OuterPaddingX - table->CellSpacingX1;
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        column->NavLayerCurrent = (ImS8)((table->FrepleRowsCount > 0 || column_n < table->FrepleColumnsCount) ? ImGuiNavLayer_Menu : ImGuiNavLayer_Main);

        if (offset_x_frozen && table->FrepleColumnsCount == visible_n)
        {
        ...
        if (visible_n < table->FrepleColumnsCount)
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        if (table->FrepleColumnsCount > 0)
            if (column->MaxX < table->Columns[table->DisplayOrderToIndex[table->FrepleColumnsCount - 1]].MaxX)
                continue;
...
// [Public]
void ImGui::TableSetupScrollFreple(int columns, int rows)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    IM_ASSERT(table != NULL && "Need to call TableSetupColumn() after BeginTable()!");
    ...
    table->FrepleColumnsRequest = (table->Flags & ImGuiTableFlags_ScrollX) ? (ImGuiTableColumnIdx)columns : 0;
    table->FrepleColumnsCount = (table->InnerWindow->Scroll.x != 0.0f) ? table->FrepleColumnsRequest : 0;
    table->FrepleRowsRequest = (table->Flags & ImGuiTableFlags_ScrollY) ? (ImGuiTableColumnIdx)rows : 0;
    table->FrepleRowsCount = (table->InnerWindow->Scroll.y != 0.0f) ? table->FrepleRowsRequest : 0;
    table->IsUnfrozenRows = (table->FrepleRowsCount == 0); // Make sure this is set before TableUpdateLayout() so ImGuiListClipper can benefit from it.b
}
...
void ImGui::TableBeginRow(ImGuiTable* table)
{
    ...
    if (table->CurrentRow == 0 && table->FrepleRowsCount > 0)
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    const bool unfreple_rows_actual = (table->CurrentRow + 1 == table->FrepleRowsCount);
    const bool unfreple_rows_request = (table->CurrentRow + 1 == table->FrepleRowsRequest);
    if (table->CurrentRow == 0)
    ...
    if (is_visible)
    {
        ...
        const bool draw_strong_bottom_border = unfreple_rows_actual;
        ...
        // Draw bottom border at the row unfrepling mark (always strong)
    ...
    if (unfreple_rows_request)
        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
        {
            ImGuiTableColumn* column = &table->Columns[column_n];
            column->NavLayerCurrent = (ImS8)((column_n < table->FrepleColumnsCount) ? ImGuiNavLayer_Menu : ImGuiNavLayer_Main);
        }
    if (unfreple_rows_actual)
    {
...
float ImGui::TableGetMaxColumnWidth(const ImGuiTable* table, int column_n)
{
    ...
    if (table->Flags & ImGuiTableFlags_ScrollX)
    {
        ...
        if (column->DisplayOrder < table->FrepleColumnsRequest)
        {
            max_width = (table->InnerClipRect.Max.x - (table->FrepleColumnsRequest - column->DisplayOrder) * min_column_distance) - column->MinX;
...
// - After crossing FrepleRowsCount, all columns see their current draw channel changed to a second set of channels.
...
// - Clip                         --> 2+D+N channels
// - FrepleRows                   --> 2+D+N*2 (unless scrolling value is zero)
// - FrepleRows || FrepleColunns  --> 3+D+N*2 (unless scrolling value is zero)
// Where D is 1 if any column is clipped or hidden (dummy channel) otherwise 0.
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    const int freple_row_multiplier = (table->FrepleRowsCount > 0) ? 2 : 1;
    const int channels_for_row = (table->Flags & ImGuiTableFlags_NoClip) ? 1 : table->ColumnsEnabledCount;
    const int channels_for_bg = 1 + 1 * freple_row_multiplier;
    ...
    const int channels_total = channels_for_bg + (channels_for_row * freple_row_multiplier) + channels_for_dummy;
    table->DrawSplitter.Split(table->InnerWindow->DrawList, channels_total);
    table->DummyDrawChannel = (ImGuiTableDrawChannelIdx)((channels_for_dummy > 0) ? channels_total - 1 : -1);
    table->Bg2DrawChannelCurrent = TABLE_DRAW_CHANNEL_BG2_FROZEN;
    table->Bg2DrawChannelUnfrozen = (ImGuiTableDrawChannelIdx)((table->FrepleRowsCount > 0) ? 2 + channels_for_row : TABLE_DRAW_CHANNEL_BG2_FROZEN);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->IsVisibleX && column->IsVisibleY)
        {
            ...
            column->DrawChannelUnfrozen = (ImGuiTableDrawChannelIdx)(draw_channel_current + (table->FrepleRowsCount > 0 ? channels_for_row + 1 : 0));
            if (!(table->Flags & ImGuiTableFlags_NoClip))
    ...
    // All our cell highlight are manually clipped with BgClipRect. When unfrepling it will be made smaller to fit scrolling rect.
...
//   [ 0. ] no freple       [ 0. ] row freple      [ 01 ] col freple      [ 01 ] row+col freple
//   [ .. ]  or no scroll   [ 2. ]  and v-scroll   [ .. ]  and h-scroll   [ 23 ]  and v+h-scroll
//
// Each column itself can use 1 channel (row freple disabled) or 2 channels (row freple enabled).
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    const bool has_freple_v = (table->FrepleRowsCount > 0);
    const bool has_freple_h = (table->FrepleColumnsCount > 0);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        const int merge_group_sub_count = has_freple_v ? 2 : 1;
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            if (!(column->Flags & ImGuiTableColumnFlags_NoClip))
            {
                float content_max_x;
                if (!has_freple_v)
                    content_max_x = ImMax(column->ContentMaxXUnfrozen, column->ContentMaxXHeadersUsed); // No row freple
                else if (merge_group_sub_n == 0)
                    content_max_x = ImMax(column->ContentMaxXFrozen, column->ContentMaxXHeadersUsed);   // Row freple: use width before freple
                else
                    content_max_x = column->ContentMaxXUnfrozen;                                        // Row freple: use width after freple
                if (content_max_x > column->ClipRect.Max.x)
                    continue;
            }

            const int merge_group_n = (has_freple_h && column_n < table->FrepleColumnsCount ? 0 : 1) + (has_freple_v && merge_group_sub_n == 0 ? 0 : 2);
    ...
    if (merge_group_mask != 0)
    {
        ...
        IM_ASSERT(has_freple_v == false || table->Bg2DrawChannelUnfrozen != TABLE_DRAW_CHANNEL_BG2_FROZEN);
        int remaining_count = splitter->_Count - (has_freple_v ? LEADING_DRAW_CHANNELS + 1 : LEADING_DRAW_CHANNELS);
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                if ((merge_group_n & 1) == 0 || !has_freple_h)
                    merge_clip_rect.Min.x = ImMin(merge_clip_rect.Min.x, host_rect.Min.x);
                if ((merge_group_n & 2) == 0 || !has_freple_v)
            ...
            if (merge_group_n == 1 && has_freple_v)
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    const float draw_y2_head = table->IsUsingHeaders ? ImMin(table->InnerRect.Max.y, (table->FrepleRowsCount >= 1 ? table->InnerRect.Min.y : table->WorkRect.Min.y) + table->LastFirstRowHeight) : draw_y1;
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            const bool is_frozen_separator = (table->FrepleColumnsCount != -1 && table->FrepleColumnsCount == order_n + 1);
        ...
        if (g.IO.MouseDelta.x < 0.0f && g.IO.MousePos.x < cell_r.Min.x)
            if (ImGuiTableColumn* prev_column = (column->PrevEnabledColumn != -1) ? &table->Columns[column->PrevEnabledColumn] : NULL)
                if (!((column->Flags | prev_column->Flags) & ImGuiTableColumnFlags_NoReorder))
                    if ((column->IndexWithinEnabledSet < table->FrepleColumnsRequest) == (prev_column->IndexWithinEnabledSet < table->FrepleColumnsRequest))
                        table->ReorderColumnDir = -1;
        if (g.IO.MouseDelta.x > 0.0f && g.IO.MousePos.x > cell_r.Max.x)
            if (ImGuiTableColumn* next_column = (column->NextEnabledColumn != -1) ? &table->Columns[column->NextEnabledColumn] : NULL)
                if (!((column->Flags | next_column->Flags) & ImGuiTableColumnFlags_NoReorder))
                    if ((column->IndexWithinEnabledSet < table->FrepleColumnsRequest) == (next_column->IndexWithinEnabledSet < table->FrepleColumnsRequest))
                        table->ReorderColumnDir = +1;
    }
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        ImFormatString(buf, IM_ARRAYSIZE(buf),
            ...
            n, column->DisplayOrder, name, column->MinX - table->WorkRect.Min.x, column->MaxX - table->WorkRect.Min.x, (n < table->FrepleColumnsRequest) ? " (Frozen)" : "",
