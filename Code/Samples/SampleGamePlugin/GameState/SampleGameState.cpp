...
PLASMA_BEGIN_DYNAMIC_REFLECTED_TYPE(SampleGameState, 1, plRTTIDefaultAllocator<SampleGameState>)
...
void SampleGameState::OnActivation(plWorld* pWorld, const plTransform* pStartPosition)
{
...
void SampleGameState::BeforeWorldUpdate()
{
  ...
  if (plImgui::GetSingleton() != nullptr)
  {
    ...
    plImgui::GetSingleton()->SetPassInputToImgui(false); // reset this state, to deactivate input processing as long as SampleGameState::ProcessInput() isn't called again
...
plGameStatePriority SampleGameState::DeterminePriority(plWorld* pWorld) const
{
  return plGameStatePriority::Default;
}

void SampleGameState::ConfigureMainWindowInputDevices(plWindow* pWindow)
{
...
void SampleGameState::ProcessInput()
{
  ...
  if (plInputManager::GetInputActionState("SamplePlugin", "SpawnObject") == plKeyState::Pressed)
  {
  ...
  if (plInputManager::GetInputActionState("SamplePlugin", "DeleteObject") == plKeyState::Pressed)
  {
