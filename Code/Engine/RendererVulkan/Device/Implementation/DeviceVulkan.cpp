...
{
  plGALTextureVulkan* pTexture = PLASMA_NEW(&m_Allocator, plGALTextureVulkan, Description, bLinearCPU, bStaging);
...
vk::Fence plGALDeviceVulkan::Submit()
{
  ...
  PLASMA_CHECK_AT_COMPILETIME(sizeof(plUInt64) == sizeof(uint64_t));
  ...
  if (timelineInfo.waitSemaphoreValueCount > 0 || timelineInfo.signalSemaphoreValueCount > 0)
  {
    ...
    PLASMA_ASSERT_DEBUG(timelineInfo.waitSemaphoreValueCount == 0 || waitSemaphores.GetCount() == waitSemaphoreValues.GetCount(), "If a timeline semaphore is present, all semaphores need a wait value.");
    PLASMA_ASSERT_DEBUG(timelineInfo.signalSemaphoreValueCount == 0 || signalSemaphores.GetCount() == signalSemaphoreValues.GetCount(), "If a timeline semaphore is present, all semaphores need a signal value.");
  }
  PLASMA_ASSERT_DEBUG(waitSemaphores.GetCount() == waitStages.GetCount(), "Each wait semaphore needs a wait stage");
...
{
  plGALTextureVulkan* pTexture = PLASMA_NEW(&m_Allocator, plGALTextureVulkan, Description, false, false);
...
{
  plGALSharedTextureVulkan* pTexture = PLASMA_NEW(&m_Allocator, plGALSharedTextureVulkan, Description, sharedType, handle);
  ...
  for (PendingDeletion& deletion : pendingDeletions)
  {
    switch (deletion.m_type)
    {
      case vk::ObjectType::eUnknown:
        ...
        {
          PLASMA_REPORT_FAILURE("Unknown pending deletion");
        }
        break;
...
void plGALDeviceVulkan::FillFormatLookupTable()
{
  ...
  if (false)
  {
    PLASMA_LOG_BLOCK("GAL Resource Formats");
