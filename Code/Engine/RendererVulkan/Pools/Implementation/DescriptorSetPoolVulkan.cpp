...
vk::DescriptorPool plDescriptorSetPoolVulkan::s_currentPool;
plHybridArray<vk::DescriptorPool, 4> plDescriptorSetPoolVulkan::s_freePools;
vk::Device plDescriptorSetPoolVulkan::s_device;
plHashTable<vk::DescriptorType, float> plDescriptorSetPoolVulkan::s_descriptorWeights;

void plDescriptorSetPoolVulkan::Initialize(vk::Device device)
{
  s_device = device;
...
void plDescriptorSetPoolVulkan::DeInitialize()
{
...
plHashTable<vk::DescriptorType, float>& plDescriptorSetPoolVulkan::AccessDescriptorPoolWeights()
{
  return s_descriptorWeights;
}

vk::DescriptorSet plDescriptorSetPoolVulkan::CreateDescriptorSet(vk::DescriptorSetLayout layout)
{
  ...
  if (bPoolExhausted)
  {
    plGALDeviceVulkan* pDevice = static_cast<plGALDeviceVulkan*>(plGALDevice::GetDefaultDevice());
...
void plDescriptorSetPoolVulkan::UpdateDescriptorSet(vk::DescriptorSet descriptorSet, plArrayPtr<vk::WriteDescriptorSet> update)
{
  s_device.updateDescriptorSets(update.GetCount(), update.GetPtr(), 0, nullptr);
}

void plDescriptorSetPoolVulkan::ReclaimPool(vk::DescriptorPool& descriptorPool)
{
  s_device.resetDescriptorPool(descriptorPool);
  s_freePools.PushBack(descriptorPool);
}

vk::DescriptorPool plDescriptorSetPoolVulkan::GetNewPool()
{
  if (s_freePools.IsEmpty())
  {
    plHybridArray<vk::DescriptorPoolSize, 20> poolSizes;
    for (auto weight : s_descriptorWeights)
    {
      if (static_cast<plUInt32>(weight.Value() * s_uiPoolBaseSize) > 0)
        poolSizes.PushBack(vk::DescriptorPoolSize(weight.Key(), static_cast<plUInt32>(weight.Value() * s_uiPoolBaseSize)));
    }
