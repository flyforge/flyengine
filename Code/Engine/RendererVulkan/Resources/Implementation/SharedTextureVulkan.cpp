  ...
  if (m_Description.m_pExisitingNativeObject == nullptr)
  {
    ...
    if (m_SharedType == plGALSharedTextureType::None || m_SharedType == plGALSharedTextureType::Exported)
    {
      ...
      if (m_SharedType == plGALSharedTextureType::Exported)
      {
        ...
        VK_SUCCEED_OR_RETURN_PLASMA_FAILURE(device.getMemoryFdKHR(&getWin32HandleInfo, &fd, m_pDevice->GetDispatchContext()));
        ...
        VK_SUCCEED_OR_RETURN_PLASMA_FAILURE(device.getSemaphoreFdKHR(&getSemaphoreWin32Info, &semaphoreFd, m_pDevice->GetDispatchContext()));
        ...
        HANDLE handle = 0;
        VK_SUCCEED_OR_RETURN_PLASMA_FAILURE(device.getMemoryWin32HandleKHR(&getWin32HandleInfo, &handle, m_pDevice->GetDispatchContext()));
        m_hSharedHandle.m_uiProcessId = plProcess::GetCurrentProcessID();
        ...
        VK_SUCCEED_OR_RETURN_PLASMA_FAILURE(device.getSemaphoreWin32HandleKHR(&getSemaphoreWin32Info, &semaphoreHandle, m_pDevice->GetDispatchContext()));
    ...
    else if (m_SharedType == plGALSharedTextureType::Imported)
    {
      ...
      if (bNeedToImportForeignProcessFileDescriptors)
      {
        ...
        PLASMA_SCOPE_EXIT(close(processFd));
      ...
      VK_SUCCEED_OR_RETURN_PLASMA_FAILURE(device.importSemaphoreFdKHR(&importSemaphoreInfo, m_pDevice->GetDispatchContext()));

      // Create Image
      VK_SUCCEED_OR_RETURN_PLASMA_FAILURE(device.createImage(&createInfo, nullptr, &m_image));
      ...
      // Import memory
      PLASMA_ASSERT_DEBUG(imageMemoryRequirements.memoryRequirements.size == m_hSharedHandle.m_uiSize, "");
      ...
      m_allocInfo = {};
      VK_SUCCEED_OR_RETURN_PLASMA_FAILURE(device.allocateMemory(&allocateInfo, nullptr, &m_allocInfo.m_deviceMemory));
      ...
      VK_SUCCEED_OR_RETURN_PLASMA_FAILURE(res);
...
PLASMA_STATICLINK_FILE(RendererVulkan, RendererVulkan_Resources_Implementation_SharedTextureVulkan);
