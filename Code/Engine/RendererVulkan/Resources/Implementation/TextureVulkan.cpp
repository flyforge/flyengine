...
{
  PLASMA_ASSERT_DEBUG(createInfo.format != vk::Format::eUndefined, "No storage format available for given format: {0}", m_Description.m_Format);
...
plGALTextureVulkan::StagingMode plGALTextureVulkan::ComputeStagingMode(plGALDeviceVulkan* m_pDevice, const plGALTextureCreationDescription& m_Description, const vk::ImageCreateInfo& createInfo)
{
  ...
  PLASMA_ASSERT_DEV(!plConversionUtilsVulkan::IsStencilFormat(createInfo.format), "Stencil read-back not implemented.");
