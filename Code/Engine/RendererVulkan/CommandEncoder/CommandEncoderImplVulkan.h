...
class PL_RENDERERVULKAN_DLL plGALCommandEncoderImplVulkan : public plGALCommandEncoderCommonPlatformInterface, public plGALCommandEncoderRenderPlatformInterface, public plGALCommandEncoderComputePlatformInterface
{
public:
...
private:
  ...
  struct SetResources
  {
    plDynamicArray<const plGALBufferVulkan*> m_pBoundConstantBuffers;
    plDynamicArray<const plGALResourceViewVulkan*> m_pBoundShaderResourceViews;
    plDynamicArray<const plGALUnorderedAccessViewVulkan*> m_pBoundUnoderedAccessViews;
    plDynamicArray<const plGALSamplerStateVulkan*> m_pBoundSamplerStates;
  };

private:
  plResult FlushDeferredStateChanges();
  const plGALResourceViewVulkan* GetShaderResourceView(const SetResources& resources, const plShaderResourceBinding& mapping);
  const plGALUnorderedAccessViewVulkan* GetShaderUAV(const SetResources& resources, const plShaderResourceBinding& mapping);
...
private:
  ...
  const plGALRenderTargetView* m_pBoundRenderTargets[PL_GAL_MAX_RENDERTARGET_COUNT] = {};
  ...
  vk::Buffer m_pBoundVertexBuffers[PL_GAL_MAX_VERTEX_BUFFER_COUNT];
  vk::DeviceSize m_VertexBufferOffsets[PL_GAL_MAX_VERTEX_BUFFER_COUNT] = {};

  plHybridArray<SetResources, 4> m_Resources;

  plDeque<vk::DescriptorImageInfo> m_TextureAndSampler;
  plHybridArray<vk::WriteDescriptorSet, 16> m_DescriptorWrites;
  plHybridArray<vk::DescriptorSet, 4> m_DescriptorSets;

  plDynamicArray<plUInt8> m_PushConstants;
};
