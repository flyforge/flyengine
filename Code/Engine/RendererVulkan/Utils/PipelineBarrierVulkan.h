...
public:
  ...
  void EnsureImageLayout(const plGALResourceViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource = false);
  void EnsureImageLayout(const plGALUnorderedAccessViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource = false);
...
private:
  ...
  struct ImageState
  {
    const plGALTextureVulkan* m_pTexture = nullptr;
    plBitfield<plHybridArray<plUInt32, 1>> m_dirty;
    plHybridArray<SubElementState, 1> m_subElementLayout;
  };
  ...
  struct BufferState
  {
    const plGALBufferVulkan* m_pBuffer = nullptr;
    plBitfield<plHybridArray<plUInt32, 1>> m_dirty;
    plHybridArray<SubBufferState, 1> m_subBufferState;
  };
...
private:
  ...
  plHybridArray<vk::BufferMemoryBarrier, 8> m_bufferBarriers;
  plHybridArray<vk::ImageMemoryBarrier, 8> m_imageBarriers;

  plMap<vk::Image, ImageState> m_imageState;
  plMap<vk::Buffer, BufferState> m_bufferState;
};
