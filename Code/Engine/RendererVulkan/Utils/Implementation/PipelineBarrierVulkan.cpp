...
namespace
{
  vk::ImageSubresourceRange CreateSubRange(const vk::ImageSubresourceRange& fullRange, plUInt32 uiLayer, plUInt32 uiMipLevel)
  {
...
void plPipelineBarrierVulkan::SetCommandBuffer(vk::CommandBuffer* pCommandBuffer)
{
  m_pCommandBuffer = pCommandBuffer;
}

void plPipelineBarrierVulkan::Flush()
{
  if (m_srcStageMask || m_dstStageMask)
  {
    ...
    {
      plLog::Warning("Layout Changed {}: {} -> {}", plArgP(img.image), vk::to_string(img.oldLayout).c_str(), vk::to_string(img.newLayout).c_str());
    }
#endif
...
void plPipelineBarrierVulkan::Submit()
{
  Flush();
  ...
  for (auto it = m_imageState.GetIterator(); it.IsValid();)
  {
    ...
    const plUInt32 uiCount = it.Value().m_subElementLayout.GetCount();
...
bool plPipelineBarrierVulkan::IsDirty(vk::Image image, const vk::ImageSubresourceRange& subResources) const
{
...
bool plPipelineBarrierVulkan::IsDirty() const
{
  return m_srcStageMask || m_dstStageMask;
}

bool plPipelineBarrierVulkan::IsDirty(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize length, vk::AccessFlags dstAccess)
{
...
void plPipelineBarrierVulkan::AccessBuffer(const plGALBufferVulkan* pBuffer, vk::DeviceSize offset, vk::DeviceSize length, vk::PipelineStageFlags srcStages, vk::AccessFlags srcAccess, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
  SubBufferState subState;
  subState.m_offset = offset;
  subState.m_length = length;
...
void plPipelineBarrierVulkan::EnsureImageLayout(const plGALTextureVulkan* pTexture, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  EnsureImageLayout(pTexture, pTexture->GetFullRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void plPipelineBarrierVulkan::EnsureImageLayout(const plGALRenderTargetViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const plGALTextureVulkan*>(pTextureView->GetTexture()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void plPipelineBarrierVulkan::EnsureImageLayout(const plGALResourceViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const plGALTextureVulkan*>(pTextureView->GetResource()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void plPipelineBarrierVulkan::EnsureImageLayout(const plGALUnorderedAccessViewVulkan* pTextureView, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  auto pTexture = static_cast<const plGALTextureVulkan*>(pTextureView->GetResource()->GetParentResource());
  EnsureImageLayout(pTexture, pTextureView->GetRange(), dstLayout, dstStages, dstAccess, bDiscardSource);
}

void plPipelineBarrierVulkan::EnsureImageLayout(const plGALTextureVulkan* pTexture, vk::ImageSubresourceRange subResources, vk::ImageLayout dstLayout, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess, bool bDiscardSource)
{
  const vk::ImageSubresourceRange fullRange = pTexture->GetFullRange();
  if (plGALResourceFormat::IsStencilFormat(pTexture->GetDescription().m_Format))
  {
  ...
  if (pState)
  {
    ...
    if (bIsFullRange)
    {
      ...
      else
      {
        ...
        const plUInt32 uiLastLayer = subResources.baseArrayLayer + subResources.layerCount;
        const plUInt32 uiLastLevel = subResources.baseMipLevel + subResources.levelCount;
        for (plUInt32 uiLayer = subResources.baseArrayLayer; uiLayer < uiLastLayer; uiLayer++)
        {
          for (plUInt32 uiMipLevel = subResources.baseMipLevel; uiMipLevel < uiLastLevel; uiMipLevel++)
          {
            const plUInt32 uiSubresourceIndex = uiMipLevel + uiLayer * fullRange.levelCount;
...
void plPipelineBarrierVulkan::TextureDestroyed(const plGALTextureVulkan* pTexture)
{
  auto it = m_imageState.Find(pTexture->GetImage());
  if (it.IsValid())
  {
    ...
    if (it.Value().m_dirty.IsAnyBitSet())
    {
      ...
      for (plInt32 i = ((plInt32)m_imageBarriers.GetCount()) - 1; i >= 0; --i)
      {
...
void plPipelineBarrierVulkan::BufferDestroyed(const plGALBufferVulkan* pBuffer)
{
...
void plPipelineBarrierVulkan::FullBarrier()
{
  vk::MemoryBarrier memoryBarrier;
...
bool plPipelineBarrierVulkan::AddBufferBarrierInternal(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize length, vk::PipelineStageFlags srcStages, vk::AccessFlags srcAccess, vk::PipelineStageFlags dstStages, vk::AccessFlags dstAccess)
{
...
bool plPipelineBarrierVulkan::IsDirtyInternal(const BufferState& state, const SubBufferState& subState) const
{
  const plUInt32 uiCount = state.m_subBufferState.GetCount();
  for (plUInt32 i = 0; i < uiCount; i++)
  {
...
bool plPipelineBarrierVulkan::AddImageBarrierInternal(vk::Image image, const vk::ImageSubresourceRange& subResources, vk::ImageLayout srcLayout, vk::AccessFlags srcAccess, vk::ImageLayout dstLayout, vk::AccessFlags dstAccess, bool bDiscardSource)
{
...
bool plPipelineBarrierVulkan::IsDirtyInternal(const ImageState& state, const vk::ImageSubresourceRange& subResources) const
{
  ...
  {
    plUInt32 uiLevelCount = state.m_pTexture->GetDescription().m_uiMipLevelCount;
    ...
    for (plUInt32 uiLayer = subResources.baseArrayLayer; uiLayer < uiLastLayer; uiLayer++)
    {
      for (plUInt32 uiMipLevel = subResources.baseMipLevel; uiMipLevel < uiLastLevel; uiMipLevel++)
      {
        const plUInt32 uiSubresourceIndex = uiMipLevel + uiLayer * uiLevelCount;
