...
///
/// By deriving from plTask you can create your own task types. These can be executed through this task system.
...
public:
  ...
  /// has been put.
  static plTaskGroupID StartSingleTask(const plSharedPtr<plTask>& pTask, plTaskPriority::Enum priority,
    plOnTaskGroupFinishedCallback callback = plOnTaskGroupFinishedCallback()); // [tested]

  /// \brief A helper function to insert a single task into the system and start it right away. Returns ID of the Group into which the task
  /// has been put. This overload allows to additionally specify a single dependency.
  static plTaskGroupID StartSingleTask(const plSharedPtr<plTask>& pTask, plTaskPriority::Enum priority, plTaskGroupID dependency,
    plOnTaskGroupFinishedCallback callback = plOnTaskGroupFinishedCallback()); // [tested]
  ...
  static plResult CancelTask(const plSharedPtr<plTask>& pTask, plOnTaskRunning::Enum onTaskRunning = plOnTaskRunning::WaitTillFinished); // [tested]

  struct TaskData
  {
    plSharedPtr<plTask> m_pTask;
    plTaskGroup* m_pBelongsToGroup = nullptr;
    plUInt32 m_uiInvocation = 0;
  };

private:
  /// \brief Searches for a task of priority between \a FirstPriority and \a LastPriority (inclusive).
  static TaskData GetNextTask(plTaskPriority::Enum FirstPriority, plTaskPriority::Enum LastPriority, bool bOnlyTasksThatNeverWait,
  ...
  static bool ExecuteTask(plTaskPriority::Enum FirstPriority, plTaskPriority::Enum LastPriority, bool bOnlyTasksThatNeverWait,
    const plTaskGroupID& WaitingForGroup, plAtomicInteger32* pWorkerState);
  ...
  static void TaskHasFinished(plSharedPtr<plTask>&& pTask, plTaskGroup* pGroup);
  ...
  static void ExecuteSomeFrameTasks(plTime smoothFrameTime);


  /// \brief Helps executing tasks that are suitable for the calling thread. Returns true if a task was found and executed.
  static bool HelpExecutingTasks(const plTaskGroupID& WaitingForGroup);
...
public:
  ...
  static plTaskGroupID CreateTaskGroup(
    plTaskPriority::Enum priority, plOnTaskGroupFinishedCallback callback = plOnTaskGroupFinishedCallback()); // [tested]

  /// \brief Adds a task to the given task group. The group must not yet have been started.
  static void AddTaskToGroup(plTaskGroupID group, const plSharedPtr<plTask>& pTask); // [tested]
  ...
  static void AddTaskGroupDependency(plTaskGroupID group, plTaskGroupID dependsOn); // [tested]
  ...
  static void StartTaskGroup(plTaskGroupID group); // [tested]
  ...
  static bool IsTaskGroupFinished(plTaskGroupID group); // [tested]
  ...
  static void WaitForGroup(plTaskGroupID group); // [tested]
...
private:
  ...
  static void ScheduleGroupTasks(plTaskGroup* pGroup, bool bHighPriority);
  ...
  static void DependencyHasFinished(plTaskGroup* pGroup);
...
public:
  ...
  /// There will always be exactly one additional thread for file access tasks (plTaskPriority::FileAccess).
  ///
  ...
  static void SetWorkerThreadCount(plInt32 iShortTasks = -1, plInt32 iLongTasks = -1); // [tested]
...
private:
  ...
  static void AllocateThreads(plWorkerThreadType::Enum type, plUInt32 uiAddThreads);
  ...
  static void DetermineTasksToExecuteOnThread(plTaskPriority::Enum& out_FirstPriority, plTaskPriority::Enum& out_LastPriority);

private:
  static plUniquePtr<plTaskSystemThreadState> s_pThreadState;
...
public:
  ...
  ///   - ParallelFor(taskItems, [](plArrayPtr<ElemType> taskItemSlice) { });
  template <typename ElemType, typename Callback>
  static void ParallelFor(
    plArrayPtr<ElemType> taskItems, Callback taskCallback, const char* szTaskName = nullptr, const plParallelForParams& params = plParallelForParams());
  ...
  /// Given an array pointer 'taskItems' with elements of type ElemType, the following invocations are possible:
  ///   - ParallelFor(taskItems, [](plUInt32 globalTaskItemIndex, ElemType taskItem) { });
  ///   - ParallelFor(taskItems, [](plUInt32 globalTaskItemIndex, ElemType& taskItem) { });
  ///   - ParallelFor(taskItems, [](plUInt32 globalTaskItemIndex, const ElemType& taskItem) { });
  template <typename ElemType, typename Callback>
...
private:
  ...
  static void ParallelForInternal(
    plArrayPtr<ElemType> taskItems, plParallelForFunction<ElemType> taskCallback, const char* taskName, const plParallelForParams& params);
...
public:
  /// \brief Writes the internal state of the plTaskSystem as a DGML graph.
...
private:
  ...
  static plMutex s_TaskSystemMutex;

  static plUniquePtr<plTaskSystemState> s_pState;
