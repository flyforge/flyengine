...
#define PLASMA_MSVC_WARNING_NUMBER 6312
...
void SetThreadName(HANDLE hThread, LPCSTR pThreadName)
{
  ...
  {
    plStringWChar threadName(pThreadName);
...
plOSThread::plOSThread(
  plOSThreadEntryPoint threadEntryPoint, void* pUserData /*= nullptr*/, const char* szName /*= "plThread"*/, plUInt32 uiStackSize /*= 128 * 1024*/)
{
  s_iThreadCount.Increment();

  PLASMA_ASSERT_ALWAYS(threadEntryPoint != nullptr, "Thread entry point is invalid.");

  m_hHandle = CreateThread(nullptr, uiStackSize, threadEntryPoint, pUserData, CREATE_SUSPENDED, nullptr);
  PLASMA_ASSERT_RELEASE(m_hHandle != INVALID_HANDLE_VALUE, "Thread creation failed!");
  PLASMA_ASSERT_RELEASE(m_hHandle != nullptr, "Thread creation failed!"); // makes the static code analysis happy
