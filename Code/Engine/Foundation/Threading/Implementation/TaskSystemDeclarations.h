...
/// Once 'plTaskSystem::FinishFrameTasks' is called, all those tasks will be moved into the 'XYZThisFrame' categories.\n
...
private:
  ...
  plUInt32 m_uiGroupCounter = 0;

  // points to the actual task group object
  plTaskGroup* m_pTaskGroup = nullptr;
};

/// \brief Callback type when a task group has been finished (or canceled).
using plOnTaskGroupFinishedCallback = plDelegate<void(plTaskGroupID)>;

/// \brief Callback type when a task has been finished (or canceled).
using plOnTaskFinishedCallback = plDelegate<void(const plSharedPtr<plTask>&)>;
...
///
/// This is an optimization hint for the plTaskSystem. Tasks that never wait on other tasks
...
using plParallelForIndexedFunction32 = plDelegate<void(plUInt32, plUInt32), 48>;
using plParallelForIndexedFunction64 = plDelegate<void(plUInt64, plUInt64), 48>;

template <typename ElemType>
using plParallelForFunction = plDelegate<void(plUInt32, plArrayPtr<ElemType>), 48>;

enum class plTaskWorkerState
{
  Active = 0,
  Idle = 1,
  Blocked = 2,
