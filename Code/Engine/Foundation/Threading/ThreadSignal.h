...
///
/// plThreadSignal is similar to plConditionVariable but adds some internal state, which makes it more suitable for common use cases.
/// For instance, in contrast to plConditionVariable, one can wait for an plThreadSignal and get awoken, even if the signal was raised
...
///
/// Once a waiting thread is woken up, the signal state is cleared automatically, unless the plThreadSignal uses 'ManualReset' mode.
...
private:
  ...
  mutable plConditionVariable m_ConditionVariable;
};
