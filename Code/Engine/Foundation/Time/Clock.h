...
public:
  ...
  static plClock* GetGlobalClock() { return s_pGlobalClock; }

public:
  /// \brief Constructor.
  plClock(plStringView sName); // [tested]
  ...
  plTimeStepSmoothing* GetTimeStepSmoothing() const; // [tested]
  ...
  void SetFixedTimeStep(plTime diff = plTime()); // [tested]

  /// \brief Returns the value for the fixed time step (zero if it is disabled).
  plTime GetFixedTimeStep() const; // [tested]
  ...
  void SetAccumulatedTime(plTime t); // [tested]
  ...
  plTime GetAccumulatedTime() const; // [tested]
  ...
  plTime GetTimeDiff() const; // [tested]
  ...
  void SetMinimumTimeStep(plTime min); // [tested]
  ...
  /// \brief Sets the name of the clock. Useful to identify the clock in tools such as plInspector.
...
public:
  ...
  struct EventData
  {
    plStringView m_sClockName;

    plTime m_RawTimeStep;
    plTime m_SmoothedTimeStep;
  };
...
private:
  ...
  plTime m_AccumulatedTime;
  plTime m_LastTimeDiff;
  plTime m_FixedTimeStep;
  plTime m_LastTimeUpdate;
  plTime m_MinTimeStep;
  plTime m_MaxTimeStep;

  double m_fSpeed;
  bool m_bPaused;

  plTimeStepSmoothing* m_pTimeStepSmoother;
};
...
public:
  ...
  ///
  /// \note It is the responsibility of each plTimeStepSmoothing class to implement
  ...
  /// \brief Called when plClock::Reset(), plClock::Load() or plClock::SetPaused(true) was called.
  ///
  /// \param pClock
