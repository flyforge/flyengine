...
public:
  ...
  PLASMA_ALWAYS_INLINE std::ptrdiff_t operator-(const const_iterator_base& rhs) const { return m_uiIndex - rhs.m_uiIndex; }

  PLASMA_ALWAYS_INLINE const_iterator_base operator+(std::ptrdiff_t rhs) const { return const_iterator_base(*m_pArray, m_uiIndex + rhs); }
  PLASMA_ALWAYS_INLINE const_iterator_base operator-(std::ptrdiff_t rhs) const { return const_iterator_base(*m_pArray, m_uiIndex - rhs); }

  PLASMA_ALWAYS_INLINE void operator+=(std::ptrdiff_t rhs) { m_uiIndex += rhs; }
  PLASMA_ALWAYS_INLINE void operator-=(std::ptrdiff_t rhs) { m_uiIndex -= rhs; }

  inline const T& operator*() const
  {
    if (reverse)
      return (*m_pArray)[m_pArray->GetCount() - (plUInt32)m_uiIndex - 1];
    else
  ...
  PLASMA_ALWAYS_INLINE const T& operator[](size_t uiIndex) const
  {
    if (reverse)
      ...
      return (*m_pArray)[static_cast<plUInt32>(m_uiIndex + uiIndex)];
  }
...
public:
  ...
  PLASMA_ALWAYS_INLINE iterator_base operator+(std::ptrdiff_t rhs) const { return iterator_base(*this->m_pArray, this->m_uiIndex + rhs); }
  PLASMA_ALWAYS_INLINE iterator_base operator-(std::ptrdiff_t rhs) const { return iterator_base(*this->m_pArray, this->m_uiIndex - rhs); }

  inline T& operator*() const
  {
    if (reverse)
      return (*this->m_pArray)[this->m_pArray->GetCount() - (plUInt32)this->m_uiIndex - 1];
    else
  ...
  PLASMA_ALWAYS_INLINE T& operator[](size_t uiIndex) const
  {
    if (reverse)
      ...
      return (*this->m_pArray)[static_cast<plUInt32>(this->m_uiIndex + uiIndex)];
  }
};
...
public:
  ...
  PLASMA_ALWAYS_INLINE std::ptrdiff_t operator-(const const_reverse_pointer_iterator& rhs) const { return rhs.m_pPtr - m_pPtr; }

  PLASMA_ALWAYS_INLINE const_reverse_pointer_iterator operator+(std::ptrdiff_t rhs) const { return const_reverse_pointer_iterator(m_pPtr - rhs); }
  PLASMA_ALWAYS_INLINE const_reverse_pointer_iterator operator-(std::ptrdiff_t rhs) const { return const_reverse_pointer_iterator(m_pPtr + rhs); }

  PLASMA_ALWAYS_INLINE void operator+=(std::ptrdiff_t rhs) { m_pPtr -= rhs; }
  PLASMA_ALWAYS_INLINE void operator-=(std::ptrdiff_t rhs) { m_pPtr += rhs; }
  ...
  PLASMA_ALWAYS_INLINE const T& operator[](std::ptrdiff_t iIndex) const { return *(m_pPtr - iIndex); }
...
public:
  ...
  PLASMA_ALWAYS_INLINE reverse_pointer_iterator operator+(std::ptrdiff_t rhs) const { return reverse_pointer_iterator(this->m_pPtr - rhs); }
  PLASMA_ALWAYS_INLINE reverse_pointer_iterator operator-(std::ptrdiff_t rhs) const { return reverse_pointer_iterator(this->m_pPtr + rhs); }

  PLASMA_ALWAYS_INLINE T& operator*() const { return *(this->m_pPtr); }
  PLASMA_ALWAYS_INLINE T* operator->() const { return this->m_pPtr; }
  PLASMA_ALWAYS_INLINE T& operator[](std::ptrdiff_t iIndex) const { return *(this->m_pPtr - iIndex); }
};
