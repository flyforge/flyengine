...
public:
  ...
  PLASMA_ALWAYS_INLINE Type& Element(plInt32 iColumn, plInt32 iRow) { return m_fElementsCM[iColumn * 3 + iRow]; }
  PLASMA_ALWAYS_INLINE Type Element(plInt32 iColumn, plInt32 iRow) const { return m_fElementsCM[iColumn * 3 + iRow]; }
...
public:
  ...
  /// \brief Tries to set the three scaling factors in the matrix. Returns PLASMA_FAILURE if the matrix columns cannot be normalized and thus no rescaling
  /// is possible.
