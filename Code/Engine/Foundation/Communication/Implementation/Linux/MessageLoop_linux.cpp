...
plMessageLoop_linux::plMessageLoop_linux()
{
  int fds[2];
  if (pipe2(fds, O_NONBLOCK | O_CLOEXEC) < 0)
  {
    plLog::Error("[IPC]Failed to create wakeup pipe for plMessageLoop_linux");
  }
  else
  {
...
bool plMessageLoop_linux::WaitForMessages(plInt32 iTimeout, plIpcChannel* pFilter)
{
  PLASMA_ASSERT_ALWAYS(pFilter == nullptr, "Not implemented");
  ...
  if (result > 0)
  {
    ...
    plUInt32 numEvents = m_pollInfos.GetCount();
    for (plUInt32 i = 1; i < numEvents;)
    {
  ...
  if (result < 0)
  {
    plLog::Error("[IPC]plMessageLoop_linux failed to poll events. Error {}", errno);
  }
  return false;
}

void plMessageLoop_linux::RegisterWait(plPipeChannel_linux* pChannel, WaitType type, int fd)
{
  plLog::Debug("[IPC]plMessageLoop_linux::RegisterWait({}}", (int)type);
  ...
  PLASMA_SCOPE_EXIT(m_numPendingPollModifications.Decrement());
  WakeUp();
  {
    plLock lock{m_pollMutex};
...
void plMessageLoop_linux::RemovePendingWaits(plPipeChannel_linux* pChannel)
{
  m_numPendingPollModifications.Increment();
  PLASMA_SCOPE_EXIT(m_numPendingPollModifications.Decrement());
  WakeUp();
  {
    plLock lock{m_pollMutex};
    plUInt32 waitCount = m_pollInfos.GetCount();
    for (plUInt32 i = 0; i < waitCount;)
    {
...
void plMessageLoop_linux::WakeUp()
{
  plUInt8 wakeupByte = 0;
...
PLASMA_STATICLINK_FILE(Foundation, Foundation_Communication_Implementation_Linux_MessageLoop_linux);
