...
plMessageLoop_win::plMessageLoop_win()
{
  ...
  PLASMA_ASSERT_DEBUG(m_hPort != INVALID_HANDLE_VALUE, "Failed to create IO completion port!");
}
...
bool plMessageLoop_win::WaitForMessages(plInt32 iTimeout, plIpcChannel* pFilter)
{
  ...
  if (item.pContext->pChannel != NULL)
  {
    ...
    {
      PLASMA_ASSERT_DEBUG(item.pContext->pChannel == item.pChannel, "");
...
bool plMessageLoop_win::ProcessInternalIOItem(const IOItem& item)
{
  if (reinterpret_cast<plMessageLoop_win*>(item.pContext) == this && reinterpret_cast<plMessageLoop_win*>(item.pChannel) == this)
  {
    ...
    PLASMA_ASSERT_DEBUG(item.uiBytesTransfered == 0, "");
...
void plMessageLoop_win::WakeUp()
{
  ...
  PLASMA_ASSERT_DEBUG(res, "Could not PostQueuedCompletionStatus: {0}", plArgErrorCode(GetLastError()));
}

#endif

PLASMA_STATICLINK_FILE(Foundation, Foundation_Communication_Implementation_Win_MessageLoop_win);
