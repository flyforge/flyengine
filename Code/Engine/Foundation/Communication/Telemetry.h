...
public:
  ...
  /// \brief Defines how the plTelemetry system was configured.
  enum ConnectionMode
  {
  ...
  static plTime GetPingToServer() { return s_PingToServer; }
  ...
  /// IsConnectedToServer() ).
  static plStringView GetServerName() { return s_sServerName; }
  ...
  /// IsConnectedToServer() ).
  static plStringView GetServerIP() { return s_sServerIP; }
  ...
  static plUInt32 GetServerID() { return s_uiServerID; }
  ...
  static plMutex& GetTelemetryMutex();
  ...
  /// \brief Adds an event handler that is called for every plTelemetry event.
  static void AddEventHandler(plEventTelemetry::Handler handler) { s_TelemetryEvents.AddEventHandler(handler); }

  /// \brief Removes a previously added event handler.
  static void RemoveEventHandler(plEventTelemetry::Handler handler) { s_TelemetryEvents.RemoveEventHandler(handler); }
...
private:
  ...
  static plResult OpenConnection(ConnectionMode Mode, plStringView sConnectTo = {});

  static void Transmit(TransmitMode tm, const void* pData, plUInt32 uiDataBytes);

  static void Send(TransmitMode tm, plUInt32 uiSystemID, plUInt32 uiMsgID, const void* pData, plUInt32 uiDataBytes);
  static void Send(TransmitMode tm, plUInt32 uiSystemID, plUInt32 uiMsgID, plStreamReader& Stream, plInt32 iDataBytes = -1);
  static void Send(TransmitMode tm, plTelemetryMessage& msg);

  friend class plTelemetryThread;

  static void FlushOutgoingQueues();

  static void InitializeAsServer();
  static plResult InitializeAsClient(plStringView sConnectTo);
  static ConnectionMode s_ConnectionMode;

  static plUInt32 s_uiApplicationID;
  static plUInt32 s_uiServerID;

  static plString s_sServerName;
  static plString s_sServerIP;

  static bool s_bConnectedToServer;
  static bool s_bConnectedToClient;
  static bool s_bAllowNetworkUpdate;

  static void QueueOutgoingMessage(TransmitMode tm, plUInt32 uiSystemID, plUInt32 uiMsgID, const void* pData, plUInt32 uiDataBytes);

  static void SendServerName();

  static plTime s_PingToServer;

  using MessageDeque = plDeque<plTelemetryMessage>;
  ...
  struct MessageQueue
  {
    ...
    void* m_pPassThrough;
    plUInt32 m_uiMaxQueuedOutgoing;

    MessageDeque m_IncomingQueue;
    MessageDeque m_OutgoingQueue;
  };

  static plMap<plUInt64, MessageQueue> s_SystemMessages;

  static plEventTelemetry s_TelemetryEvents;

private:
  static plMutex s_TelemetryMutex;
