...
// static
plUInt32 plHashingUtils::MurmurHash32(const void* pKey, size_t uiSizeInByte, plUInt32 uiSeed /*= 0*/)
{
  const plUInt32 m = plInternal::MURMUR_M;
  const plUInt32 r = plInternal::MURMUR_R;
  ...
  if (reinterpret_cast<size_t>(pKey) % 4 != 0)
  {
    ...
    while (uiSizeInByte >= 4)
    {
      ...
      pKey = plMemoryUtils::AddByteOffset(pKey, sizeof(k));
...
#endif
  {
    const plUInt32* pData = static_cast<const plUInt32*>(pKey);
  ...
  const plUInt8* pData2 = reinterpret_cast<const plUInt8*>(pKey);
...
// static
plUInt64 plHashingUtils::MurmurHash64(const void* pKey, size_t uiSizeInByte, plUInt64 uiSeed /*= 0*/)
{
  const plUInt64 m = 0xc6a4a7935bd1e995ULL;
  const plUInt64 r = 47;
  ...
  if (reinterpret_cast<size_t>(pKey) % 8 != 0)
  {
    ...
    const void* pEnd = plMemoryUtils::AddByteOffset(pKey, (uiSizeInByte / 8) * 8);

    while (pData != pEnd)
    {
      plUInt64 k = 0;
      memcpy(&k, pData, sizeof(k));
      pData = plMemoryUtils::AddByteOffset(pData, sizeof(k));
...
#endif
  {
    const plUInt64* pData = static_cast<const plUInt64*>(pKey);
    const plUInt64* pEnd = pData + (uiSizeInByte / 8);
  ...
  {
    case 7:
      h ^= plUInt64(pData2[6]) << 48;
    case 6:
      h ^= plUInt64(pData2[5]) << 40;
    case 5:
      h ^= plUInt64(pData2[4]) << 32;
    case 4:
      h ^= plUInt64(pData2[3]) << 24;
    case 3:
      h ^= plUInt64(pData2[2]) << 16;
    case 2:
      h ^= plUInt64(pData2[1]) << 8;
    case 1:
      h ^= plUInt64(pData2[0]);
      h *= m;
  };
...
// static
plUInt32 plHashingUtils::CRC32Hash(const void* pKey, size_t uiSizeInBytes)
{
  if (pKey == 0 || uiSizeInBytes <= 0)
    return 0;

  plUInt32 uiCRC32 = 0xFFFFFFFF;

  for (size_t i = 0; i < uiSizeInBytes; i++)
    uiCRC32 = (uiCRC32 >> 8) ^ uiCRC32Table[(uiCRC32 & 0xFF) ^ static_cast<const plUInt8*>(pKey)[i]];
...
// static
plUInt64 plHashingUtils::xxHash64(const void* pKey, size_t uiSizeInByte, plUInt64 uiSeed /*= 0*/)
{
