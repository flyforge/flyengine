...
class PL_FOUNDATION_DLL plDeduplicationReadContext : public plSerializationContext<plDeduplicationReadContext>
{
  PL_DECLARE_SERIALIZATION_CONTEXT(plDeduplicationReadContext);
...
public:
  ...
  plResult ReadObject(plStreamReader& inout_stream, T*& ref_pObject,
    plAllocator* pAllocator = plFoundation::GetDefaultAllocator()); // [tested]
  ...
  plResult ReadMap(plStreamReader& inout_stream, plMapBase<KeyType, ValueType, Comparer>& ref_map, ReadMapMode mode,
    plAllocator* pKeyAllocator = plFoundation::GetDefaultAllocator(),
    plAllocator* pValueAllocator = plFoundation::GetDefaultAllocator()); // [tested]

private:
  template <typename T>
  plResult ReadObject(plStreamReader& stream, T& obj, plAllocator* pAllocator); // [tested]
