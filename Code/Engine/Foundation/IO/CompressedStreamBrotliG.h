...
/// \brief A stream reader that will decompress data that was stored using the plCompressedStreamWriterBrotliG.
///
/// The reader takes another reader as its source for the compressed data (e.g. a file or a memory stream).
class PLASMA_FOUNDATION_DLL plCompressedStreamReaderBrotliG : public plStreamReader
{
public:
  plCompressedStreamReaderBrotliG(); // [tested]

  /// \brief Takes an input stream as the source from which to read the compressed data.
  plCompressedStreamReaderBrotliG(plStreamReader* pInputStream); // [tested]
  ...
  /// one.
  void SetInputStream(plStreamReader* pInputStream); // [tested]
  ...
  plUInt64 ReadBytes(void* pReadBuffer, plUInt64 uiBytesToRead) override; // [tested]

private:
  plDynamicArray<plUInt8> m_DecompressedData;
  plUInt64 m_uiCursor{0};
...
class PLASMA_FOUNDATION_DLL plCompressedStreamWriterBrotliG : public plStreamWriter
{
public:
  plCompressedStreamWriterBrotliG();

  /// \brief The constructor takes another stream writer to pass the output into.
  plCompressedStreamWriterBrotliG(plStreamWriter* pOutputStream); // [tested]

  /// \brief Calls FinishCompressedStream() internally.
  ~plCompressedStreamWriterBrotliG(); // [tested]

  /// \brief Configures to which other plStreamWriter the compressed data should be passed along.
  ///
  ...
  void SetOutputStream(plStreamWriter* pOutputStream, plUInt32 uiPageSizeKB = 64); // [tested]
  ...
  plResult FinishCompressedStream(); // [tested]

  /// \brief Returns the size of the data in its uncompressed state.
  PLASMA_NODISCARD PLASMA_ALWAYS_INLINE plUInt64 GetUncompressedSize() const { return m_uiUncompressedSize; } // [tested]
  ...
  PLASMA_NODISCARD PLASMA_ALWAYS_INLINE plUInt64 GetCompressedSize() const { return m_uiCompressedSize; } // [tested]
  ...
  PLASMA_NODISCARD PLASMA_ALWAYS_INLINE plUInt64 GetWrittenBytes() const { return m_uiWrittenBytes; } // [tested]
  ...
  virtual plResult Flush() override; // [tested]

private:
  plUInt64 m_uiUncompressedSize{0};
  plUInt64 m_uiCompressedSize{0};
  plUInt64 m_uiWrittenBytes{0};
  plUInt64 m_uiPageSize{65536};

  plStreamWriter* m_pOutputStream = nullptr;

  plDynamicArray<plUInt8> m_CompressedCache;
  plDynamicArray<plUInt8> m_UncompressedCache;
};
