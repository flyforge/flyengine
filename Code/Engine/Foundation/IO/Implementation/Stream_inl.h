...
#if PL_ENABLED(PL_PLATFORM_BIG_ENDIAN)

template <typename T>
plResult plStreamReader::ReadWordValue(T* pWordValue)
{
  PL_CHECK_AT_COMPILETIME(sizeof(T) == sizeof(plUInt16));
  ...
  *reinterpret_cast<plUInt16*>(pWordValue) = plEndianHelper::Switch(uiTemp);
  ...
  return (uiRead == sizeof(T)) ? PL_SUCCESS : PL_FAILURE;
}

template <typename T>
plResult plStreamReader::ReadDWordValue(T* pDWordValue)
{
  PL_CHECK_AT_COMPILETIME(sizeof(T) == sizeof(plUInt32));
...
{
  PL_CHECK_AT_COMPILETIME(sizeof(T) == sizeof(plUInt64));
...
plTypeVersion plStreamReader::ReadVersion(plTypeVersion expectedMaxVersion)
{
  ...
  PL_ASSERT_ALWAYS(v <= expectedMaxVersion, "Read version ({0}) is larger than expected max version ({1}).", v, expectedMaxVersion);
  PL_ASSERT_ALWAYS(v > 0, "Invalid version.");
...
{
  PL_ASSERT_ALWAYS(version > 0, "Version cannot be zero.");
...
namespace plStreamWriterUtil
{
  ...
  template <class T>
  PL_ALWAYS_INLINE auto SerializeImpl(plStreamWriter& inout_stream, const T& obj, int) -> decltype(inout_stream << obj, plResult(PL_SUCCESS))
  {
    inout_stream << obj;
  ...
  template <class T>
  PL_ALWAYS_INLINE auto SerializeImpl(plStreamWriter& inout_stream, const T& obj, long) -> decltype(obj.Serialize(inout_stream).IgnoreResult(), plResult(PL_SUCCESS))
  {
    return plToResult(obj.Serialize(inout_stream));
  }

  template <class T>
  PL_ALWAYS_INLINE auto SerializeImpl(plStreamWriter& inout_stream, const T& obj, float) -> decltype(obj.serialize(inout_stream).IgnoreResult(), plResult(PL_SUCCESS))
  {
    return plToResult(obj.serialize(inout_stream));
  }

  template <class T>
  PL_ALWAYS_INLINE auto Serialize(plStreamWriter& inout_stream, const T& obj) -> decltype(SerializeImpl(inout_stream, obj, 0).IgnoreResult(), plResult(PL_SUCCESS))
  {
  ...
  template <class T>
  PL_ALWAYS_INLINE auto SerializeArrayImpl(plStreamWriter& inout_stream, const T* pArray, plUInt64 uiCount, int) -> decltype(SerializeArray(inout_stream, pArray, uiCount), plResult(PL_SUCCESS))
  {
  ...
  plResult SerializeArrayImpl(plStreamWriter& inout_stream, const T* pArray, plUInt64 uiCount, long)
  {
    ...
    {
      PL_SUCCEED_OR_RETURN(plStreamWriterUtil::Serialize<T>(inout_stream, pArray[i]));
    }
  ...
  template <class T>
  PL_ALWAYS_INLINE plResult SerializeArray(plStreamWriter& inout_stream, const T* pArray, plUInt64 uiCount)
  {
...
plResult plStreamWriter::WriteArray(const plArrayBase<ValueType, ArrayType>& array)
{
  ...
  PL_SUCCEED_OR_RETURN(WriteQWordValue(&uiCount));
...
plResult plStreamWriter::WriteArray(const ValueType (&array)[uiSize])
{
  ...
  PL_SUCCEED_OR_RETURN(WriteQWordValue(&uiWriteSize));
...
plResult plStreamWriter::WriteSet(const plSetBase<KeyType, Comparer>& set)
{
  ...
  {
    PL_SUCCEED_OR_RETURN(plStreamWriterUtil::Serialize<KeyType>(*this, item));
  }
...
plResult plStreamWriter::WriteMap(const plMapBase<KeyType, ValueType, Comparer>& map)
{
  ...
  {
    PL_SUCCEED_OR_RETURN(plStreamWriterUtil::Serialize<KeyType>(*this, It.Key()));
    PL_SUCCEED_OR_RETURN(plStreamWriterUtil::Serialize<ValueType>(*this, It.Value()));
  }
...
{
  template <class T>
  PL_ALWAYS_INLINE auto DeserializeImpl(plStreamReader& inout_stream, T& ref_obj, int) -> decltype(inout_stream >> ref_obj, plResult(PL_SUCCESS))
  {
  ...
  template <class T>
  PL_ALWAYS_INLINE auto DeserializeImpl(plStreamReader& inout_stream, T& inout_obj, long) -> decltype(inout_obj.Deserialize(inout_stream).IgnoreResult(), plResult(PL_SUCCESS))
  {
    return plToResult(inout_obj.Deserialize(inout_stream));
  }

  template <class T>
  PL_ALWAYS_INLINE auto DeserializeImpl(plStreamReader& inout_stream, T& inout_obj, float) -> decltype(inout_obj.deserialize(inout_stream).IgnoreResult(), plResult(PL_SUCCESS))
  {
    return plToResult(inout_obj.deserialize(inout_stream));
  }

  template <class T>
  PL_ALWAYS_INLINE auto Deserialize(plStreamReader& inout_stream, T& inout_obj) -> decltype(DeserializeImpl(inout_stream, inout_obj, 0).IgnoreResult(), plResult(PL_SUCCESS))
  {
...
#if PL_DISABLED(PL_PLATFORM_WINDOWS_UWP)
  template <class T>
  PL_ALWAYS_INLINE auto DeserializeArrayImpl(plStreamReader& inout_stream, T* pArray, plUInt64 uiCount, int) -> decltype(DeserializeArray(inout_stream, pArray, uiCount), plResult(PL_SUCCESS))
  {
  ...
  plResult DeserializeArrayImpl(plStreamReader& inout_stream, T* pArray, plUInt64 uiCount, long)
  {
    for (plUInt64 i = 0; i < uiCount; ++i)
    {
      PL_SUCCEED_OR_RETURN(plStreamReaderUtil::Deserialize<T>(inout_stream, pArray[i]));
    }
  ...
  template <class T>
  PL_ALWAYS_INLINE plResult DeserializeArray(plStreamReader& inout_stream, T* pArray, plUInt64 uiCount)
  {
...
{
  plUInt64 uiCount = 0;
  PL_SUCCEED_OR_RETURN(ReadQWordValue(&uiCount));
  ...
  if (uiCount < plMath::MaxValue<plUInt32>())
  {
    ...
    if (uiCount > 0)
    {
      ...
      PL_SUCCEED_OR_RETURN(plStreamReaderUtil::DeserializeArray<ValueType>(*this, inout_array.GetData(), uiCount));
    }
...
plResult plStreamReader::ReadArray(plSmallArray<ValueType, uiSize, AllocatorWrapper>& ref_array)
{
  ...
  if (uiCount < plMath::MaxValue<plUInt16>())
  {
    ...
    if (uiCount > 0)
    {
      ...
      PL_SUCCEED_OR_RETURN(plStreamReaderUtil::DeserializeArray<ValueType>(*this, ref_array.GetData(), uiCount));
    }
...
plResult plStreamReader::ReadArray(ValueType (&array)[uiSize])
{
  ...
  if (uiCount < plMath::MaxValue<plUInt32>())
  {
    PL_SUCCEED_OR_RETURN(plStreamReaderUtil::DeserializeArray<ValueType>(*this, array, uiCount));
...
plResult plStreamReader::ReadSet(plSetBase<KeyType, Comparer>& inout_set)
{
  ...
  if (uiCount < plMath::MaxValue<plUInt32>())
  {
    ...
    {
      KeyType Item;
      PL_SUCCEED_OR_RETURN(plStreamReaderUtil::Deserialize(*this, Item));
...
plResult plStreamReader::ReadMap(plMapBase<KeyType, ValueType, Comparer>& inout_map)
{
  ...
  if (uiCount < plMath::MaxValue<plUInt32>())
  {
    ...
    for (plUInt32 i = 0; i < static_cast<plUInt32>(uiCount); ++i)
    {
      KeyType Key;
      ValueType Value;
      PL_SUCCEED_OR_RETURN(plStreamReaderUtil::Deserialize(*this, Key));
      PL_SUCCEED_OR_RETURN(plStreamReaderUtil::Deserialize(*this, Value));
