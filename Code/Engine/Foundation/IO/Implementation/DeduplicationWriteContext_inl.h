...
template <typename T>
PL_ALWAYS_INLINE plResult plDeduplicationWriteContext::WriteObject(plStreamWriter& inout_stream, const T& obj)
{
  return WriteObjectInternal(inout_stream, plInternal::WriteObjectHelper<T>::GetAddress(obj));
}

template <typename T>
PL_ALWAYS_INLINE plResult plDeduplicationWriteContext::WriteObject(plStreamWriter& inout_stream, const plSharedPtr<T>& pObject)
{
  return WriteObjectInternal(inout_stream, pObject.Borrow());
}

template <typename T>
PL_ALWAYS_INLINE plResult plDeduplicationWriteContext::WriteObject(plStreamWriter& inout_stream, const plUniquePtr<T>& pObject)
{
  return WriteObjectInternal(inout_stream, pObject.Borrow());
}
...
plResult plDeduplicationWriteContext::WriteArray(plStreamWriter& inout_stream, const plArrayBase<ValueType, ArrayType>& array)
{
  ...
  PL_SUCCEED_OR_RETURN(inout_stream.WriteQWordValue(&uiCount));

  for (plUInt32 i = 0; i < static_cast<plUInt32>(uiCount); ++i)
  {
    PL_SUCCEED_OR_RETURN(WriteObject(inout_stream, array[i]));
  }
...
plResult plDeduplicationWriteContext::WriteSet(plStreamWriter& inout_stream, const plSetBase<KeyType, Comparer>& set)
{
  ...
  PL_SUCCEED_OR_RETURN(inout_stream.WriteQWordValue(&uiWriteSize));

  for (const auto& item : set)
  {
    PL_SUCCEED_OR_RETURN(WriteObject(inout_stream, item));
  }
...
plResult plDeduplicationWriteContext::WriteMap(plStreamWriter& inout_stream, const plMapBase<KeyType, ValueType, Comparer>& map, WriteMapMode mode)
{
  ...
  if (mode == WriteMapMode::DedupKey)
  {
    ...
    {
      PL_SUCCEED_OR_RETURN(WriteObject(inout_stream, It.Key()));
      PL_SUCCEED_OR_RETURN(plStreamWriterUtil::Serialize<ValueType>(inout_stream, It.Value()));
    }
  }
  else if (mode == WriteMapMode::DedupValue)
  {
    for (auto It = map.GetIterator(); It.IsValid(); ++It)
    {
      PL_SUCCEED_OR_RETURN(plStreamWriterUtil::Serialize<KeyType>(inout_stream, It.Key()));
      PL_SUCCEED_OR_RETURN(WriteObject(inout_stream, It.Value()));
    }
  }
  else
  {
