...
#  include <direct.h>
#  define PLASMA_USE_OLD_POSIX_FUNCTIONS PLASMA_ON
#else
#  include <dirent.h>
#  include <fnmatch.h>
#  include <pwd.h>
...
#  include <sys/types.h>
#  include <unistd.h>
#  define PLASMA_USE_OLD_POSIX_FUNCTIONS PLASMA_OFF
#endif
...
#if PLASMA_DISABLED(PLASMA_PLATFORM_WINDOWS_UWP) // UWP does not support these functions
  int fd = -1;
  switch (OpenMode)
  {
...
{
#if PLASMA_ENABLED(PLASMA_USE_OLD_POSIX_FUNCTIONS)
...
void plOSFile::InternalSetFilePosition(plInt64 iDistance, plFileSeekMode::Enum Pos) const
{
  ...
  switch (Pos)
  {
    case plFileSeekMode::FromStart:
      PLASMA_VERIFY(fseek(m_FileData.m_pFileHandle, (long)iDistance, SEEK_SET) == 0, "Seek Failed");
      break;
    case plFileSeekMode::FromEnd:
      PLASMA_VERIFY(fseek(m_FileData.m_pFileHandle, (long)iDistance, SEEK_END) == 0, "Seek Failed");
      break;
    case plFileSeekMode::FromCurrent:
      PLASMA_VERIFY(fseek(m_FileData.m_pFileHandle, (long)iDistance, SEEK_CUR) == 0, "Seek Failed");
      break;
  }
#else
  switch (Pos)
  {
    case plFileSeekMode::FromStart:
      PLASMA_VERIFY(fseeko(m_FileData.m_pFileHandle, iDistance, SEEK_SET) == 0, "Seek Failed");
      break;
    case plFileSeekMode::FromEnd:
      PLASMA_VERIFY(fseeko(m_FileData.m_pFileHandle, iDistance, SEEK_END) == 0, "Seek Failed");
      break;
    case plFileSeekMode::FromCurrent:
      PLASMA_VERIFY(fseeko(m_FileData.m_pFileHandle, iDistance, SEEK_CUR) == 0, "Seek Failed");
      break;
  }
#endif
}
...
{
  FILE* pFile = fopen(plString(sFile), "r");
...
#if PLASMA_ENABLED(PLASMA_SUPPORTS_FILE_STATS) && PLASMA_DISABLED(PLASMA_PLATFORM_WINDOWS_UWP)
...
#  if PLASMA_ENABLED(PLASMA_PLATFORM_OSX)
    ...
    if (bundlePath != nullptr)
    {
      ...
      plArrayPtr<char> temp = PLASMA_DEFAULT_NEW_ARRAY(char, maxSize);
      ...
      PLASMA_DEFAULT_DELETE_ARRAY(temp);
    }
...
const plString plOSFile::GetCurrentWorkingDirectory()
{
  ...
  plStringBuilder clean = getcwd(tmp, PLASMA_ARRAY_SIZE(tmp));
  clean.MakeCleanPath();
...
void plFileSystemIterator::StartSearch(plStringView sSearchTerm, plBitflags<plFileSystemIteratorFlags> flags /*= plFileSystemIteratorFlags::All*/)
{
  ...
  {
    PLASMA_ASSERT_DEV(false, "Recursive file iteration does not support wildcards. Either don't use recursion, or filter the filenames manually.");
    return;
  }
  ...
  PLASMA_ASSERT_DEV(m_sCurPath.IsAbsolutePath(), "The path '{0}' is not absolute.", m_sCurPath);
...
#endif // PLASMA_DISABLED(PLASMA_PLATFORM_WINDOWS_UWP)
