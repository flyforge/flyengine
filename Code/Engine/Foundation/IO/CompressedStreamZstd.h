...
/// \brief A stream reader that will decompress data that was stored using the plCompressedStreamWriterZstd.
///
/// The reader takes another reader as its source for the compressed data (e.g. a file or a memory stream).
class PLASMA_FOUNDATION_DLL plCompressedStreamReaderZstd : public plStreamReader
{
public:
  plCompressedStreamReaderZstd(); // [tested]

  /// \brief Takes an input stream as the source from which to read the compressed data.
  plCompressedStreamReaderZstd(plStreamReader* pInputStream); // [tested]
...
class PLASMA_FOUNDATION_DLL plCompressedStreamWriterZstd final : public plStreamWriter
{
public:
  ...
  plUInt64 GetWrittenBytes() const { return m_uiWrittenBytes; } // [tested]
