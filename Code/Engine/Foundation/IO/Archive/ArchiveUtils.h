...
namespace plArchiveUtils
{
  ...
  PLASMA_FOUNDATION_DLL plHybridArray<plString, 4, plStaticAllocatorWrapper>& GetAcceptedArchiveFileExtensions();

  /// \brief Checks case insensitive, whether the given extension is in the list of GetAcceptedArchiveFileExtensions().
  PLASMA_FOUNDATION_DLL bool IsAcceptedArchiveFileExtensions(plStringView sExtension);

  /// \brief Writes the header that identifies the plArchive file and version to the stream
  PLASMA_FOUNDATION_DLL plResult WriteHeader(plStreamWriter& inout_stream);

  /// \brief Reads the plArchive header. Returns success and the version, if the stream is a valid plArchive file.
  PLASMA_FOUNDATION_DLL plResult ReadHeader(plStreamReader& inout_stream, plUInt8& out_uiVersion);

  /// \brief Writes the archive TOC to the stream. This must be the last thing in the stream, if ExtractTOC() is supposed to work.
  PLASMA_FOUNDATION_DLL plResult AppendTOC(plStreamWriter& inout_stream, const plArchiveTOC& toc);
  ...
  PLASMA_FOUNDATION_DLL plResult ExtractTOC(plMemoryMappedFile& ref_memFile, plArchiveTOC& ref_toc, plUInt8 uiArchiveVersion);
  ...
  PLASMA_FOUNDATION_DLL plResult WriteEntry(plStreamWriter& inout_stream, plStringView sAbsSourcePath, plUInt32 uiPathStringOffset,
  ...
  PLASMA_FOUNDATION_DLL plResult WriteEntryOptimal(plStreamWriter& inout_stream, plStringView sAbsSourcePath, plUInt32 uiPathStringOffset,
    plArchiveCompressionMode compression, plInt32 iCompressionLevel, plArchiveEntry& ref_tocEntry, plUInt64& inout_uiCurrentStreamPosition,
    FileWriteProgressCallback progress = FileWriteProgressCallback());
  ...
  PLASMA_FOUNDATION_DLL void ConfigureRawMemoryStreamReader(
  ...
  PLASMA_FOUNDATION_DLL plUniquePtr<plStreamReader> CreateEntryReader(const plArchiveEntry& entry, const void* pStartOfArchiveData);

  PLASMA_FOUNDATION_DLL plResult ReadZipHeader(plStreamReader& inout_stream, plUInt8& out_uiVersion);
  PLASMA_FOUNDATION_DLL plResult ExtractZipTOC(plMemoryMappedFile& ref_memFile, plArchiveTOC& ref_toc);
