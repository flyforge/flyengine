  ...
  plOSThreadEntryPoint threadEntryPoint, void* pUserData /*= nullptr*/, const char* szName /*= "plThread"*/, plUInt32 uiStackSize /*= 128 * 1024*/)
{
  ...
  PL_ASSERT_ALWAYS(threadEntryPoint != nullptr, "Thread entry point is invalid.");

  m_hHandle = CreateThread(nullptr, uiStackSize, threadEntryPoint, pUserData, CREATE_SUSPENDED, nullptr);
  PL_ASSERT_RELEASE(m_hHandle != INVALID_HANDLE_VALUE, "Thread creation failed!");
  PL_ASSERT_RELEASE(m_hHandle != nullptr, "Thread creation failed!"); // makes the static code analysis happy
