...
#  if PL_ENABLED(PL_PLATFORM_WINDOWS_UWP)
        ...
        HStringReference(RuntimeClass_Windows_Networking_Connectivity_NetworkInformation).Get(), &networkInformation)))
  {
    ...
    {
      plUwpUtils::plWinRtIterateIVectorView<IHostName*>(hostNames, [](UINT, IHostName* hostName)
        {
        ...
        if (hostNameType == HostNameType_DomainName)
        {
          ...
          plStringUtils::Copy(s_SystemInformation.m_sHostName, sizeof(s_SystemInformation.m_sHostName), plStringUtf8(name).GetData());
          return false;
        }
...
#  if PL_ENABLED(PL_PLATFORM_WINDOWS_DESKTOP)
  ...
  return plMath::Clamp(util, 0.f, 1.f) * 100.f;
