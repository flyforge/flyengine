...
bool plPipeChannel_win::CreatePipe(plStringView sAddress)
{
  ...
  if (m_hPipeHandle != INVALID_HANDLE_VALUE)
  {
    ...
    PL_IGNORE_UNUSED(port);
    PL_ASSERT_DEBUG(pMsgLoopWin->GetPort() == port, "Failed to CreateIoCompletionPort: {0}", plArgErrorCode(GetLastError()));
  }
  return true;
}
...
#  if PL_ENABLED(PL_COMPILE_FOR_DEBUG)
  if (m_ThreadId != 0)
    PL_ASSERT_DEBUG(m_ThreadId == plThreadUtils::GetCurrentThreadID(), "Function must be called from worker thread!");
#  endif
...
bool plPipeChannel_win::ProcessIncomingMessages(DWORD uiBytesRead)
{
  ...
  while (true)
  {
    ...
    PL_ASSERT_DEBUG(uiBytesRead != 0, "We really should have data at this point.");
...
{
  PL_ASSERT_DEBUG(IsConnected(), "Must be connected to process outgoing messages.");
  PL_ASSERT_DEBUG(m_ThreadId == plThreadUtils::GetCurrentThreadID(), "Function must be called from worker thread!");
...
void plPipeChannel_win::OnIOCompleted(IOContext* pContext, DWORD uiBytesTransfered, DWORD uiError)
{
  ...
  {
    PL_ASSERT_DEBUG(pContext == &m_OutputState.Context, "");
