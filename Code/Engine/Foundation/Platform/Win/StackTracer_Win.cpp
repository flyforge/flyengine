...
namespace
{
  ...
  struct StackTracerImplementation
  {
    ...
    StackTracerImplementation()
    {
      ...
      PL_ASSERT_DEV(kernel32Dll != nullptr, "StackTracer could not load kernel32.dll");
      ...
      PL_ASSERT_DEV(dbgHelpDll != nullptr, "StackTracer could not load dbghelp.dll");
  ...
  static void Initialize()
  {
    ...
    {
      alignas(PL_ALIGNMENT_OF(StackTracerImplementation)) static plUInt8 ImplementationBuffer[sizeof(StackTracerImplementation)];
      s_pImplementation = new (ImplementationBuffer) StackTracerImplementation();
      PL_ASSERT_DEV(s_pImplementation != nullptr, "StackTracer initialization failed");
    }
  }
  ...
  static bool SymbolInitializeImpl()
  {
    ...
    // so when using PL together with for example a third party library that also records stack traces, just calling SymInitialize will fail
...
void plStackTracer::OnPluginEvent(const plPluginEvent& e)
{
  ...
  if (false) // e.m_EventType == plPluginEvent::AfterLoading)
  {
    ...
    mbstowcs(szPluginPath, buffer, PL_ARRAY_SIZE(szPluginPath));

    wchar_t szPluginName[256];
    mbstowcs(szPluginName, e.m_sPluginBinary.GetData(tmp), PL_ARRAY_SIZE(szPluginName));
    ...
    if (!(*s_pImplementation->getModuleInfo)(currentProcess, moduleAddress, &moduleInfo))
    {
      ...
      char errStr[1024];
      plStringUtils::snprintf(errStr, 1024, "StackTracer could not get module info for '%s'. Error-Code %u (\"%s\")\n", e.m_sPluginBinary.GetData(tmp), err, static_cast<char*>(lpMsgBuf));
      plLog::Print(errStr);
...
void plStackTracer::ResolveStackTrace(const plArrayPtr<void*>& trace, PrintFunc printFunc)
{
  ...
  if (s_pImplementation->symbolFromAddress != nullptr && s_pImplementation->lineFromAdress != nullptr)
  {
    ...
    for (plUInt32 i = 0; i < uiNumTraceEntries; i++)
    {
      ...
      char finalStr[1024];
      wcstombs(finalStr, str, PL_ARRAY_SIZE(finalStr));
