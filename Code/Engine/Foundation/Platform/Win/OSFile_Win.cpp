...
plResult plOSFile::InternalOpen(plStringView sFile, plFileOpenMode::Enum OpenMode, plFileShareMode::Enum FileShareMode)
{
  ...
  while (iRetries > 0)
  {
    ...
    const plResult res = ((m_FileData.m_pFileHandle != nullptr) && (m_FileData.m_pFileHandle != INVALID_HANDLE_VALUE)) ? PL_SUCCESS : PL_FAILURE;
...
void plOSFile::InternalSetFilePosition(plInt64 iDistance, plFileSeekMode::Enum Pos) const
{
  ...
  switch (Pos)
  {
    case plFileSeekMode::FromStart:
      PL_VERIFY(SetFilePointerEx(m_FileData.m_pFileHandle, pos, &newpos, FILE_BEGIN), "Seek Failed.");
      break;
    case plFileSeekMode::FromEnd:
      PL_VERIFY(SetFilePointerEx(m_FileData.m_pFileHandle, pos, &newpos, FILE_END), "Seek Failed.");
      break;
    case plFileSeekMode::FromCurrent:
      PL_VERIFY(SetFilePointerEx(m_FileData.m_pFileHandle, pos, &newpos, FILE_CURRENT), "Seek Failed.");
      break;
  }
}
...
#  endif // not PL_USE_POSIX_FILE_API
...
void plFileSystemIterator::StartSearch(plStringView sSearchStart, plBitflags<plFileSystemIteratorFlags> flags /*= plFileSystemIteratorFlags::All*/)
{
  ...
  PL_ASSERT_DEV(flags.IsSet(plFileSystemIteratorFlags::Recursive) == false || bHasWildcard == false, "Recursive file iteration does not support wildcards. Either don't use recursion, or filter the filenames manually.");

  m_sCurPath = sSearch.GetFileDirectory();

  PL_ASSERT_DEV(sSearch.IsAbsolutePath(), "The path '{0}' is not absolute.", m_sCurPath);
...
plStringView plOSFile::GetApplicationDirectory()
{
  if (s_sApplicationPath.IsEmpty())
  {
    ...
    while (true)
    {
      ...
      PL_REPORT_FAILURE("GetModuleFileNameW failed: {0}", plArgErrorCode(error));
    }
...
const plString plOSFile::GetCurrentWorkingDirectory()
{
  ...
  {
    PL_REPORT_FAILURE("GetCurrentDirectoryW failed: {}", plArgErrorCode(GetLastError()));
    return plString();
  }
