...
{
  PL_ASSERT_ALWAYS(pFilter == nullptr, "Not implemented");
  ...
  if (result > 0)
  {
    ...
    if (m_pollInfos[0].revents != 0)
    {
      ...
      PL_IGNORE_UNUSED(readResult);
      PL_ASSERT_DEV(readResult == sizeof(wakeupByte), "Wakeup byte not read");
      m_pollInfos[0].revents = 0;
      return true;
    }

    for (plUInt32 i = 1; i < m_waitInfos.GetCount();)
    {
...
void plMessageLoop_linux::RegisterWait(plPipeChannel_linux* pChannel, WaitType type, int fd)
{
  ...
  PL_SCOPE_EXIT(m_numPendingPollModifications.Decrement());
  WakeUp();
  {
    plLock lock{m_pollMutex};
...
void plMessageLoop_linux::RemovePendingWaits(plPipeChannel_linux* pChannel)
{
  ...
  {
    plLock lock{m_pollMutex};
    for (plUInt32 i = 0; i < m_pollInfos.GetCount();)
    {
...
void plMessageLoop_linux::WakeUp()
{
  ...
  PL_IGNORE_UNUSED(writeResult);
  PL_ASSERT_DEV(writeResult == sizeof(wakeupByte), "Wakeup byte not written");
}
