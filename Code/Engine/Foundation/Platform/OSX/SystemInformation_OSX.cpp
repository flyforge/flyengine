...
void plSystemInformation::Initialize()
{
  ...
  s_SystemInformation.m_uiCPUCoreCount = static_cast<plUInt32>(sysconf(_SC_NPROCESSORS_ONLN));

  plUInt64 uiPageSize = sysconf(_SC_PAGE_SIZE);

  s_SystemInformation.m_uiMemoryPageSize = static_cast<plUInt32>(uiPageSize);
...
plUInt64 plSystemInformation::GetAvailableMainMemory() const
{
  ...
  return static_cast<plUInt64>(s_SystemInformation.m_uiMemoryPageSize) * static_cast<plUInt64>(vmt.t_free);
}
