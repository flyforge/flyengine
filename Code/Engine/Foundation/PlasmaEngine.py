...
def __lldb_init_module(debugger, internal_dict):
    ...
    debugger.HandleCommand('type synthetic add -x "^plDynamicArray<" --python-class PlasmaEngine.plDynamicArraySynthProvider')
    debugger.HandleCommand('type synthetic add -x "^plHybridArray<" --python-class PlasmaEngine.plHybridArraySynthProvider')
    debugger.HandleCommand('type synthetic add -x "^plHybridString<" --python-class PlasmaEngine.plHybridStringSynthProvider')
    debugger.HandleCommand('type synthetic add plStringBuilder --python-class PlasmaEngine.plHybridStringSynthProvider')
    debugger.HandleCommand('type synthetic add plStringView --python-class PlasmaEngine.plStringViewSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^plEnum<" --python-class PlasmaEngine.plEnumSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^plArrayPtr<" --python-class PlasmaEngine.plArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add plByteArrayPtr --python-class PlasmaEngine.plArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add plConstByteArrayPtr --python-class PlasmaEngine.plArrayPtrSynthProvider')
    debugger.HandleCommand('type synthetic add -x "^plMap<" --python-class PlasmaEngine.plMapSynthProvider')

    debugger.HandleCommand('type summary add -x "^plHybridString<" --python-function PlasmaEngine.plHybridString_SummaryProvider')
    debugger.HandleCommand('type summary add plStringBuilder --python-function PlasmaEngine.plHybridString_SummaryProvider')
    debugger.HandleCommand('type summary add plStringView --python-function PlasmaEngine.plStringView_SummaryProvider')
    debugger.HandleCommand('type summary add -x "^plEnum<" --python-function PlasmaEngine.plEnum_SummaryProvider')
...
class plDynamicArraySynthProvider:
...
class plHybridArraySynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
...
class plHybridStringSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
...
def plHybridString_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "plHybridString_SummaryProvider"
...
class plStringViewSynthProvider:
    def __init__(self, valobj, dict):
        logger = lldb.formatters.Logger.Logger()
        self.valobj = valobj
    ...
    def update(self):
        logger = lldb.formatters.Logger.Logger()
        logger >> "plStringViewSynthProvider::update"
        self.valid = False
        try:
...
def plStringView_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "plStringView_SummaryProvider"
        content = valobj.GetChildAtIndex(0)
        return '"' + make_string(content) + '"'
    except Exception as inst:
        logger >> str(inst)
...
class plEnumSynthProvider:
    ...
    def update(self):
        self.value = "?"
        logger = lldb.formatters.Logger.Logger()
        logger >> "plEnumSynthProvider::Update"
        try:
...
def plEnum_SummaryProvider(valobj, dict):
    logger = lldb.formatters.Logger.Logger()
    try:
        logger >> "plEnum_SummaryProvider"
        content = valobj.GetChildAtIndex(0)
        return make_string(content)
    except Exception as inst:
        logger >> str(inst)
