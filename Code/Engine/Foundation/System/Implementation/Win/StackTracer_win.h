#ifdef PLASMA_STACKTRACER_WIN_INL_H_INCLUDED
#  error "This file must not be included twice."
#endif

#define PLASMA_STACKTRACER_WIN_INL_H_INCLUDED

#include <Foundation/FoundationInternal.h>
PLASMA_FOUNDATION_INTERNAL_HEADER

#define PLASMA_MSVC_WARNING_NUMBER 4091
...
namespace
{
  ...
  struct StackTracerImplementation
  {
    ...
    {
      plMemoryUtils::ZeroFill(this, 1);

      kernel32Dll = LoadLibraryW(L"kernel32.dll");
      PLASMA_ASSERT_DEV(kernel32Dll != nullptr, "StackTracer could not load kernel32.dll");
      ...
      PLASMA_ASSERT_DEV(dbgHelpDll != nullptr, "StackTracer could not load dbghelp.dll");
  ...
  static void Initialize()
  {
    ...
    {
      alignas(PLASMA_ALIGNMENT_OF(StackTracerImplementation)) static plUInt8 ImplementationBuffer[sizeof(StackTracerImplementation)];
      s_pImplementation = new (ImplementationBuffer) StackTracerImplementation();
      PLASMA_ASSERT_DEV(s_pImplementation != nullptr, "StackTracer initialization failed");
    }
  }
  ...
  static bool SymbolInitializeImpl()
  {
    ...
    // so when using PLASMA together with for example a third party library that also records stack traces, just calling SymInitialize will fail
  ...
  static void SymbolInitialize()
  {
    ...
    {
      plLog::Error("StackTracer could not initialize symbols. Error-Code {0}", plArgErrorCode(::GetLastError()));
      return;
    }
    ...
    if (!(*s_pImplementation->symSetSearchPath)(GetCurrentProcess(), plStringWChar(plOSFile::GetApplicationDirectory())))
    {
      plLog::Error("StackTracer could not set symbol search path. Error-Code {0}", plArgErrorCode(::GetLastError()));
      return;
    }
  }
} // namespace
...
void plStackTracer::OnPluginEvent(const plPluginEvent& e)
{
  ...
  if (e.m_EventType == plPluginEvent::BeforeUnloading)
  {
    SymbolInitialize();
  }

  if (false) // e.m_EventType == plPluginEvent::AfterLoading)
  {
    plStringBuilder tmp;

    char buffer[1024];
    strcpy_s(buffer, plOSFile::GetApplicationDirectory().GetStartPointer());
    strcat_s(buffer, e.m_sPluginBinary.GetData(tmp));
    strcat_s(buffer, ".dll");

    wchar_t szPluginPath[1024];
    mbstowcs(szPluginPath, buffer, PLASMA_ARRAY_SIZE(szPluginPath));

    wchar_t szPluginName[256];
    mbstowcs(szPluginName, e.m_sPluginBinary.GetData(tmp), PLASMA_ARRAY_SIZE(szPluginName));
    ...
    if (moduleAddress == 0)
    {
      ...
      {
        plLog::Error("StackTracer could not load symbols for '{0}'. Error-Code {1}", e.m_sPluginBinary, plArgErrorCode(err));
      }
    ...
    plMemoryUtils::ZeroFill(&moduleInfo, 1);
...
// static
plUInt32 plStackTracer::GetStackTrace(plArrayPtr<void*>& ref_trace, void* pContext)
{
  Initialize();
...
#  error Unsupported platform
    ...
    for (plInt32 i = 0; i < (plInt32)ref_trace.GetCount(); i++)
    {
      ...
      else
      {
        ...
        return plMath::Max(i - 4, 0);
      }
    }
  }
  else if (s_pImplementation->captureStackBackTrace != nullptr)
  {
    const plUInt32 uiSkip = 1;
    const plUInt32 uiMaxNumTrace = plMath::Min(62U, ref_trace.GetCount());
    plInt32 iNumTraces = (*s_pImplementation->captureStackBackTrace)(uiSkip, uiMaxNumTrace, ref_trace.GetPtr(), nullptr);

    // skip the last three stack-frames since they are useless
    return plMath::Max(iNumTraces - 3, 0);
  }
...
void plStackTracer::ResolveStackTrace(const plArrayPtr<void*>& trace, PrintFunc printFunc)
{
  ...
  if (s_pImplementation->symbolFromAddress != nullptr && s_pImplementation->lineFromAdress != nullptr)
  {
    ...
    for (plUInt32 i = 0; i < uiNumTraceEntries; i++)
    {
      ...
      char finalStr[1024];
      wcstombs(finalStr, str, PLASMA_ARRAY_SIZE(finalStr));
