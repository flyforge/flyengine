...
public:
  ...
  static plResult MakePipe(plFd (&fds)[2], int flags = 0)
  {
    fds[0].Close();
    fds[1].Close();
#if PL_ENABLED(PL_USE_LINUX_POSIX_EXTENSIONS)
...
struct plProcessImpl
{
      ...
      if (result > 0)
      {
        ...
        for (plUInt32 i = 1; i < pollfds.GetCount(); ++i)
        {
          if (pollfds[i].revents & POLLIN)
          {
            ...
            while (true)
            {
              ssize_t numBytes = read(stream.fd.Borrow(), buffer, PL_ARRAY_SIZE(buffer));
              if (numBytes < 0)
              {
              ...
              if (numBytes < PL_ARRAY_SIZE(buffer))
              {
                break;
              }
            }
  ...
  plResult StartStreamWatcher()
  {
    ...
    m_streamWatcherThread = PL_DEFAULT_NEW(plOSThread, &StreamWatcherThread, this, "StdStrmWtch");
  ...
  void StopStreamWatcher()
  {
    ...
    {
      char c = 0;
      PL_IGNORE_UNUSED(write(m_wakeupPipeWriteEnd.Borrow(), &c, 1));
  ...
  static plResult StartChildProcess(const plProcessOptions& opt, pid_t& outPid, bool suspended, plFd& outStdOutFd, plFd& outStdErrFd)
  {
    ...
    if (childPid == 0) // We are the child
    {
      ...
      if (!opt.m_sWorkingDirectory.IsEmpty())
      {
        if (chdir(opt.m_sWorkingDirectory.GetData()) < 0)
        {
          ...
          PL_IGNORE_UNUSED(write(startupErrorPipe[1].Borrow(), &err, sizeof(err)));
    ...
    else
    {
      ...
      if (errSize > 0)
      {
        PL_ASSERT_DEV(errSize == sizeof(err), "Child process should have written a full ProcessStartupError struct");
        switch (err.type)
        {
...
{
  PL_ASSERT_DEV(m_pImpl->m_childPid == -1, "Can not reuse an instance of plProcess");
...
{
  int childStatus = 0;
  PL_SCOPE_EXIT(m_pImpl->StopStreamWatcher());
...
{
  PL_ASSERT_DEV(false, "There is no process handle on posix");
  return nullptr;
}

plOsProcessID plProcess::GetProcessID() const
{
  PL_ASSERT_DEV(m_pImpl->m_childPid != -1, "No ProcessID available");
