...
public:
  ...
  plUInt64 GetInstalledMainMemory() const { return m_uiInstalledMainMemory; }
  ...
  plUInt32 GetMemoryPageSize() const { return m_uiMemoryPageSize; }

  /// \brief Returns the CPU core count of the system.
  plUInt32 GetCPUCoreCount() const { return m_uiCPUCoreCount; }
  ...
  const plCpuFeatures& GetCpuFeatures() const { return m_CpuFeatures; }
...
private:
  plUInt64 m_uiInstalledMainMemory;
  plUInt32 m_uiMemoryPageSize;
  plUInt32 m_uiCPUCoreCount;
  const char* m_szPlatformName = nullptr;
  const char* m_szBuildConfiguration = nullptr;
  char m_sHostName[256];
  bool m_bB64BitOS;
  bool m_bIsInitialized;
  plCpuFeatures m_CpuFeatures;

  static void Initialize();

  static plSystemInformation s_SystemInformation;
};
