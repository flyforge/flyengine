...
public:
  ...
  explicit plPointerWithFlags(PtrType* pPtr, plUInt8 uiFlags = 0) { SetPtrAndFlags(pPtr, uiFlags); }
  ...
  {
    const std::uintptr_t& iptr = *reinterpret_cast<const std::uintptr_t*>(&m_pPtr);
    return static_cast<plUInt8>(iptr & FlagsMask);
  }
