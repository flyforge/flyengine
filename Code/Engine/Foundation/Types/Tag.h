...
class PL_FOUNDATION_DLL plTag
{
public:
  PL_ALWAYS_INLINE plTag();

  PL_ALWAYS_INLINE bool operator==(const plTag& rhs) const; // [tested]

  PL_ALWAYS_INLINE bool operator!=(const plTag& rhs) const; // [tested]

  PL_ALWAYS_INLINE bool operator<(const plTag& rhs) const;

  PL_ALWAYS_INLINE const plString& GetTagString() const; // [tested]

  PL_ALWAYS_INLINE bool IsValid() const; // [tested]
