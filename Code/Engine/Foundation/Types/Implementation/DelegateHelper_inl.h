...
struct PL_FOUNDATION_DLL plLambdaDelegateStorageBase
{
  plLambdaDelegateStorageBase() = default;
  virtual ~plLambdaDelegateStorageBase() = default;
  virtual plLambdaDelegateStorageBase* Clone(plAllocator* pAllocator) const = 0;
...
public:
  virtual plLambdaDelegateStorageBase* Clone(plAllocator* pAllocator) const override
  {
    if constexpr (std::is_copy_constructible<Function>::value)
    {
      return PL_NEW(pAllocator, plLambdaDelegateStorage<Function>, m_func);
    }
    else
    {
      PL_REPORT_FAILURE("The plDelegate stores a lambda that is not copyable. Copying this plDelegate is not supported.");
      return nullptr;
    }
  }
  ...
  virtual void InplaceMove(plUInt8* pBuffer) override
  {
    ...
    {
      PL_REPORT_FAILURE("The plDelegate stores a lambda that is not movable. Moving this plDelegate is not supported.");
    }
  }
...
public:
  PL_ALWAYS_INLINE plDelegate()
    : m_DispatchFunction(nullptr)
  {
  }

  PL_ALWAYS_INLINE plDelegate(const SelfType& other) { *this = other; }

  PL_ALWAYS_INLINE plDelegate(SelfType&& other) { *this = std::move(other); }
  ...
  {
    PL_CHECK_AT_COMPILETIME_MSG(DataSize >= 16, "DataSize must be at least 16 bytes");
    ...
    else
    {
      ...
      else
      {
        ...
        m_pLambdaStorage = PL_NEW(pAllocator, plLambdaDelegateStorage<Function>, std::move(function));
  ...
  PL_ALWAYS_INLINE ~plDelegate() { Invalidate(); }
  ...
  PL_FORCE_INLINE void operator=(std::nullptr_t) { Invalidate(); }
  ...
  {
    PL_ASSERT_DEBUG(m_DispatchFunction != nullptr, "Delegate is not bound.");
  ...
  {
    PL_REPORT_FAILURE("operator== for plDelegate must not be used. Use IsEqualIfNotHeapAllocated() and read its documentation!");
    return false;
  }
  ...
  PL_ALWAYS_INLINE bool IsValid() const { return m_DispatchFunction != nullptr; }
  ...
  PL_FORCE_INLINE void Invalidate()
  {
    ...
    if (IsHeapLambda())
    {
      PL_DELETE(m_pAllocator, m_pLambdaStorage);
    }
  ...
  PL_ALWAYS_INLINE void* GetClassInstance() const { return IsComparable() ? m_Instance.m_Ptr : nullptr; }
  ...
  PL_ALWAYS_INLINE bool IsComparable() const { return m_Instance.m_ConstPtr < InplaceLambda(); } // [tested]

private:
  template <typename Function>
  PL_FORCE_INLINE void CopyFunctionToInplaceStorage(Function function)
  {
    PL_ASSERT_DEBUG(
      plMemoryUtils::IsAligned(&m_Data, PL_ALIGNMENT_OF(Function)), "Wrong alignment. Expected {0} bytes alignment", PL_ALIGNMENT_OF(Function));
  ...
  PL_FORCE_INLINE void CopyMemberFunctionToInplaceStorage(Method method)
  {
    PL_CHECK_AT_COMPILETIME_MSG(DataSize >= 16, "DataSize must be at least 16 bytes");
    PL_CHECK_AT_COMPILETIME_MSG(sizeof(Method) <= DataSize, "Member function pointer must not be bigger than 16 bytes");
...
#if PL_ENABLED(PL_COMPILER_MSVC) || (PL_ENABLED(PL_PLATFORM_WINDOWS) && PL_ENABLED(PL_COMPILER_CLANG))
    *reinterpret_cast<plUInt32*>(m_Data + 12) = 0;
#endif
  }

  PL_ALWAYS_INLINE bool IsInplaceLambda() const { return m_Instance.m_ConstPtr == InplaceLambda(); }
  PL_ALWAYS_INLINE bool IsHeapLambda() const { return m_Instance.m_ConstPtr == HeapLambda(); }

  template <typename Method, typename Class>
  static PL_FORCE_INLINE R DispatchToMethod(const SelfType& self, Args... params)
  {
    PL_ASSERT_DEBUG(self.m_Instance.m_Ptr != nullptr, "Instance must not be null.");
    Method method = *reinterpret_cast<Method*>(&self.m_Data);
    return (static_cast<Class*>(self.m_Instance.m_Ptr)->*method)(params...);
  }

  template <typename Method, typename Class>
  static PL_FORCE_INLINE R DispatchToConstMethod(const SelfType& self, Args... params)
  {
    PL_ASSERT_DEBUG(self.m_Instance.m_ConstPtr != nullptr, "Instance must not be null.");
    Method method = *reinterpret_cast<Method*>(&self.m_Data);
    return (static_cast<const Class*>(self.m_Instance.m_ConstPtr)->*method)(params...);
  }

  template <typename Function>
  static PL_ALWAYS_INLINE R DispatchToFunction(const SelfType& self, Args... params)
  {
    return (*reinterpret_cast<Function*>(&self.m_Data))(params...);
  }

  template <typename Function>
  static PL_ALWAYS_INLINE R DispatchToHeapLambda(const SelfType& self, Args... params)
  {
    return static_cast<plLambdaDelegateStorage<Function>*>(self.m_pLambdaStorage)->m_func(params...);
  }

  template <typename Function>
  static PL_ALWAYS_INLINE R DispatchToInplaceLambda(const SelfType& self, Args... params)
  {
