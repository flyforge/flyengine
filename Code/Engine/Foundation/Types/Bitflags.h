...
///
/// The easier, less powerful way: Use the PLASMA_DECLARE_FLAGS() macro.\n
/// Example:\n
/// \code{.cpp}
///   PLASMA_DECLARE_FLAGS(plUInt8, SimpleRenderFlags, EnableEffects, EnableLighting, EnableShadows);
/// \endcode
...
///     enum Enum
///     {
///       EnableEffects   = PLASMA_BIT(0),
///       EnableLighting  = PLASMA_BIT(1),
///       EnableShadows   = PLASMA_BIT(2),
...
///   };
///
///   PLASMA_DECLARE_FLAGS_OPERATORS(SimpleRenderFlags);
/// \endcode
///
...
/// Finally the macro PLASMA_DECLARE_FLAGS_OPERATORS will define the required operator to be able to combine bitflags of your type.
...
public:
  ...
  PLASMA_ALWAYS_INLINE plBitflags()
    : m_Value(T::Default) // [tested]
  {
  }

  /// \brief Converts the incoming type to plBitflags<T>
  PLASMA_ALWAYS_INLINE plBitflags(Enum flag1) // [tested]
  {
    m_Value = (StorageType)flag1;
  }

  PLASMA_ALWAYS_INLINE void operator=(Enum flag1) { m_Value = (StorageType)flag1; }

  /// \brief Comparison operator.
  PLASMA_ALWAYS_INLINE bool operator==(const StorageType rhs) const // [tested]
  {
    return m_Value == rhs;
  }

  /// \brief Comparison operator.
  PLASMA_ALWAYS_INLINE bool operator!=(const StorageType rhs) const { return m_Value != rhs; }

  /// \brief Comparison operator.
  PLASMA_ALWAYS_INLINE bool operator==(const plBitflags<T>& rhs) const { return m_Value == rhs.m_Value; }

  /// \brief Comparison operator.
  PLASMA_ALWAYS_INLINE bool operator!=(const plBitflags<T>& rhs) const { return m_Value != rhs.m_Value; }

  /// \brief Clears all flags
  PLASMA_ALWAYS_INLINE void Clear() // [tested]
  {
    m_Value = 0;
  }

  /// \brief Checks if certain flags are set within the bitfield.
  PLASMA_ALWAYS_INLINE bool IsSet(Enum flag) const // [tested]
  {
    return (m_Value & flag) != 0;
  }

  /// \brief Returns whether all the given flags are set.
  PLASMA_ALWAYS_INLINE bool AreAllSet(const plBitflags<T>& rhs) const // [tested]
  {
    return (m_Value & rhs.m_Value) == rhs.m_Value;
  }

  /// \brief Returns whether none of the given flags is set.
  PLASMA_ALWAYS_INLINE bool AreNoneSet(const plBitflags<T>& rhs) const // [tested]
  {
    return (m_Value & rhs.m_Value) == 0;
  }

  /// \brief  Returns whether any of the given flags is set.
  PLASMA_ALWAYS_INLINE bool IsAnySet(const plBitflags<T>& rhs) const // [tested]
  {
    return (m_Value & rhs.m_Value) != 0;
  }

  /// \brief Sets the given flag.
  PLASMA_ALWAYS_INLINE void Add(const plBitflags<T>& rhs) // [tested]
  {
    m_Value |= rhs.m_Value;
  }

  /// \brief Removes the given flag.
  PLASMA_ALWAYS_INLINE void Remove(const plBitflags<T>& rhs) // [tested]
  {
    m_Value &= (~rhs.m_Value);
  }

  /// \brief Toggles the state of the given flag.
  PLASMA_ALWAYS_INLINE void Toggle(const plBitflags<T>& rhs) // [tested]
  {
    m_Value ^= rhs.m_Value;
  }

  /// \brief Sets or clears the given flag.
  PLASMA_ALWAYS_INLINE void AddOrRemove(const plBitflags<T>& rhs, bool bState) // [tested]
  {
  ...
  PLASMA_ALWAYS_INLINE plBitflags<T> operator|(const plBitflags<T>& rhs) const // [tested]
  {
  ...
  PLASMA_ALWAYS_INLINE void operator|=(const plBitflags<T>& rhs) // [tested]
  {
    m_Value |= rhs.m_Value;
  }
  ...
  PLASMA_ALWAYS_INLINE bool IsNoFlagSet() const // [tested]
  {
    return m_Value == 0;
  }

  /// \brief Returns true if any bitflag is set.
  PLASMA_ALWAYS_INLINE bool IsAnyFlagSet() const // [tested]
  {
    return m_Value != 0;
  }

private:
  PLASMA_ALWAYS_INLINE explicit plBitflags(StorageType flags)
    : m_Value(flags)
  {
  }
...
#define PLASMA_DECLARE_FLAGS_OPERATORS(FlagsType)                                      \
...
///
/// Usage: PLASMA_DECLARE_FLAGS(plUInt32, FlagsTypeName, Flag1Name, Flag2Name, Flag3Name, Flag4Name, ...)
///
...
#define PLASMA_DECLARE_FLAGS_WITH_DEFAULT(InternalStorageType, BitflagsTypeName, DefaultValue, ...) \
  struct BitflagsTypeName                                                                       \
  {                                                                                             \
    static const plUInt32 Count = PLASMA_VA_NUM_ARGS(__VA_ARGS__);                                  \
    using StorageType = InternalStorageType;                                                    \
    enum Enum                                                                                   \
    {                                                                                           \
      PLASMA_EXPAND_ARGS_WITH_INDEX(PLASMA_DECLARE_FLAGS_ENUM, ##__VA_ARGS__) Default = DefaultValue    \
    };                                                                                          \
    struct Bits                                                                                 \
    {                                                                                           \
      PLASMA_EXPAND_ARGS(PLASMA_DECLARE_FLAGS_BITS, ##__VA_ARGS__)                                      \
    };                                                                                          \
    PLASMA_ENUM_TO_STRING(__VA_ARGS__)                                                              \
  };                                                                                            \
  PLASMA_DECLARE_FLAGS_OPERATORS(BitflagsTypeName)

#define PLASMA_DECLARE_FLAGS(InternalStorageType, BitflagsTypeName, ...) \
  PLASMA_DECLARE_FLAGS_WITH_DEFAULT(InternalStorageType, BitflagsTypeName, 0, ##__VA_ARGS__)
/// \cond

/// Internal Do not use.
#define PLASMA_DECLARE_FLAGS_ENUM(name, n) name = PLASMA_BIT(n),

/// Internal Do not use.
#define PLASMA_DECLARE_FLAGS_BITS(name) StorageType name : 1;
...
/// \brief Similar to plBitflags but without type-safety.
///
...
template <typename T>
struct plTypelessBitflags
{
public:
  using StorageType = T;

  /// \brief Initializes the flags zero.
  PLASMA_ALWAYS_INLINE plTypelessBitflags() // [tested]
    : m_Value(0)
  {
  }

  PLASMA_ALWAYS_INLINE plTypelessBitflags(StorageType flags) // [tested]
  {
    m_Value = flags;
  }

  PLASMA_ALWAYS_INLINE void operator=(StorageType flags) // [tested]
  {
    m_Value = flags;
  }

  PLASMA_ALWAYS_INLINE bool operator==(const StorageType rhs) const { return m_Value == rhs; }                              // [tested]
  PLASMA_ALWAYS_INLINE bool operator!=(const StorageType rhs) const { return m_Value != rhs; }                              // [tested]
  PLASMA_ALWAYS_INLINE bool operator==(const plTypelessBitflags<StorageType>& rhs) const { return m_Value == rhs.m_Value; } // [tested]
  PLASMA_ALWAYS_INLINE bool operator!=(const plTypelessBitflags<StorageType>& rhs) const { return m_Value != rhs.m_Value; } // [tested]

  /// \brief Clears all flags
  PLASMA_ALWAYS_INLINE void Clear() { m_Value = 0; } // [tested]

  /// \brief Returns whether all the given flags are set.
  PLASMA_ALWAYS_INLINE bool AreAllSet(const plTypelessBitflags<StorageType>& rhs) const // [tested]
  {
    return (m_Value & rhs.m_Value) == rhs.m_Value;
  }

  /// \brief Returns whether none of the given flags is set.
  PLASMA_ALWAYS_INLINE bool AreNoneSet(const plTypelessBitflags<StorageType>& rhs) const // [tested]
  {
    return (m_Value & rhs.m_Value) == 0;
  }

  /// \brief  Returns whether any of the given flags is set.
  PLASMA_ALWAYS_INLINE bool IsAnySet(const plTypelessBitflags<StorageType>& rhs) const // [tested]
  {
    return (m_Value & rhs.m_Value) != 0;
  }

  /// \brief Sets the given flag.
  PLASMA_ALWAYS_INLINE void Add(const plTypelessBitflags<StorageType>& rhs) // [tested]
  {
    m_Value |= rhs.m_Value;
  }

  /// \brief Removes the given flag.
  PLASMA_ALWAYS_INLINE void Remove(const plTypelessBitflags<StorageType>& rhs) // [tested]
  {
    m_Value &= (~rhs.m_Value);
  }

  /// \brief Toggles the state of the given flag.
  PLASMA_ALWAYS_INLINE void Toggle(const plTypelessBitflags<StorageType>& rhs) // [tested]
  {
    m_Value ^= rhs.m_Value;
  }

  /// \brief Sets or clears the given flag.
  PLASMA_ALWAYS_INLINE void AddOrRemove(const plTypelessBitflags<StorageType>& rhs, bool bState) // [tested]
  {
    m_Value = (bState) ? m_Value | rhs.m_Value : m_Value & (~rhs.m_Value);
  }

  /// \brief Returns an object that has the flags of \a this and \a rhs combined.
  PLASMA_ALWAYS_INLINE plTypelessBitflags<StorageType> operator|(const plTypelessBitflags<StorageType>& rhs) const // [tested]
  {
    return plTypelessBitflags<StorageType>(m_Value | rhs.m_Value);
  }
  ...
  /// \brief Modifies \a this to also contain the bits from \a rhs.
  PLASMA_ALWAYS_INLINE void operator|=(const plTypelessBitflags<StorageType>& rhs) // [tested]
  {
    m_Value |= rhs.m_Value;
  }
