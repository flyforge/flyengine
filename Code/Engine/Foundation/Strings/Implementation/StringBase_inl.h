...
PL_ALWAYS_INLINE const char* plStringBase<Derived>::InternalGetData() const
{
...
template <typename Derived>
PL_ALWAYS_INLINE const char* plStringBase<Derived>::InternalGetDataEnd() const
{
  const Derived* pDerived = static_cast<const Derived*>(this);
...
template <typename Derived>
PL_ALWAYS_INLINE plUInt32 plStringBase<Derived>::InternalGetElementCount() const
{
  const Derived* pDerived = static_cast<const Derived*>(this);
...
template <typename Derived>
PL_ALWAYS_INLINE bool plStringBase<Derived>::IsEmpty() const
{
...
const char* plStringBase<Derived>::FindSubString(plStringView sStringToFind, const char* szStartSearchAt /* = nullptr */) const
{
  ...
  PL_ASSERT_DEV((szStartSearchAt >= InternalGetData()) && (szStartSearchAt <= InternalGetDataEnd()), "The given pointer to start searching at is not inside this strings valid range.");
...
inline const char* plStringBase<Derived>::FindLastSubString(plStringView sStringToFind, const char* szStartSearchAt /* = nullptr */) const
{
  ...
  PL_ASSERT_DEV((szStartSearchAt >= InternalGetData()) && (szStartSearchAt <= InternalGetDataEnd()),
...
template <typename Derived>
inline const char* plStringBase<Derived>::FindWholeWord(const char* szSearchFor, plStringUtils::PL_CHARACTER_FILTER isDelimiterCB, const char* szStartSearchAt /* = nullptr */) const
{
  if (szStartSearchAt == nullptr)
    szStartSearchAt = InternalGetData();
...
template <typename Derived>
inline const char* plStringBase<Derived>::FindWholeWord_NoCase(const char* szSearchFor, plStringUtils::PL_CHARACTER_FILTER isDelimiterCB, const char* szStartSearchAt /* = nullptr */) const
{
  if (szStartSearchAt == nullptr)
    szStartSearchAt = InternalGetData();
...
const char* plStringBase<Derived>::ComputeCharacterPosition(plUInt32 uiCharacterIndex) const
{
  ...
  if (plUnicodeUtils::MoveToNextUtf8(pos, InternalGetDataEnd(), uiCharacterIndex).Failed())
    return nullptr;
...
PL_ALWAYS_INLINE bool operator==(const plStringBase<DerivedLhs>& lhs, const plStringBase<DerivedRhs>& rhs) // [tested]
{
  return lhs.IsEqual(rhs.GetView());
}

template <typename DerivedRhs>
PL_ALWAYS_INLINE bool operator==(const char* lhs, const plStringBase<DerivedRhs>& rhs) // [tested]
{
  return rhs.IsEqual(lhs);
}

template <typename DerivedLhs>
PL_ALWAYS_INLINE bool operator==(const plStringBase<DerivedLhs>& lhs, const char* rhs) // [tested]
{
...
template <typename DerivedLhs>
PL_ALWAYS_INLINE plStringBase<DerivedLhs>::operator plStringView() const
{
  return plStringView(InternalGetData(), InternalGetElementCount());
}

template <typename Derived>
PL_ALWAYS_INLINE plStringView plStringBase<Derived>::GetView() const
{
  return plStringView(InternalGetData(), InternalGetElementCount());
}
