...
struct plStringIterator
{
  ...
  PLASMA_ALWAYS_INLINE plStringIterator() = default; // [tested]

  /// \brief Constructs either a begin or end iterator for the given string.
  PLASMA_FORCE_INLINE explicit plStringIterator(const char* pStartPtr, const char* pEndPtr, const char* pCurPtr)
  {
  ...
  PLASMA_ALWAYS_INLINE bool IsValid() const { return m_pCurPtr != nullptr && m_pCurPtr != m_pEndPtr; } // [tested]

  /// \brief Returns the currently pointed to character in Utf32 encoding.
  PLASMA_ALWAYS_INLINE plUInt32 GetCharacter() const { return IsValid() ? plUnicodeUtils::ConvertUtf8ToUtf32(m_pCurPtr) : plUInt32(0); } // [tested]

  /// \brief Returns the currently pointed to character in Utf32 encoding.
  PLASMA_ALWAYS_INLINE plUInt32 operator*() const { return GetCharacter(); } // [tested]

  /// \brief Returns the address the iterator currently points to.
  PLASMA_ALWAYS_INLINE const char* GetData() const { return m_pCurPtr; } // [tested]

  /// \brief Checks whether the two iterators point to the same element.
  PLASMA_ALWAYS_INLINE bool operator==(const plStringIterator& it2) const { return (m_pCurPtr == it2.m_pCurPtr); } // [tested]
  ...
  PLASMA_ALWAYS_INLINE plUInt32 Advance()
  {
    const char* pPrevElement = m_pCurPtr;

    if (m_pCurPtr < m_pEndPtr)
    {
      plUnicodeUtils::MoveToNextUtf8(m_pCurPtr);
    }
  ...
  PLASMA_ALWAYS_INLINE plStringIterator& operator++() // [tested]
  {
    if (m_pCurPtr < m_pEndPtr)
    {
      plUnicodeUtils::MoveToNextUtf8(m_pCurPtr);
  ...
  PLASMA_ALWAYS_INLINE plStringIterator& operator--() // [tested]
  {
    ...
    {
      plUnicodeUtils::MoveToPriorUtf8(m_pCurPtr);
    }
  ...
  /// \brief Move to the next Utf8 character
  PLASMA_ALWAYS_INLINE plStringIterator operator++(int) // [tested]
  {
  ...
  PLASMA_FORCE_INLINE void operator+=(difference_type d) // [tested]
  {
    while (d > 0)
    {
      ++(*this);
  ...
  PLASMA_ALWAYS_INLINE plStringIterator operator+(difference_type d) const // [tested]
  {
  ...
  {
    PLASMA_ASSERT_DEV((szCurPos >= m_pStartPtr) && (szCurPos <= m_pEndPtr), "New position must still be inside the iterator's range.");
...
struct plStringReverseIterator
{
  ...
  /// \brief Constructs an invalid iterator.
  PLASMA_ALWAYS_INLINE plStringReverseIterator() = default; // [tested]

  /// \brief Constructs either a rbegin or rend iterator for the given string.
  PLASMA_FORCE_INLINE explicit plStringReverseIterator(const char* pStartPtr, const char* pEndPtr, const char* pCurPtr) // [tested]
  {
    m_pStartPtr = pStartPtr;
    m_pEndPtr = pEndPtr;
    m_pCurPtr = pCurPtr;
  ...
  PLASMA_ALWAYS_INLINE bool IsValid() const { return (m_pCurPtr != nullptr); } // [tested]
  ...
  /// \brief Checks whether the two iterators point to the same element.
  PLASMA_ALWAYS_INLINE bool operator==(const plStringReverseIterator& it2) const { return (m_pCurPtr == it2.m_pCurPtr); } // [tested]

  /// \brief Checks whether the two iterators point to the same element.
  PLASMA_ALWAYS_INLINE bool operator!=(const plStringReverseIterator& it2) const { return (m_pCurPtr != it2.m_pCurPtr); } // [tested]

  /// \brief Move to the next Utf8 character
  PLASMA_FORCE_INLINE plStringReverseIterator& operator++() // [tested]
  {
  ...
  /// \brief Move to the next Utf8 character
  PLASMA_ALWAYS_INLINE plStringReverseIterator operator++(int) // [tested]
  {
  ...
  /// \brief Returns an iterator that is advanced forwards by d characters.
  PLASMA_ALWAYS_INLINE plStringReverseIterator operator+(difference_type d) const // [tested]
  {
  ...
  ///
  /// Must be between the iterators start and end range.
  PLASMA_FORCE_INLINE void SetCurrentPosition(const char* szCurPos)
  {
    PLASMA_ASSERT_DEV((szCurPos == nullptr) || ((szCurPos >= m_pStartPtr) && (szCurPos < m_pEndPtr)), "New position must still be inside the iterator's range.");
