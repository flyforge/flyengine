...
namespace
{
  ...
  {
    plUInt8 m_uiLeftOperand;
    plUInt8 m_uiRightOperand;
  };
  ...
  static plExpression::StreamDesc CreateScalarizedStreamDesc(const plExpression::StreamDesc& desc, plEnum<plExpressionAST::VectorComponent> component)
  {
    plStringBuilder sNewName = desc.m_sName.GetView();
    sNewName.Append(".", plExpressionAST::VectorComponent::GetName(component));

    plExpression::StreamDesc newDesc;
    newDesc.m_sName.Assign(sNewName);
    newDesc.m_DataType = static_cast<plProcessingStream::DataType>((plUInt32)desc.m_DataType & ~3u);
...
plExpressionAST::Node* plExpressionAST::TypeDeductionAndConversion(Node* pNode)
{
  ...
  {
    plLog::Error("No matching overload found for '{}'", NodeType::GetName(nodeType));
    return nullptr;
  }
  ...
  for (plUInt32 i = 0; i < children.GetCount(); ++i)
  {
    ...
    if (expectedChildDataType != DataType::Unknown && pChildNode->m_ReturnType != expectedChildDataType)
    {
      ...
      const plUInt32 childElementCount = DataType::GetElementCount(pChildNode->m_ReturnType);
      const auto expectedRegisterType = DataType::GetRegisterType(expectedChildDataType);
      const plUInt32 expectedElementCount = DataType::GetElementCount(expectedChildDataType);
      ...
      {
        pChildNode = CreateConstructorCall(expectedChildDataType, plMakeArrayPtr(&pChildNode, 1));
      }
      else if (childElementCount < expectedElementCount)
      {
        plLog::Error("Cannot implicitly convert '{}' to '{}'", DataType::GetName(pChildNode->m_ReturnType), DataType::GetName(expectedChildDataType));
        return nullptr;
      }
    }
  }
...
plExpressionAST::Node* plExpressionAST::ReplaceVectorInstructions(Node* pNode)
{
  const NodeType::Enum nodeType = pNode->m_Type;
  const DataType::Enum returnType = pNode->m_ReturnType;
  const plUInt32 uiNumInputElements = pNode->m_uiNumInputElements;
  ...
  else if (nodeType == NodeType::Normalize)
  {
    ...
    return CreateBinaryOperator(NodeType::Divide, pUnaryNode->m_pOperand, CreateConstructorCall(returnType, plMakeArrayPtr(&pLength, 1)));
  }
  else if (nodeType == NodeType::All || nodeType == NodeType::Any)
  {
    ...
    for (plUInt32 i = 1; i < uiNumInputElements; ++i)
    {
  ...
  else if (nodeType == NodeType::Cross)
  {
    ...
    {
      plLog::Error("Cross product is only defined for vec3");
      return nullptr;
    }
    ...
    plEnum<VectorComponent> yzx[] = {VectorComponent::Y, VectorComponent::Z, VectorComponent::X};
    plEnum<VectorComponent> zxy[] = {VectorComponent::Z, VectorComponent::X, VectorComponent::Y};
    auto pMul0 = CreateBinaryOperator(NodeType::Multiply, CreateSwizzle(plMakeArrayPtr(yzx), pA), CreateSwizzle(plMakeArrayPtr(zxy), pB));
    auto pMul1 = CreateBinaryOperator(NodeType::Multiply, CreateSwizzle(plMakeArrayPtr(zxy), pA), CreateSwizzle(plMakeArrayPtr(yzx), pB));
  ...
  else if (nodeType == NodeType::Reflect)
  {
    ...
    pMul = CreateBinaryOperator(NodeType::Multiply, pN, CreateConstructorCall(returnType, plMakeArrayPtr(&pMul, 1)));
...
plExpressionAST::Node* plExpressionAST::ScalarizeVectorInstructions(Node* pNode)
{
  const NodeType::Enum nodeType = pNode->m_Type;
  ...
  if (nodeType == NodeType::Swizzle)
  {
    ...
    {
      plEnum<VectorComponent> component = pSwizzleNode->m_Components[0];
      ...
      if (NodeType::IsConstant(childNodeType))
      {
        ...
        const plUInt32 uiNumConstantElements = DataType::GetElementCount(pConstantNode->m_ReturnType);
        if (static_cast<plUInt32>(component) >= uiNumConstantElements)
        {
          plLog::Error("Invalid subscript .{} for constant of type '{}'", VectorComponent::GetName(component), DataType::GetName(pConstantNode->m_ReturnType));
          return nullptr;
        }

        plVariant newValue = pConstantNode->m_Value[component];
      ...
      else if (NodeType::IsSwizzle(childNodeType))
      {
        ...
        if (static_cast<plUInt32>(component) >= pChildSwizzleNode->m_NumComponents)
        {
          plLog::Error("Invalid Swizzle");
          return nullptr;
        }
      ...
      else if (NodeType::IsInput(childNodeType))
      {
        ...
        const plUInt32 uiNumInputElements = DataType::GetElementCount(pInput->m_ReturnType);
        if (static_cast<plUInt32>(component) >= uiNumInputElements)
        {
          plLog::Error("Invalid subscript .{} for input '{}' of type '{}'", VectorComponent::GetName(component), pInput->m_Desc.m_sName, DataType::GetName(pInput->m_ReturnType));
          return nullptr;
        }
      ...
      plSmallArray<Node*, 8> newSwizzleNodes;
    ...
    {
      plLog::Error("Failed to scalarize AST");
      return nullptr;
    }
  }
...
plExpressionAST::Node* plExpressionAST::ReplaceUnsupportedInstructions(Node* pNode)
{
  const NodeType::Enum nodeType = pNode->m_Type;
  const DataType::Enum returnType = pNode->m_ReturnType;
  ...
  else if (nodeType == NodeType::RadToDeg)
  {
    auto pUnaryNode = static_cast<const UnaryOperator*>(pNode);
    auto pValue = pUnaryNode->m_pOperand;
    auto pMultiplier = CreateConstant(plAngle::RadToDegMultiplier<float>(), returnType);
  ...
  else if (nodeType == NodeType::DegToRad)
  {
    auto pUnaryNode = static_cast<const UnaryOperator*>(pNode);
    auto pValue = pUnaryNode->m_pOperand;
    auto pMultiplier = CreateConstant(plAngle::DegToRadMultiplier<float>(), returnType);
    return CreateBinaryOperator(NodeType::Multiply, pValue, pMultiplier);
  }
  ...
  else if (nodeType == NodeType::Log)
  {
    ...
    if (NodeType::IsConstant(pBase->m_Type))
    {
      ...
      {
        auto pFactor = CreateConstant(1.0f / plMath::Log2(fBaseValue));
  ...
  else if (nodeType == NodeType::Pow)
  {
    ...
    if (NodeType::IsConstant(pExp->m_Type))
    {
      ...
      if (isWholeNumber && fExpValue > 1 && fExpValue < PLASMA_ARRAY_SIZE(s_MultiplicationChains))
      {
        ...
        const auto& chain = s_MultiplicationChains[(plUInt32)fExpValue].m_Chain;
        plUInt32 uiIndex = 0;
        do
        {
  ...
  else if (nodeType == NodeType::ConstructorCall)
  {
    ...
    {
      plLog::Error("Constructor of type '{}' has too many arguments", DataType::GetName(returnType));
      return nullptr;
    }
...
plExpressionAST::Node* plExpressionAST::FoldConstants(Node* pNode)
{
  const NodeType::Enum nodeType = pNode->m_Type;
  const DataType::Enum returnType = pNode->m_ReturnType;
    ...
    if (NodeType::IsConstant(pUnaryNode->m_pOperand->m_Type))
    {
      ...
      else if (returnType == DataType::Int)
      {
        ...
        switch (nodeType)
        {
          ...
          case NodeType::Absolute:
            return CreateConstant(plMath::Abs(iValue), returnType);
          case NodeType::Saturate:
            return CreateConstant(plMath::Saturate(iValue), returnType);
          case NodeType::Log2:
            return CreateConstant(plMath::Log2i(iValue), returnType);
          case NodeType::Pow2:
            return CreateConstant(plMath::Pow2(iValue), returnType);
      ...
      else if (returnType == DataType::Float)
      {
        ...
        switch (nodeType)
        {
          ...
          case NodeType::Absolute:
            return CreateConstant(plMath::Abs(fValue));
          case NodeType::Saturate:
            return CreateConstant(plMath::Saturate(fValue));
          case NodeType::Sqrt:
            return CreateConstant(plMath::Sqrt(fValue));
          case NodeType::Exp:
            return CreateConstant(plMath::Exp(fValue));
          case NodeType::Ln:
            return CreateConstant(plMath::Ln(fValue));
          case NodeType::Log2:
            return CreateConstant(plMath::Log2(fValue));
          case NodeType::Log10:
            return CreateConstant(plMath::Log10(fValue));
          case NodeType::Pow2:
          ...
          case NodeType::ACos:
            return CreateConstant(plMath::ACos(fValue).GetRadian());
          case NodeType::ATan:
            return CreateConstant(plMath::ATan(fValue).GetRadian());
          case NodeType::RadToDeg:
            return CreateConstant(plAngle::RadToDeg(fValue));
          case NodeType::DegToRad:
            return CreateConstant(plAngle::DegToRad(fValue));
          case NodeType::Round:
            return CreateConstant(plMath::Round(fValue));
          case NodeType::Floor:
            return CreateConstant(plMath::Floor(fValue));
          case NodeType::Ceil:
            return CreateConstant(plMath::Ceil(fValue));
          case NodeType::Trunc:
            return CreateConstant(plMath::Trunc(fValue));
          case NodeType::Frac:
  ...
  else if (NodeType::IsBinary(nodeType))
  {
      ...
      else if (leftType == DataType::Int)
      {
        ...
        switch (nodeType)
        {
          ...
          case NodeType::Modulo:
            return CreateConstant(iLeftValue % iRightValue, returnType);
          case NodeType::Pow:
            return CreateConstant(plMath::Pow(iLeftValue, iRightValue), returnType);
          case NodeType::Min:
            return CreateConstant(plMath::Min(iLeftValue, iRightValue), returnType);
          case NodeType::Max:
            return CreateConstant(plMath::Max(iLeftValue, iRightValue), returnType);
      ...
      else if (leftType == DataType::Float)
      {
        ...
        switch (nodeType)
        {
          ...
          case NodeType::Divide:
            return CreateConstant(fLeftValue / fRightValue, returnType);
          case NodeType::Modulo:
            return CreateConstant(plMath::Mod(fLeftValue, fRightValue), returnType);
          case NodeType::Log:
            return CreateConstant(plMath::Log(fLeftValue, fRightValue), returnType);
          case NodeType::Pow:
            return CreateConstant(plMath::Pow(fLeftValue, fRightValue), returnType);
          case NodeType::Min:
            return CreateConstant(plMath::Min(fLeftValue, fRightValue), returnType);
          case NodeType::Max:
            return CreateConstant(plMath::Max(fLeftValue, fRightValue), returnType);
          case NodeType::Equal:
            return CreateConstant(fLeftValue == fRightValue, returnType);
          case NodeType::NotEqual:
            return CreateConstant(fLeftValue != fRightValue, returnType);
    ...
    else if (bRightIsConstant)
    {
      ...
      else if (leftType == DataType::Int)
      {
        ...
        else if (nodeType == NodeType::Divide && plMath::IsPowerOf2(iRightValue))
        {
          auto pShiftValue = CreateConstant(plMath::Log2i(iRightValue), returnType);
...
plExpressionAST::Node* plExpressionAST::CommonSubexpressionElimination(Node* pNode)
{
  UpdateHash(pNode);
...
plExpressionAST::Node* plExpressionAST::Validate(Node* pNode)
{
  const NodeType::Enum nodeType = pNode->m_Type;

  if (pNode->m_ReturnType == DataType::Unknown)
  {
    plLog::Error("Unresolved return type on '{}'", NodeType::GetName(nodeType));
    return nullptr;
  }
  ...
  if (NodeType::IsUnary(nodeType) || NodeType::IsBinary(nodeType) || NodeType::IsTernary(nodeType))
  {
    ...
    {
      plLog::Error("Unresolved overload on '{}'", NodeType::GetName(nodeType));
      return nullptr;
    }
  }
  else if (NodeType::IsConstant(nodeType))
  {
    ...
    {
      plLog::Error("Invalid constant value");
      return nullptr;
    }
  }
  else if (NodeType::IsFunctionCall(nodeType))
  {
    if (pNode->m_uiOverloadIndex == 0xFF)
    {
      plLog::Error("Unresolved function overload on");
      return nullptr;
    }
    ...
    {
      plLog::Error("Not enough arguments for function '{}'", pDesc->m_sName);
      return nullptr;
    }
  }
  ...
  for (plUInt32 i = 0; i < children.GetCount(); ++i)
  {
    ...
    if (expectedChildDataType != DataType::Unknown && pChildNode->m_ReturnType != expectedChildDataType)
    {
      plLog::Error("Invalid data type for argument {} on '{}'. Expected {} got {}", i, NodeType::GetName(nodeType), DataType::GetName(expectedChildDataType), DataType::GetName(pChildNode->m_ReturnType));
      return nullptr;
    }
  }
