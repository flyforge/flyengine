...
{
  return __builtin_shufflevector(m_v, other.m_v, PLASMA_TO_SHUFFLE(s));
}
...
{
  float32x4_t a = vmulq_f32(m_v, __builtin_shufflevector(v.m_v, v.m_v, PLASMA_TO_SHUFFLE(plSwizzle::YZXW)));
  float32x4_t b = vmulq_f32(v.m_v, __builtin_shufflevector(m_v, m_v, PLASMA_TO_SHUFFLE(plSwizzle::YZXW)));
  float32x4_t c = vsubq_f32(a, b);

  return __builtin_shufflevector(c, c, PLASMA_TO_SHUFFLE(plSwizzle::YZXW));
}
