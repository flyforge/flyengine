...
{
  PL_FOUNDATION_DLL void SetConsoleCtrlHandler(plMinWindows::BOOL(PL_WINDOWS_WINAPI* consoleHandler)(plMinWindows::DWORD dwCtrlType));
  PL_FOUNDATION_DLL plMutex& GetShutdownMutex();

  template <typename AppClass, typename... Args>
  int ConsoleEntry(int iArgc, const char** pArgv, Args&&... arguments)
  {
#if PL_ENABLED(PL_COMPILER_MSVC)             // Internal compiler error in MSVC. Can not align buffer otherwise the compiler will crash.
...
#define PL_CONSOLEAPP_ENTRY_POINT(AppClass, ...)                                                \
...
//
// In PL_APPLICATION_ENTRY_POINT we use macro magic to concatenate strings in such a way that depending on whether windows.h has
...
#define _PL_APPLICATION_ENTRY_POINT_HINSTANCE HINSTANCE
#define _PL_APPLICATION_ENTRY_POINT_LPSTR LPSTR
#define _PL_APPLICATION_ENTRY_POINT_HINSTANCE_WINDOWS_ plMinWindows::HINSTANCE
#define _PL_APPLICATION_ENTRY_POINT_LPSTR_WINDOWS_ plMinWindows::LPSTR
...
#define PL_APPLICATION_ENTRY_POINT(AppClass, ...)                                                                          \
  ...
  PL_APPLICATION_ENTRY_POINT_CODE_INJECTION                                                                                \
  int PL_WINDOWS_CALLBACK WinMain(_In_ PL_CONCAT(_PL_, PL_CONCAT(APPLICATION_ENTRY_POINT_HINSTANCE, _WINDOWS_)) hInstance, \
    _In_opt_ PL_CONCAT(_PL_, PL_CONCAT(APPLICATION_ENTRY_POINT_HINSTANCE, _WINDOWS_)) hPrevInstance,                       \
    _In_ PL_CONCAT(_PL_, PL_CONCAT(APPLICATION_ENTRY_POINT_LPSTR, _WINDOWS_)) lpCmdLine, _In_ int nCmdShow)                \
  {                                                                                                                        \
