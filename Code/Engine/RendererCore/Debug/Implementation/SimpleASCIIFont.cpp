...
static void CopyCharacter(plUInt32* pImage, plInt32 c, const char* szChar)
{
  plInt32 x = ((plInt32)c % 16) * 16;
  plInt32 y = ((plInt32)c / 16) * 16;

  pImage += (y + 3) * 256; // 3 pixel boundary at the top
  pImage += x + 3;         // 3 pixel boundary at the left side

  for (plUInt32 i1 = 0; i1 < 10; ++i1)
  {
    for (plUInt32 i2 = 0; i2 < 10; ++i2)
    {
...
void plGraphicsUtils::CreateSimpleASCIIFontTexture(plImage& ref_img, bool bSetEmptyToUnknown)
{
  ...
  // pre-fill with black
  {
    plUInt32* pPixelData = Img.GetPixelPointer<plUInt32>();
  ...
  class VariableReader : public plStreamReader
  {
  public:
    virtual plUInt64 ReadBytes(void* pReadBuffer, plUInt64 uiBytesToRead) override
    {
      const plUInt64 uiSize = plMath::Min(uiBytesToRead, m_uiSize);
      m_uiSize -= uiSize;

      if (uiSize > 0)
      {
        plMemoryUtils::Copy((plUInt8*)pReadBuffer, m_pData, (size_t)uiSize);
        m_pData += uiSize;
      }
  ...
  plImageConversion::Convert(ref_img, ref_img, plImageFormat::R8G8B8A8_UNORM).IgnoreResult();

  for (plUInt32 y = 0; y < ref_img.GetHeight(); ++y)
  {
    for (plUInt32 x = 0; x < ref_img.GetWidth(); ++x)
    {
      plUInt8* pPixel = ref_img.GetPixelPointer<plUInt8>(0, 0, 0, x, y, 0);
