...
void Occluder::bake(const __m128* vertices, plUInt32 numVertices, __m128 refMin, __m128 refMax)
{
  ...
  plHybridArray<__m128, 32, plAlignedAllocatorWrapper> quadNormals;
  for (plUInt32 i = 0; i < numVertices; i += 4)
  {
  ...
  plHybridArray<__m128, 32, plAlignedAllocatorWrapper> centroids;
  plHybridArray<uint32_t, 32> centroidAssignment;
  ...
  plHybridArray<__m128, 64, plAlignedAllocatorWrapper> orderedVertices;
  for (uint32_t k = 0; k < centroids.GetCount(); ++k)
  {
    for (plUInt32 j = 0; j < numVertices / 4; ++j)
    {
  ...
  for (plUInt32 i = 0; i < orderedVertices.GetCount(); ++i)
  {
