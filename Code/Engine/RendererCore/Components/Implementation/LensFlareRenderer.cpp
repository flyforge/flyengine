...
PL_CHECK_AT_COMPILETIME(sizeof(plPerLensFlareData) == 48);

PL_BEGIN_DYNAMIC_REFLECTED_TYPE(plLensFlareRenderer, 1, plRTTIDefaultAllocator<plLensFlareRenderer>)
...
void plLensFlareRenderer::RenderBatch(const plRenderViewContext& renderViewContext, const plRenderPipelinePass* pPass, const plRenderDataBatch& batch) const
{
  ...
  PL_SCOPE_EXIT(DeleteLensFlareDataBuffer(hLensFlareData));
...
PL_STATICLINK_FILE(RendererCore, RendererCore_Components_Implementation_LensFlareRenderer);
