...
#include <RendererCore/Pipeline/Passes/SSAOPass.h>
...
// clang-format off
PLASMA_BEGIN_DYNAMIC_REFLECTED_TYPE(plSSAOPass, 1, plRTTIDefaultAllocator<plSSAOPass>)
{
...
plSSAOPass::plSSAOPass()
  : plRenderPipelinePass("SSAOPass", true)
  , m_fRadius(1.0f)
...
bool plSSAOPass::GetRenderTargetDescriptions(const plView& view, const plArrayPtr<plGALTextureCreationDescription* const> inputs, plArrayPtr<plGALTextureCreationDescription> outputs)
{
...
void plSSAOPass::Execute(const plRenderViewContext& renderViewContext, const plArrayPtr<plRenderPipelinePassConnection* const> inputs, const plArrayPtr<plRenderPipelinePassConnection* const> outputs)
{
...
void plSSAOPass::ExecuteInactive(const plRenderViewContext& renderViewContext, const plArrayPtr<plRenderPipelinePassConnection* const> inputs, const plArrayPtr<plRenderPipelinePassConnection* const> outputs)
{
  auto pOutput = outputs[m_PinOutput.m_uiOutputIndex];
  if (pOutput == nullptr)
  {
...
void plSSAOPass::SetFadeOutStart(float fStart)
{
  m_fFadeOutStart = plMath::Clamp(fStart, 0.0f, m_fFadeOutEnd);
}

float plSSAOPass::GetFadeOutStart() const
{
  return m_fFadeOutStart;
}

void plSSAOPass::SetFadeOutEnd(float fEnd)
{
...
float plSSAOPass::GetFadeOutEnd() const
{
  return m_fFadeOutEnd;
}

void plSSAOPass::CreateSamplerState()
{
  if (m_hSSAOSamplerState.IsInvalidated())
  {
