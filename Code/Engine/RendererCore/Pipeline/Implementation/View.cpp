...
// clang-format off
PLASMA_BEGIN_STATIC_REFLECTED_ENUM(plCameraUsageHint, 1)
  PLASMA_ENUM_CONSTANT(plCameraUsageHint::None),
  PLASMA_ENUM_CONSTANT(plCameraUsageHint::MainView),
  PLASMA_ENUM_CONSTANT(plCameraUsageHint::EditorView),
  PLASMA_ENUM_CONSTANT(plCameraUsageHint::RenderTarget),
  PLASMA_ENUM_CONSTANT(plCameraUsageHint::Culling),
  PLASMA_ENUM_CONSTANT(plCameraUsageHint::Thumbnail),
PLASMA_END_STATIC_REFLECTED_ENUM;

PLASMA_BEGIN_DYNAMIC_REFLECTED_TYPE(plView, 1, plRTTINoAllocator)
{
  PLASMA_BEGIN_PROPERTIES
  {
    PLASMA_MEMBER_PROPERTY("RenderTarget0", m_PinRenderTarget0),
    PLASMA_MEMBER_PROPERTY("RenderTarget1", m_PinRenderTarget1),
    PLASMA_MEMBER_PROPERTY("RenderTarget2", m_PinRenderTarget2),
    PLASMA_MEMBER_PROPERTY("RenderTarget3", m_PinRenderTarget3),
...
{
  PLASMA_ASSERT_DEV(IsValid(), "Cannot extract data from an invalid view");
...
void plView::ApplyRenderPassProperties()
{
  for (auto it = m_PassProperties.GetIterator(); it.IsValid(); ++it)
  {
    ...
    if (szDot != nullptr)
    {
      PLASMA_REPORT_FAILURE("Setting renderer properties is not possible anymore");
    }
    else
    {
