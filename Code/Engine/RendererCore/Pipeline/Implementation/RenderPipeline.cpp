...
bool plRenderPipeline::Connect(plRenderPipelinePass* pOutputNode, plHashedString sOutputPinName, plRenderPipelinePass* pInputNode, plHashedString sInputPinName)
{
  ...
  {
    pConnection = PL_DEFAULT_NEW(plRenderPipelinePassConnection);
...
bool plRenderPipeline::SortPasses()
{
  ...
  while (!usable.IsEmpty())
  {
    ...
    PL_ASSERT_DEBUG(data.m_Inputs.GetCount() == pPass->GetInputPins().GetCount(), "Input pin count missmatch!");
    PL_ASSERT_DEBUG(data.m_Outputs.GetCount() == pPass->GetOutputPins().GetCount(), "Output pin count missmatch!");
    ...
    for (plUInt32 i = 0; i < data.m_Outputs.GetCount(); i++)
    {
      if (data.m_Outputs[i] != nullptr)
      {
        ...
        {
          PL_ASSERT_DEBUG(pPin->m_pParent != nullptr, "Pass was not initialized!");
  ...
  struct plPipelineSorter
  {
    ...
    PL_FORCE_INLINE bool Less(const plUniquePtr<plRenderPipelinePass>& a, const plUniquePtr<plRenderPipelinePass>& b) const { return m_pDone->IndexOf(a.Borrow()) < m_pDone->IndexOf(b.Borrow()); }

    /// \brief Returns true if a is equal to b
    PL_ALWAYS_INLINE bool Equal(const plUniquePtr<plRenderPipelinePass>& a, const plUniquePtr<plRenderPipelinePass>& b) const { return a.Borrow() == b.Borrow(); }
...
bool plRenderPipeline::CreateRenderTargetUsage(const plView& view)
{
  ...
  PL_ASSERT_DEBUG(m_TextureUsage.IsEmpty(), "Need to call ClearRenderPassGraphTextures before re-creating the pipeline.");
    ...
    for (plRenderPipelinePassConnection* pConn : data.m_Outputs)
    {
      if (pConn != nullptr)
      {
        if (pConn->m_pOutput->m_Type == plRenderPipelineNodePin::Type::PassThrough && data.m_Inputs[pConn->m_pOutput->m_uiInputIndex] != nullptr)
        {
          ...
          PL_ASSERT_DEV(m_ConnectionToTextureIndex.Contains(pCorrespondingInputConn), "");
          plUInt32 uiDataIdx = m_ConnectionToTextureIndex[pCorrespondingInputConn];
          m_TextureUsage[uiDataIdx].m_UsedBy.PushBack(pConn);
          m_TextureUsage[uiDataIdx].m_uiLastUsageIdx = i;

          PL_ASSERT_DEV(!m_ConnectionToTextureIndex.Contains(pConn), "");
      ...
      for (plUInt32 j = 0; j < data.m_Inputs.GetCount(); j++)
      {
        ...
        if (pConn != nullptr)
        {
          ...
          else if (!hTexture->IsInvalidated() || pConn->m_Desc.CalculateHash() == defaultTextureDescHash)
          {
            ...
            PL_ASSERT_DEV(reinterpret_cast<const plGALTextureHandle*>(&renderTargets)[m_TextureUsage[uiDataIdx].m_iTargetTextureIndex] == *hTexture, "Offset computation broken.");
  ...
  struct FirstUsageComparer
  {
    ...
    PL_ALWAYS_INLINE bool Less(plUInt16 a, plUInt16 b) const { return m_TextureUsage[a].m_uiFirstUsageIdx < m_TextureUsage[b].m_uiFirstUsageIdx; }
  ...
  struct LastUsageComparer
  {
    ...
    PL_ALWAYS_INLINE bool Less(plUInt16 a, plUInt16 b) const { return m_TextureUsage[a].m_uiLastUsageIdx < m_TextureUsage[b].m_uiLastUsageIdx; }
...
void plRenderPipeline::UpdateViewData(const plView& view, plUInt32 uiDataIndex)
{
  ...
  PL_ASSERT_DEV(uiDataIndex <= 1, "Data index must be 0 or 1");
...
void plRenderPipeline::RemoveConnections(plRenderPipelinePass* pPass)
{
  ...
  for (plUInt32 i = 0; i < data.m_Inputs.GetCount(); i++)
  {
    ...
    if (pConn != nullptr)
    {
      ...
      PL_IGNORE_UNUSED(bRes);
      PL_ASSERT_DEBUG(bRes, "plRenderPipeline::RemoveConnections should not fail to disconnect pins!");
    }
  }
  for (plUInt32 i = 0; i < data.m_Outputs.GetCount(); i++)
  {
...
{
  PL_ASSERT_DEV(m_CurrentExtractThread == (plThreadID)0, "Extract must not be called from multiple threads.");
  ...
  {
    PL_REPORT_FAILURE("View '{0}' is extracted multiple times", view.GetName());
    return;
  }
  ...
  for (auto& pExtractor : m_Extractors)
  {
    ...
    {
      PL_PROFILE_SCOPE(pExtractor->m_sName.GetData());
...
// clang-format off
PL_BEGIN_SUBSYSTEM_DECLARATION(RendererCore, SwRasterizer)
  ...
  {
    g_pRasterizerViewPool = PL_DEFAULT_NEW(plRasterizerViewPool);
  }
...
{
  PL_PROFILE_SCOPE("Visibility Culling");
  ...
  PL_SCOPE_EXIT(g_pRasterizerViewPool->ReturnRasterizerView(pRasterizer));
  ...
  {
    PL_PROFILE_SCOPE("Occlusion::FindVisibleObjects");
...
{
  // PL_PROFILE_AND_MARKER(pRenderContext->GetGALContext(), m_sName.GetData());
  PL_PROFILE_SCOPE(m_sName.GetData());

  PL_ASSERT_DEV(m_PipelineState != PipelineState::Uninitialized, "Pipeline must be rebuild before rendering.");
  if (m_PipelineState == PipelineState::RebuildError)
  {
    return;
  }

  PL_ASSERT_DEV(m_CurrentRenderThread == (plThreadID)0, "Render must not be called from multiple threads.");
  m_CurrentRenderThread = plThreadUtils::GetCurrentThreadID();

  PL_ASSERT_DEV(m_uiLastRenderFrame != plRenderWorld::GetFrameCounter(), "Render must not be called multiple times per frame.");
  ...
  {
    PL_PROFILE_SCOPE("BeforePipelineExecution");
  ...
  for (plUInt32 i = 0; i < m_Passes.GetCount(); ++i)
  {
    ...
    PL_PROFILE_SCOPE(pPass->GetName());
    ...
    for (; uiCurrentFirstUsageIdx < m_TextureUsageIdxSortedByFirstUsage.GetCount();)
    {
      ...
      if (usageData.m_uiFirstUsageIdx == i)
      {
        ...
        PL_ASSERT_DEV(!hTexture.IsInvalidated(), "GPU pool returned an invalidated texture!");
  ...
  PL_ASSERT_DEV(uiCurrentFirstUsageIdx == m_TextureUsageIdxSortedByFirstUsage.GetCount(), "Rendering all passes should have moved us through all texture usage blocks!");
  PL_ASSERT_DEV(uiCurrentLastUsageIdx == m_TextureUsageIdxSortedByLastUsage.GetCount(), "Rendering all passes should have moved us through all texture usage blocks!");
  ...
  {
    PL_PROFILE_SCOPE("AfterPipelineExecution");
    plRenderWorld::s_RenderEvent.Broadcast(renderEvent);
  }
...
void plRenderPipeline::CreateDgmlGraph(plDGMLGraph& ref_graph)
{
  ...
  for (plUInt32 p = 0; p < m_Passes.GetCount(); ++p)
  {
    ...
    sTmp.SetFormat("#{}: {}", p, plStringUtils::IsNullOrEmpty(pPass->GetName()) ? pPass->GetDynamicRTTI()->GetTypeName() : pPass->GetName());
  ...
  for (plUInt32 i = 0; i < m_TextureUsage.GetCount(); ++i)
  {
    ...
    for (const plRenderPipelinePassConnection* pCon : data.m_UsedBy)
    {
      ...
      sTmp.SetFormat("{} #{}: {}x{}:{}, MSAA:{}, {}Format: {}", data.m_iTargetTextureIndex != -1 ? "RenderTarget" : "PoolTexture", i, pCon->m_Desc.m_uiWidth, pCon->m_Desc.m_uiHeight, pCon->m_Desc.m_uiArraySize, (int)pCon->m_Desc.m_SampleCount, plGALResourceFormat::IsDepthFormat(pCon->m_Desc.m_Format) ? "Depth" : "Color", sFormat);
...
plRasterizerView* plRenderPipeline::PrepareOcclusionCulling(const plFrustum& frustum, const plView& view)
{
  ...
  PL_PROFILE_SCOPE("Occlusion::RasterizeView");
  ...
  {
    PL_PROFILE_SCOPE("Occlusion::FindOccluders");
...
void plRenderPipeline::PreviewOcclusionBuffer(const plRasterizerView& rasterizer, const plView& view)
{
  ...
  PL_PROFILE_SCOPE("Occlusion::DebugPreview");
...
PL_STATICLINK_FILE(RendererCore, RendererCore_Pipeline_Implementation_RenderPipeline);
