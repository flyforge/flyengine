...
// clang-format off
PLASMA_BEGIN_STATIC_REFLECTED_TYPE(plRenderPipelineResourceLoaderConnection, plNoBase, 1, plRTTIDefaultAllocator<plRenderPipelineResourceLoaderConnection>)
{
}
...
{
  PLASMA_VERIFY(plTypeVersionReadContext::GetContext()->GetTypeVersion(plGetStaticRTTI<plRenderPipelineResourceLoaderConnection>()) == 1, "Unknown version");
...
void plRenderPipelineResourceLoader::CreateRenderPipelineResourceDescriptor(const plRenderPipeline* pPipeline, plRenderPipelineResourceDescriptor& ref_desc)
{
  ...
  for (plUInt32 i = 0; i < passes.GetCount(); i++)
  {
    ...
    for (const plRenderPipelineNodePin* pPinSource : outputs)
    {
      ...
      {
        PLASMA_VERIFY(passToIndex.TryGetValue(pPinTarget->m_pParent, data.m_uiTarget), "Failed to resolve render pass to index");
...
plResult plRenderPipelineResourceLoader::ExportPipeline(plArrayPtr<const plRenderPipelinePass* const> passes, plArrayPtr<const plExtractor* const> extractors, plArrayPtr<const plRenderPipelineResourceLoaderConnection> connections, plStreamWriter& ref_streamWriter)
{
  ...
  // passes
  {
    ...
    for (auto& pass : passes)
    {
      ...
      PLASMA_SUCCEED_OR_RETURN(pass->Serialize(stream));
    }
  }
  ...
  // extractors
  {
    ...
    for (auto& extractor : extractors)
    {
      ...
      PLASMA_SUCCEED_OR_RETURN(extractor->Serialize(stream));
    }
  }
  ...
  // Connections
  {
    ...
    {
      PLASMA_SUCCEED_OR_RETURN(connection.Serialize(stream));
    }
  }
