...
plResult plGALSharedTextureDX11::InitPlatform(plGALDevice* pDevice, plArrayPtr<plGALSystemMemoryDescription> pInitialData)
{
  ...
  PLASMA_ASSERT_DEBUG(m_SharedType != plGALSharedTextureType::None, "Shared texture must either be exported or imported");
  PLASMA_ASSERT_DEBUG(m_Description.m_Type == plGALTextureType::Texture2DShared, "Shared texture must be of type plGALTextureType::Texture2DShared");
  ...
  if (m_SharedType == plGALSharedTextureType::Imported)
  {
    ...
    PLASMA_SCOPE_EXIT(d3d11ResPtr->Release());
  ...
  else if (m_SharedType == plGALSharedTextureType::Exported)
  {
    ...
    PLASMA_SCOPE_EXIT(pDXGIResource->Release());
    HANDLE hTexture = 0;
...
{
  PLASMA_GAL_DX11_RELEASE(m_pKeyedMutex);
...
PLASMA_STATICLINK_FILE(RendererDX11, RendererDX11_Resources_Implementation_SharedTextureDX11);
