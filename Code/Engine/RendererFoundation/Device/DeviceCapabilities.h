...
struct plGALResourceFormatSupport
{
  ...
  {
    None = 0,
    Sample = PL_BIT(0),          ///< The format can be sampled as a texture in a shader or in case of integer textures load can be called.
    Render = PL_BIT(1),          ///< The format can be used as a render target texture.
    VertexAttribute = PL_BIT(2), ///< The format can be used as a vertex attribute.
    MSAA2x = PL_BIT(3),          ///< The format supports 2x MSAA
    MSAA4x = PL_BIT(4),          ///< The format supports 4x MSAA
    MSAA8x = PL_BIT(5),          ///< The format supports 8x MSAA
    Default = 0
  };
...
PL_DECLARE_FLAGS_OPERATORS(plGALResourceFormatSupport);
...
struct PL_RENDERERFOUNDATION_DLL plGALDeviceCapabilities
{
  // Device description
