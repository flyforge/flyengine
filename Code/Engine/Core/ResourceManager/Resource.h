...
class PL_CORE_DLL plResource : public plReflectedClass
{
  PL_ADD_DYNAMIC_REFLECTION(plResource, plReflectedClass);
...
public:
  ...
  PL_ALWAYS_INLINE const plString& GetResourceID() const { return m_sUniqueID; }

  /// \brief Returns the hash of the unique ID.
  PL_ALWAYS_INLINE plUInt64 GetResourceIDHash() const { return m_uiUniqueIDHash; }
  ...
  PL_ALWAYS_INLINE plResourceState GetLoadingState() const { return m_LoadingState; }
  ...
  PL_ALWAYS_INLINE plUInt8 GetNumQualityLevelsDiscardable() const { return m_uiQualityLevelsDiscardable; }

  /// \brief Returns how many quality levels the resource may additionally load.
  PL_ALWAYS_INLINE plUInt8 GetNumQualityLevelsLoadable() const { return m_uiQualityLevelsLoadable; }
  ...
  PL_ALWAYS_INLINE const plBitflags<plResourceFlags>& GetBaseResourceFlags() const { return m_Flags; }

  /// \brief Returns the information about the current memory usage of the resource.
  PL_ALWAYS_INLINE const MemoryUsage& GetMemoryUsage() const { return m_MemoryUsage; }
  ...
  PL_ALWAYS_INLINE plTime GetLastAcquireTime() const { return m_LastAcquire; }

  /// \brief Returns the reference count of this resource.
  PL_ALWAYS_INLINE plInt32 GetReferenceCount() const { return m_iReferenceCount; }
  ...
  PL_ALWAYS_INLINE const plTimestamp& GetLoadedFileModificationTime() const { return m_LoadedFileModificationTime; }
  ...
  PL_ALWAYS_INLINE plUInt32 GetCurrentResourceChangeCounter() const { return m_uiResourceChangeCounter; }

  /// \brief Allows to manually increase the resource change counter to signal that dependent code might need to update.
  PL_ALWAYS_INLINE void IncResourceChangeCounter() { ++m_uiResourceChangeCounter; }
  ...
  ///
  /// Only implemented if PL_RESOURCEHANDLE_STACK_TRACES is PL_ON.
...
private:
  ...
  friend PL_CORE_DLL_FRIEND void IncreaseResourceRefCount(plResource* pResource, const void* pOwner);
  friend PL_CORE_DLL_FRIEND void DecreaseResourceRefCount(plResource* pResource, const void* pOwner);

#if PL_ENABLED(PL_RESOURCEHANDLE_STACK_TRACES)
  friend PL_CORE_DLL_FRIEND void MigrateResourceRefCount(plResource* pResource, const void* pOldOwner, const void* pNewOwner);
...
#define PL_RESOURCE_DECLARE_COMMON_CODE(SELF)                                                                                                \
  friend class ::plResourceManager;                                                                                                          \
                                                                                                                                             \
public:                                                                                                                                      \
  /*                                                                                                                                     \ \ \
...
private:                                                                                                                                     \
  ...
  static const plTypedResourceHandle<SELF>& GetResourceTypeLoadingFallback()                                                                 \
  {                                                                                                                                          \
    return s_TypeLoadingFallback;                                                                                                            \
  }                                                                                                                                          \
  static const plTypedResourceHandle<SELF>& GetResourceTypeMissingFallback()                                                                 \
  {                                                                                                                                          \
...
#define PL_RESOURCE_IMPLEMENT_COMMON_CODE(SELF)                                                                         \
  ...
  {                                                                                                                     \
    PL_ASSERT_DEV(GetReferenceCount() > 0, "This resource is being deallocated, do not store a handle to it anymore!"); \
  ...
  {                                                                                                                     \
    ...
    PL_RESOURCE_VALIDATE_FALLBACK(SELF);                                                                                \
...
#if PL_ENABLED(PL_COMPILE_FOR_DEVELOPMENT)
#  define PL_RESOURCE_VALIDATE_FALLBACK(SELF)                                       \
...
#define PL_RESOURCE_DECLARE_CREATEABLE(SELF, SELF_DESCRIPTOR)      \
protected:                                                         \
  plResourceLoadDesc CreateResource(SELF_DESCRIPTOR&& descriptor); \
                                                                   \
private:

#define PL_RESOURCE_IMPLEMENT_CREATEABLE(SELF, SELF_DESCRIPTOR) plResourceLoadDesc SELF::CreateResource(SELF_DESCRIPTOR&& descriptor)
