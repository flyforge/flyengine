...
///
/// All input handling is usually handled through the plInputManager class.
...
///
/// By deriving from plInputDevice you can extend what hardware the engine supports. The derived class should override
...
private:
  ...
  /// on Windows the WM_CHAR message sends this information. An plInputDevice derived class should never try to compute this
  /// itself, but instead query this information from the OS, which will also handle localization.
  plUInt32 RetrieveLastCharacter();

  /// \brief Calls UpdateHardwareState() on all devices.
  static void UpdateAllHardwareStates(plTime tTimeDifference);
  ...
  static plUInt32 RetrieveLastCharacterFromAllDevices();

protected:
  /// \brief Calls RegisterInputSlot() on the plInputManager and passes the parameters through.
  static void RegisterInputSlot(plStringView sName, plStringView sDefaultDisplayName, plBitflags<plInputSlotFlags> SlotFlags); // [tested]
  ...
  plMap<plString, float> m_InputSlotValues; // [tested]

  /// \brief If this input device type handles character input, it should write the last typed character into this variable.
  /// The plInputManager calls RetrieveLastCharacter() to query what the user typed last.
  plUInt32 m_uiLastCharacter; // [tested]
  ...
  /// \brief Override this, if you need to reset certain input slot values to zero, after the plInputManager is finished with the current
  /// frame update.
  ...
  /// \brief This function is called once after plInputManager::Update with the same time delta value.
