...
bool plLuaWrapper::IsVariableAvailable(const char* szName) const
{
...
bool plLuaWrapper::IsFunctionAvailable(const char* szFunction) const
{
  if (m_States.m_iOpenTables == 0)
...
plInt32 plLuaWrapper::GetIntVariable(const char* szName, plInt32 iDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
bool plLuaWrapper::GetBoolVariable(const char* szName, bool bDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
float plLuaWrapper::GetFloatVariable(const char* szName, float fDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
const char* plLuaWrapper::GetStringVariable(const char* szName, const char* szDefault) const
{
  if (m_States.m_iOpenTables == 0)
    lua_getglobal(m_pState, szName);
  else
...
PLASMA_STATICLINK_FILE(Core, Core_Scripting_LuaWrapper_Variables);
