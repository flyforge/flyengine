...
template <typename R PLASMA_COMMA_IF(ARG_COUNT) PLASMA_LIST(typename P, ARG_COUNT)>
class plConsoleFunction<R(PLASMA_LIST(P, ARG_COUNT))> : public plConsoleFunctionBase
{
public:
  typedef plDelegate<R(PLASMA_LIST(P, ARG_COUNT))> FUNC;
  ...
  {
    PLASMA_ASSERT_DEV(uiParam < GetNumParameters(), "Invalid Parameter Index {0}", uiParam);
...
#  endif
  ...
  virtual plResult Call(plArrayPtr<plVariant> params) override
  {
    ...
    m_Func(PLASMA_LIST(param, ARG_COUNT));
