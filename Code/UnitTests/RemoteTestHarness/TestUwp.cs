...
namespace plUwpTestHarness
{
  ...
  class plTestUWP
  {
    ...
    public plTestUWP(string plWorkspace, string testOutputPath,
        string configuration, string platform, string project)
    {
      _plWorkspace = plWorkspace;
    ...
    private void DeployAppX(out string fullPackageName)
    {
      ...
      string absSlnPath = Path.Combine(_plWorkspace, "plVs2019.sln");
      ...
      // "C:\Program Files (x86)\Microsoft Visual Studio 14.0\Common7\IDE\devenv.com" "F:\Development\current_development\plEngine\build_uwp64\plEngine.sln" /Deploy "Dev|x64" /project CoreTest
      var deployProcessResults = plProcessHelper.RunExternalExe(devEnvPath,
    ...
    private void StartAppX(string fullPackageName, string args, out uint pid)
    {
      ...
      //
      // The following method seems to work just fine for plEngine projects if not for all AppX in general.
      //
      ...
      string appUserModelId = appFamilyName + "!App"; // Apply our knowledge of plEngine's appx manifests.
    ...
    public void ExecuteTest()
    {
      ...
      Func<plProcessHelper.ProcessResult> startFileserve = () =>
      {
        Console.WriteLine("Starting Fileserve ...");

        // 60s timeout for connect, 2s timeout for closing after connection loss.
        string args = string.Format("-specialdirs pltest \"{0}\" -fs_start -fs_wait_timeout 120 -fs_close_timeout 4", _absTestOutputDirectory);
        return plProcessHelper.RunExternalExe(absFilerserveFilename, args, absBinDir, fileserveTimeoutMS);
      };

      // create a real time user mode session
      using (var session = new TraceEventSession("plETWMonitorSession"))
      {
 
        session.EnableProvider(new Guid("BFD4350A-BA77-463D-B4BE-E30374E42494")); //plLogProvider
        session.Source.Dynamic.AddCallbackForProviderEvent("plLogProvider", "LogMessge", delegate (TraceEvent data)
        {
          ...
          if (Type != (int)plLogMsgType.EndGroup)
          {
            Console.Out.WriteLine("".PadLeft(Indentation) + Text);
          }

          if (Type == (int)plLogMsgType.InfoMsg && Text == "All tests passed.")
          {
            Interlocked.Exchange(ref this._failed, 0);
            Interlocked.Exchange(ref this._finished, 1);
          }
          else if (Type == (int)plLogMsgType.InfoMsg && Text != null && Text.StartsWith("Tests failed: ") && Text.Contains("Tests passed: "))
          {
        ...
        using (Task<plProcessHelper.ProcessResult> runFileServe = Task.Factory.StartNew(startFileserve, System.Threading.Tasks.TaskCreationOptions.LongRunning))
        {
          ...
          if (!appXProcess.WaitForExit(5000))
          {
            ...
            try
            {
              ...
              plProcessHelper.RunExternalExe("procdump", args, absBinDir);
            }
            catch (Exception e)
            {
