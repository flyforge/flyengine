...
static plGameEngineTestParticles s_GameEngineTestParticles;

const char* plGameEngineTestParticles::GetTestName() const
{
  return "Particle Tests";
}

plGameEngineTestApplication* plGameEngineTestParticles::CreateApplication()
{
  m_pOwnApplication = PLASMA_DEFAULT_NEW(plGameEngineTestApplication_Particles);
  return m_pOwnApplication;
}

void plGameEngineTestParticles::SetupSubTests()
{
...
plResult plGameEngineTestParticles::InitializeSubTest(plInt32 iIdentifier)
{
  ...
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/Billboards.plObjectGraph");
    return PLASMA_SUCCESS;
  }
  else if (iIdentifier == SubTests::PullAlongBehavior)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/PullAlong.plObjectGraph");
    return PLASMA_SUCCESS;
  }
  else if (iIdentifier == SubTests::DistanceEmitter)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/DistanceEmitter.plObjectGraph");
    return PLASMA_SUCCESS;
  }
  else if (iIdentifier == SubTests::SharedInstances)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/SharedInstances.plObjectGraph");
    return PLASMA_SUCCESS;
  }
  else if (iIdentifier == SubTests::EventReactionEffect)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/EventReactionEffect.plObjectGraph");
    return PLASMA_SUCCESS;
  }
  else if (iIdentifier == SubTests::LocalSpaceSim)
  {
    m_pOwnApplication->SetupSceneSubTest("Particles/AssetCache/Common/LocalSpaceSim.plObjectGraph");
    return PLASMA_SUCCESS;
  }
  else
  {
...
plTestAppRun plGameEngineTestParticles::RunSubTest(plInt32 iIdentifier, plUInt32 uiInvocationCount)
{
  ++m_iFrame;
...
plUInt32 plGameEngineTestParticles::GetImageCompareThreshold(plInt32 iIdentifier)
{
...
plGameEngineTestApplication_Particles::plGameEngineTestApplication_Particles()
  : plGameEngineTestApplication("Particles")
{
}

//////////////////////////////////////////////////////////////////////////

void plGameEngineTestApplication_Particles::SetupSceneSubTest(const char* szFile)
{
  LoadScene(szFile).IgnoreResult();
}

void plGameEngineTestApplication_Particles::SetupParticleSubTest(const char* szFile)
{
  LoadScene("Particles/AssetCache/Common/Particles1.plObjectGraph").IgnoreResult();
  ...
  {
    plParticleComponent* pEffect;
    m_pWorld->GetOrCreateComponentManager<plParticleComponentManager>()->CreateComponent(pObject, pEffect);
    pEffect->SetParticleEffectFile(szFile);
    pEffect->m_uiRandomSeed = 42;
  }
}

plTestAppRun plGameEngineTestApplication_Particles::ExecParticleSubTest(plInt32 iCurFrame)
{
  ...
  {
    case 15:
      PLASMA_TEST_IMAGE(0, m_uiImageCompareThreshold);
      break;

    case 30:
      PLASMA_TEST_IMAGE(1, m_uiImageCompareThreshold);
      break;

    case 60:
      PLASMA_TEST_IMAGE(2, m_uiImageCompareThreshold);
      return plTestAppRun::Quit;
  }
