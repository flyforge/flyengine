...
PLASMA_CREATE_SIMPLE_TEST_GROUP(CodeUtils);

plResult FileLocator(plStringView sCurAbsoluteFile, plStringView sIncludeFile, plPreprocessor::IncludeType incType, plStringBuilder& out_sAbsoluteFilePath)
{
  ...
  else if (incType == plPreprocessor::GlobalInclude)
  {
    s = "Preprocessor";
    s.AppendPath(sIncludeFile);
    s.MakeCleanPath();
...
class Logger : public plLogInterface
{
public:
  virtual void HandleLogMessage(const plLoggingEventData& le) override { m_sOutput.AppendFormat("Log: '{0}'\r\n", le.m_sText); }

  void EventHandler(const plPreprocessor::ProcessingEvent& ed)
  {
    switch (ed.m_Type)
    {
      case plPreprocessor::ProcessingEvent::Error:
      case plPreprocessor::ProcessingEvent::Warning:
        m_EventStack.PushBack(ed);
        break;
      case plPreprocessor::ProcessingEvent::BeginExpansion:
        m_EventStack.PushBack(ed);
        return;
      case plPreprocessor::ProcessingEvent::EndExpansion:
    ...
    for (plUInt32 i = 0; i < m_EventStack.GetCount(); ++i)
    {
      const plPreprocessor::ProcessingEvent& event = m_EventStack[i];
      ...
      switch (event.m_Type)
      {
        ...
        case plPreprocessor::ProcessingEvent::Warning:
          ...
          break;
        case plPreprocessor::ProcessingEvent::BeginExpansion:
          m_sOutput.AppendFormat("In Macro: '{0}'", plString(event.m_pToken->m_DataView));
          break;
        case plPreprocessor::ProcessingEvent::EndExpansion:
          break;
  ...
  plDeque<plPreprocessor::ProcessingEvent> m_EventStack;
  plStringBuilder m_sOutput;
};

PLASMA_CREATE_SIMPLE_TEST(CodeUtils, Preprocessor)
{
  ...
  PLASMA_TEST_BOOL(plFileSystem::AddDataDirectory(sReadDir, "PreprocessorTest") == PLASMA_SUCCESS);
  PLASMA_TEST_BOOL_MSG(plFileSystem::AddDataDirectory(sWriteDir, "PreprocessorTest", "output", plFileSystem::AllowWrites) == PLASMA_SUCCESS, "Failed to mount data dir '%s'", sWriteDir.GetData());

  plTokenizedFileCache SharedCache;
...

  {
    ...
    plStringBuilder fileNameOut;
    plStringBuilder fileNameExp;

    for (int i = 0; i < PLASMA_ARRAY_SIZE(TestSettings); i++)
    {
      PLASMA_TEST_BLOCK(plTestBlock::Enabled, TestSettings[i].m_szFileName)
      {
        Logger log;
        ...
        pp.m_ProcessingEvents.AddEventHandler(plDelegate<void(const plPreprocessor::ProcessingEvent&)>(&Logger::EventHandler, &log));
        pp.AddCustomDefine("PP_OBJ").IgnoreResult();
        pp.AddCustomDefine("PP_FUNC(a) a").IgnoreResult();
        pp.SetPassThroughUnknownCmdsCB([](plStringView s) -> bool { return s == "version"; }); // TestSettings[i].m_bPassThroughUnknownCommands);

        {
          ...
          plFileWriter fout;
          PLASMA_VERIFY(fout.Open(fileNameOut).Succeeded(), "Could not create output file '{0}'", fileNameOut);

          if (pp.Process(fileName, sOutput) == PLASMA_SUCCESS)
          {
            plString sError = "Processing succeeded\r\n";
          ...
          {
            plString sError = "Processing failed\r\n";
            fout.WriteBytes(sError.GetData(), sError.GetElementCount()).IgnoreResult();
          }

          fout.WriteBytes(log.m_sOutput.GetData(), log.m_sOutput.GetElementCount()).IgnoreResult();

          PLASMA_TEST_BOOL_MSG(plFileSystem::ExistsFile(fileNameOut), "Output file is missing: '%s'", fileNameOut.GetData());
        }

        PLASMA_TEST_TEXT_FILES(fileNameOut.GetData(), fileNameExp.GetData(), "");
      }
    }
  }


  plFileSystem::RemoveDataDirectoryGroup("PreprocessorTest");
}
