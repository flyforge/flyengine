...
class TestReader : public plJSONParser
{
private:
  plDeque<ParseResult> m_Results;
...
public:
  ...
  ~TestReader()
  {
    PLASMA_TEST_INT(m_iExpectedParsingErrors, 0);
    PLASMA_TEST_INT(m_Results.GetCount(), 0);
  }

  void ParseStream(plStreamReader& inout_stream)
  {
  ...
  {
    PLASMA_TEST_BOOL(!m_Results.IsEmpty());
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_Function == Variable);
    PLASMA_TEST_STRING(m_Results.PeekFront().m_szValue, sVarName);
  ...
  {
    PLASMA_TEST_BOOL(!m_Results.IsEmpty());
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_Function == ValueString);
    PLASMA_TEST_STRING(m_Results.PeekFront().m_szValue, sValue);
  ...
  {
    PLASMA_TEST_BOOL(!m_Results.IsEmpty());
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_Function == ValueDouble);
    PLASMA_TEST_DOUBLE(m_Results.PeekFront().m_fValue, fValue, 0.0001);
  ...
  {
    PLASMA_TEST_BOOL(!m_Results.IsEmpty());
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_Function == ValueBool);
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_bValue == bValue);
  ...
  {
    PLASMA_TEST_BOOL(!m_Results.IsEmpty());
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_Function == ValueNULL);
  ...
  {
    PLASMA_TEST_BOOL(!m_Results.IsEmpty());
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_Function == BeginObject);
  ...
  {
    PLASMA_TEST_BOOL(!m_Results.IsEmpty());
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_Function == EndObject);
  ...
  {
    PLASMA_TEST_BOOL(!m_Results.IsEmpty());
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_Function == BeginArray);
  ...
  {
    PLASMA_TEST_BOOL(!m_Results.IsEmpty());
    PLASMA_TEST_BOOL(m_Results.PeekFront().m_Function == EndArray);

    m_Results.PopFront();
  }

  plInt32 m_iExpectedParsingErrors;
  ...
  virtual void OnParsingError(plStringView sMessage, bool bFatal, plUInt32 uiLine, plUInt32 uiColumn) override
  {
    ...
    plStringBuilder tmp;
    PLASMA_TEST_FAILURE("JSON Parsing Error", "(%u, %u): %s", uiLine, uiColumn, sMessage.GetData(tmp));
  }
...
PLASMA_CREATE_SIMPLE_TEST(IO, JSONParser)
{
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "All Features")
  {
...
\"test\" : \"text\"\n\
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Empty Document (string")
  {
    const char* szTestData = "";
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Empty Document (Whitespace)")
  {
    const char* szTestData = " \n  \t ";
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Empty Document")
  {
    const char* szTestData = "{}";
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Two Empty Documents")
  {
    const char* szTestData = "{}{}";
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "No Whitespace")
  {
    ...
    PLASMA_TEST_INT(reader.m_iExpectedParsingErrors, 0);
  }

  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Empty Blocks")
  {
    // I want C++ 11 raw string literals
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Superfluous separators")
  {
    // I want C++ 11 raw string literals
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Too many commas in array")
  {
    // I want C++ 11 raw string literals
    ...
    PLASMA_TEST_INT(reader.m_iExpectedParsingErrors, 0); // must fail to parse
  }

  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Comments")
  {
    // I want C++ 11 raw string literals
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Skip Object")
  {
    // I want C++ 11 raw string literals
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Skip Array")
  {
    // I want C++ 11 raw string literals
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Skip Variable")
  {
    // I want C++ 11 raw string literals
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Unicode escape sequence")
  {
  ...
  PLASMA_TEST_BLOCK(plTestBlock::Enabled, "Unicode escape sequence parsing errors")
  {
    const char* szTestData = "{\"a\":\"\\u006G\",\"b\":\"\\u03f\",\"c\":\"a\\ud83pl\",\"d\":\"\\ud83e\t\"}";
