...
inline bool SortTest_Operator(const plTestEntry& lhs, const plTestEntry& rhs)
{
  return strcmp(lhs.m_szTestName, rhs.m_szTestName) < 0;
}

/// Operator to sort sub-tests alphabetically
inline bool SortSubTest_Operator(const plSubTestEntry& lhs, const plSubTestEntry& rhs)
{
  return strcmp(lhs.m_szSubTestName, rhs.m_szSubTestName) < 0;
}

/// Sorts all tests and subtests alphabetically
void SortTestsAlphabetically(std::deque<plTestEntry>& inout_tests)
{
  std::sort(inout_tests.begin(), inout_tests.end(), SortTest_Operator);

  for (plUInt32 i = 0; i < inout_tests.size(); ++i)
...
void SaveTestOrder(const char* szFile, const std::deque<plTestEntry>& allTests)
{
...
/// indented).
inline bool ReadLine(FILE* pFile, char* pDest, plUInt32 uiBufferSize)
{
  plUInt32 iPos = 0;
...
void LoadTestOrder(const char* szFile, std::deque<plTestEntry>& ref_allTests)
{
  ...
  for (plTestEntry& test : ref_allTests)
  {
    test.m_bEnableTest = false;
    for (plSubTestEntry& subTest : test.m_SubTests)
    {
      subTest.m_bEnableTest = false;
    }
  }

  plInt32 iLastMainTest = 0;
  ...
  while (!feof(pFile))
  {
    ...
    if (pEnd)
    {
      plInt32 iPos = (plInt32)(pEnd - szTestName);
      szTestName[iPos] = '\0';
    }
    ...
    if (szTestName[0] != '\0')
    {
      if (!bIndented)
      {
        ...
        for (plUInt32 t = 0; t < ref_allTests.size(); ++t)
        {
      ...
      else
      {
        ...
        if (iLastMainTest >= 0)
        {
          for (plUInt32 t = 0; t < ref_allTests[iLastMainTest].m_SubTests.size(); ++t)
          {
...
PLASMA_STATICLINK_FILE(TestFramework, TestFramework_Utilities_TestOrder);
