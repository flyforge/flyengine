...
////////////////////////////////////////////////////////////////////////
// plQtTestGUI public functions
////////////////////////////////////////////////////////////////////////

plQtTestGUI::plQtTestGUI(plQtTestFramework& ref_testFramework)
  : QMainWindow()
  , m_pTestFramework(&ref_testFramework)
{
  ...
  QCoreApplication::setApplicationName("plTestFramework");
  ...
  // Model
  m_pModel = new plQtTestModel(this, m_pTestFramework);
  testTreeView->setModel(m_pModel);

  // Delegate
  m_pDelegate = new plQtTestDelegate(this);
  ...
  // Message Log Dock
  m_pMessageLogDock = new plQtLogMessageDock(this, &m_pTestFramework->GetTestResult());
...
void plQtTestGUI::closeEvent(QCloseEvent* e)
{
...
////////////////////////////////////////////////////////////////////////
// plQtTestGUI public slots
////////////////////////////////////////////////////////////////////////

void plQtTestGUI::on_actionAssertOnTestFail_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void plQtTestGUI::on_actionOpenHTMLOutput_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void plQtTestGUI::on_actionKeepConsoleOpen_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void plQtTestGUI::on_actionShowMessageBox_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void plQtTestGUI::on_actionDisableSuccessfulTests_triggered(bool bChecked)
{
  TestSettings settings = m_pTestFramework->GetSettings();
...
void plQtTestGUI::on_actionSaveTestSettingsAs_triggered()
{
  plStringBuilder tmp;
  plStringView defaultDir = plPathUtils::GetFileDirectory(m_pTestFramework->GetAbsTestSettingsFilePath());
...
void plQtTestGUI::on_actionSaveTestOrderAs_triggered()
{
  plStringBuilder tmp;
  plStringView defaultDir = plPathUtils::GetFileDirectory(m_pTestFramework->GetAbsTestSettingsFilePath());
  QString dir = defaultDir.IsValid() ? defaultDir.GetData(tmp) : QString();
...
void plQtTestGUI::on_actionRunTests_triggered()
{
  ...
  while (m_pTestFramework->RunTestExecutionLoop() == plTestAppRun::Continue)
  {
    UpdateButtonStates();
...
void plQtTestGUI::on_actionAbort_triggered()
{
  m_bAbort = true;
}

void plQtTestGUI::on_actionQuit_triggered()
{
  close();
}

void plQtTestGUI::on_actionEnableOnlyThis_triggered()
{
...
void plQtTestGUI::on_actionEnableOnlyFailed_triggered()
{
  m_pTestFramework->SetAllFailedTestsEnabledStatus();
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void plQtTestGUI::on_actionEnableAllChildren_triggered()
{
  QModelIndex CurrentIndex = testTreeView->currentIndex();
  if (!CurrentIndex.isValid())
    return;
...
void plQtTestGUI::on_actionEnableAll_triggered()
{
  m_pTestFramework->SetAllTestsEnabledStatus(true);
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void plQtTestGUI::on_actionDisableAll_triggered()
{
  m_pTestFramework->SetAllTestsEnabledStatus(false);
  m_pModel->dataChanged(QModelIndex(), QModelIndex());
}

void plQtTestGUI::on_actionExpandAll_triggered()
{
  testTreeView->expandAll();
}

void plQtTestGUI::on_actionCollapseAll_triggered()
{
  testTreeView->collapseAll();
}

void plQtTestGUI::onTestFrameworkTestResultReceived(qint32 iTestIndex, qint32 iSubTestIndex)
{
  ...
  // Update status bar
  const plUInt32 uiTestCount = m_uiTestsEnabledCount;
  const plUInt32 uiFailed = m_pTestFramework->GetTestsFailedCount();
  const plUInt32 uiPassed = m_pTestFramework->GetTestsPassedCount();
  const plUInt32 uiErrors = m_pTestFramework->GetTotalErrorCount();
  ...
  {
    fSubTestPercentage =
      (float)m_pTestFramework->GetTestResult().GetSubTestCount(m_pTestFramework->GetCurrentTestIndex(), plTestResultQuery::Executed) /
...
void plQtTestGUI::onTestTreeViewCustomContextMenuRequested(const QPoint& pnt)
{
  QModelIndex CurrentIndex = testTreeView->currentIndex();
...
void plQtTestGUI::onSelectionModelCurrentRowChanged(const QModelIndex& index)
{
  if (!index.isValid())
  {
  ...
  const plTestResultData* pTestResult = pEntry->GetTestResult();
...
void plQtTestGUI::on_actionOpenTestDataFolder_triggered()
{
...
void plQtTestGUI::on_actionOpenOutputFolder_triggered()
{
...
void plQtTestGUI::on_actionOpenHTMLFile_triggered()
{
...
void plQtTestGUI::on_actionUpdateReferenceImages_triggered()
{
  m_pTestFramework->UpdateReferenceImages();
}

////////////////////////////////////////////////////////////////////////
// plQtTestGUI private functions
////////////////////////////////////////////////////////////////////////

void plQtTestGUI::UpdateButtonStates()
{
...
void plQtTestGUI::SaveGUILayout()
{
  QSettings Settings;
...
void plQtTestGUI::LoadGUILayout()
{
  QSettings Settings;
  Settings.beginGroup("MainWindow");
...
void plQtTestGUI::SetCheckStateRecursive(const QModelIndex& index, bool bChecked)
{
  m_pModel->setData(index, bChecked ? Qt::Checked : Qt::Unchecked, Qt::CheckStateRole);

  plInt32 iChildren = m_pModel->rowCount(index);
...
void plQtTestGUI::EnableAllParents(const QModelIndex& index)
{
...
////////////////////////////////////////////////////////////////////////
// plQtTestGUI public static functions
////////////////////////////////////////////////////////////////////////

void plQtTestGUI::SetDarkTheme()
{
  // return;
...
PLASMA_STATICLINK_FILE(TestFramework, TestFramework_Framework_Qt_qtTestGUI);
