...
class plQtTestModel;
class plQtTestDelegate;
class plQtLogMessageDock;
class QLabel;
class QPoint;
...
class PLASMA_TEST_DLL plQtTestGUI : public QMainWindow, public Ui_qtTestGUI
{
  Q_OBJECT
public:
  plQtTestGUI(plQtTestFramework& ref_testFramework);
  ~plQtTestGUI();

private:
  plQtTestGUI(plQtTestGUI&);
  void operator=(plQtTestGUI&);
...
private:
  plQtTestFramework* m_pTestFramework = nullptr;
  plQtTestModel* m_pModel = nullptr;
  plQtTestDelegate* m_pDelegate = nullptr;
  plQtLogMessageDock* m_pMessageLogDock = nullptr;
  QLabel* m_pStatusTextWorkState = nullptr;
  QLabel* m_pStatusText = nullptr;
  bool m_bExpandedCurrentTest = false;
  bool m_bAbort = false;
  plUInt32 m_uiTestsEnabledCount = 0;
  plUInt32 m_uiSubTestsEnabledCount = 0;
};
