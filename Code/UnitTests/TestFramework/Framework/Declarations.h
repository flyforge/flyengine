...
class plTestBaseClass;

/// \brief Stores the identification of a sub-test.
struct plSubTestEntry
{
  plSubTestEntry()

    = default;

  plInt32 m_iSubTestIdentifier = -1;
...
struct plTestEntry
{
  ...
  plTestBaseClass* m_pTest = nullptr;
  const char* m_szTestName = "";
  std::deque<plSubTestEntry> m_SubTests;
...
struct TestSettings
{
  ...
  plUInt8 m_uiFullPasses = 1;        /// All tests are done this often, to check whether some tests fail only when executed multiple times.
};
