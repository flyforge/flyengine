...
plResult CppStructure::ParseXML(plStreamReader& stream)
{
  ...
  for (auto node = root_node->first_node(); node != nullptr; node = node->next_sibling())
  {
    ...
    {
      plLog::Error("Unknown type '{}'", xmlNodeType);
    }
  }
...
plUInt32 CppStructure::ParseNamespace(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseTypedef(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseStructure(xml_node<>* node, CE_Structure::Type type)
{
  ...
  {
    plHybridArray<plStringView, 32> members;

    plStringBuilder mem = pMembersAttr->value();
    mem.Split(false, members, " ");

    ce.m_Members.SetCount(members.GetCount());
    for (plUInt32 i = 0; i < members.GetCount(); ++i)
    {
...
plUInt32 CppStructure::ParseFunction(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseVariable(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseOperatorFunction(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseUnimplemented(xml_node<>* node)
{
  // no interesting data in here
  return plMath::MaxValue<plUInt32>();
}

plUInt32 CppStructure::ParseCvQualifiedType(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseEnumeration(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseFundamentalType(xml_node<>* node)
{
...
plUInt32 CppStructure::ParsePointerType(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseConstructor(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseOperatorMethod(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseDestructor(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseField(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseElaboratedType(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseReferenceType(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseMethod(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseArrayType(xml_node<>* node)
{
  ...
  plConversionUtils::StringToUInt(pAttrSize->value(), ce.m_uiSize).IgnoreResult(); // this can fail, ie. the string can be empty

  return m_CesArrayType.GetCount() - 1;
}

plUInt32 CppStructure::ParseConverter(xml_node<>* node)
{
  return plMath::MaxValue<plUInt32>();
}

plUInt32 CppStructure::ParseFunctionType(xml_node<>* node)
{
  return plMath::MaxValue<plUInt32>();
}

plUInt32 CppStructure::ParseComment(xml_node<>* node)
{
...
plUInt32 CppStructure::ParseFile(xml_node<>* node)
{
  auto& ce = m_CesFile.ExpandAndGetRef();

  auto pAttr = node->first_attribute("name", 4);

  plStringBuilder tmp = pAttr->value();
  tmp.MakeCleanPath();
...
void CppStructure::ParseArguments(xml_node<>* arg, plDynamicArray<CE_Argument>& arguments)
{
  for (; arg != nullptr; arg = arg->next_sibling())
  {
    auto& a = arguments.ExpandAndGetRef();

    if (plStringUtils::IsEqual(arg->name(), "Ellipsis"))
    {
      a.m_sName = "...";
      a.m_sTypeID = "...";
    }
...
void CppStructure::ParseEnumValues(xml_node<>* arg, plDynamicArray<CE_EnumValue>& values)
{
  for (; arg != nullptr; arg = arg->next_sibling())
  {
...
void CppStructure::ParseVisibility(xml_node<>* node, CE_Visibility& out)
{
  ...
  {
    if (plStringUtils::IsEqual(pAttrVis->value(), "public"))
    {
      out = CE_Visibility::Public;
    }
    else if (plStringUtils::IsEqual(pAttrVis->value(), "protected"))
    {
      out = CE_Visibility::Protected;
    }
    else if (plStringUtils::IsEqual(pAttrVis->value(), "private"))
    {
      out = CE_Visibility::Private;
    }
    else
    {
      PLASMA_REPORT_FAILURE("Unknown access qualifier.");
    }
  }
}

void CppStructure::ParseContext(xml_node<>* node, plString& out)
{
...
plResult CppStructure::SynthesizeTemplate(const char* szTargetType, const char* szTemplateType1, const char* szTemplateType2)
{
  PLASMA_LOG_BLOCK("SynthesizeTemplate", szTargetType);
  ...
  {
    plLog::Error("Couldn't find both templates '{}' and '{}'", szTemplateType1, szTemplateType2);
  ...
  {
    plLog::Error("Member count differs: {} / {}", pC1->m_Members.GetCount(), pC2->m_Members.GetCount());
    return PLASMA_FAILURE;
  }
  ...
  for (plUInt32 m = 0; m < pC1->m_Members.GetCount(); ++m)
  {
    PLASMA_SUCCEED_OR_RETURN(SynthesizeMember(synth, pC1->m_Members[m], pC2->m_Members[m]));
  }
...
plString CppStructure::RegisterSynthElement(CodeElementType type, plUInt32 index)
{
  plStringBuilder tmp;
...
{
  for (plUInt32 i = 0; i < s.m_Members.GetCount(); ++i)
  {
...
plResult CppStructure::SynthesizeMember(CE_Structure& synth, const plString& m1, const plString& m2)
{
  ...
  {
    plLog::Error("Member CodeElementTypes differ: {} vs {}", (int)mt1.m_Type, (int)mt2.m_Type);
    return PLASMA_FAILURE;
  }
  ...
  switch (mt1.m_Type)
  {
    ...
    case CodeElementType::OperatorMethod:
    {
      ...
      const plString synthId = RegisterSynthElement(mt1.m_Type, m_CesOperatorMethod.GetCount() - 1);
    ...
    case CodeElementType::Method:
    {
      ...
      const plString synthId = RegisterSynthElement(mt1.m_Type, m_CesMethod.GetCount() - 1);
      synth.m_Members.PushBack(synthId);
    ...
    case CodeElementType::Field:
    {
      ...
      plString newType;
      PLASMA_SUCCEED_OR_RETURN(SynthesizeType(newType, f1.m_sTypeID, f2.m_sTypeID));
      ...
      const plString synthId = RegisterSynthElement(CodeElementType::Field, m_CesField.GetCount() - 1);
...
plResult CppStructure::SynthesizeMethod(CE_Method& out, const CE_Method& f1, const CE_Method& f2)
{
  if (f1.m_sName != f2.m_sName)
    return PLASMA_FAILURE;
  ...
  plString newType;
  PLASMA_SUCCEED_OR_RETURN(SynthesizeType(newType, f1.m_sReturnTypeID, f2.m_sReturnTypeID));
  ...
  for (plUInt32 arg = 0; arg < f1.m_Arguments.GetCount(); ++arg)
  {
    ...
    PLASMA_SUCCEED_OR_RETURN(SynthesizeType(newType, a1.m_sTypeID, a2.m_sTypeID));
...
plResult CppStructure::SynthesizeType(plString& type, const plString& m1, const plString& m2)
{
  if (m1 == m2)
  {
    type = m2;
  ...
  if (id1.m_Type == CodeElementType::CvQualifiedType)
  {
    ...
    PLASMA_SUCCEED_OR_RETURN(SynthesizeType(type, cv1.m_sTypeID, cv2.m_sTypeID));
  ...
  if (id1.m_Type == CodeElementType::ReferenceType)
  {
    ...
    PLASMA_SUCCEED_OR_RETURN(SynthesizeType(type, ref1.m_sTypeID, ref2.m_sTypeID));
  ...
  if (id1.m_Type == CodeElementType::PointerType)
  {
    ...
    PLASMA_SUCCEED_OR_RETURN(SynthesizeType(type, ptr1.m_sTypeID, ptr2.m_sTypeID));
  ...
  if (id1.m_Type == CodeElementType::Class || id1.m_Type == CodeElementType::Struct || id1.m_Type == CodeElementType::Union)
  {
    ...
    plStringBuilder n1 = cs1.m_sName;
    plStringBuilder n2 = cs2.m_sName;
