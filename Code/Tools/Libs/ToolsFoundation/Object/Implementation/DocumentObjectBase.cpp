...
{
  PL_ASSERT_DEV(pObject != nullptr, "");
  PL_ASSERT_DEV(!sProperty.IsEmpty(), "Child objects must have a parent property to insert into");
  ...
  PL_ASSERT_DEV(pProp && pProp->GetFlags().IsSet(plPropertyFlags::Class) &&
  ...
  {
    PL_ASSERT_DEV(index.IsA<plString>(), "Map key must be a string.");
    bool bRes = accessor.InsertValue(sProperty, index, pObject->GetGuid());
    PL_ASSERT_DEV(bRes, "");
  }
...
{
  PL_ASSERT_DEV(pObject != nullptr, "");
  PL_ASSERT_DEV(!pObject->m_sParentProperty.IsEmpty(), "");
  PL_ASSERT_DEV(this == pObject->m_pParent, "");
  plIReflectedTypeAccessor& accessor = GetTypeAccessor();
...
bool plDocumentObject::IsOnHeap() const
{
  ...
  PL_ASSERT_DEV(m_pParent != nullptr,
