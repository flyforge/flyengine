  ...
  , m_sRootProperty(sRootProperty)
{
  ...
  PL_ASSERT_DEV(pRootProp, "Given root property '{0}' does not exist on root object", sRootProperty);
  PL_ASSERT_DEV(pRootProp->GetCategory() == plPropertyCategory::Set || pRootProp->GetCategory() == plPropertyCategory::Array,
  ...
  // PL_ASSERT_DEV(pChildProp, "Given child property '{0}' does not exist", szChildrenProperty);
  // PL_ASSERT_DEV(pChildProp->GetCategory() == plPropertyCategory::Set || pRootProp->GetCategory() == plPropertyCategory::Array, "Traverser
...
void plDocumentObjectVisitor::TraverseChildren(const plDocumentObject* pObject, plStringView sProperty, VisitorFunction& function)
{
  ...
  for (plInt32 i = 0; i < iChildren; i++)
  {
    ...
    PL_ASSERT_DEBUG(obj.IsValid() && obj.IsA<plUuid>(), "null obj found during traversal.");
