  ...
  const plDocumentObject* pObject, const plAbstractProperty* pProp, const plVariant& newValue, plVariant index)
{
  ...
  PL_ASSERT_DEBUG(pObj, "Object is not part of this document manager.");
  bool bRes = pObj->GetTypeAccessor().SetValue(pProp->GetPropertyName(), newValue, index);
  return plStatus(bRes ? PL_SUCCESS : PL_FAILURE);
}
...
{
  PL_SUCCEED_OR_RETURN(m_pManager->CanAdd(pType, pParent, pParentProp->GetPropertyName(), index));

  plDocumentObject* pPar = m_pManager->GetObject(pParent->GetGuid());
  PL_ASSERT_DEBUG(pPar, "Parent is not part of this document manager.");
...
{
  PL_SUCCEED_OR_RETURN(m_pManager->CanRemove(pObject));
...
{
  PL_SUCCEED_OR_RETURN(m_pManager->CanMove(pObject, pNewParent, pParentProp->GetPropertyName(), index));
