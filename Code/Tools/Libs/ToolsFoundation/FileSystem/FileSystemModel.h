...
struct PL_TOOLSFOUNDATION_DLL plFolderChangedEvent
{
  enum class Type
  {
    None,
...
struct PL_TOOLSFOUNDATION_DLL plFileChangedEvent
{
  enum class Type
  {
    None,
...
class PL_TOOLSFOUNDATION_DLL plFileSystemModel
{
  PL_DECLARE_SINGLETON(plFileSystemModel);
...
public:
  ...
  /// \return Returns PL_SUCCESS if the file was found.
  ...
  /// \param visitor Called for every file in the model. If this functions returns true, the search is canceled and the function returns PL_SUCCESS.
  /// \return Returns PL_SUCCESS if the visitor returned true for a file.
  ...
  /// \return Returns PL_SUCCESS if the file existed in the model and could be linked.
  plResult LinkDocument(plStringView sAbsolutePath, const plUuid& documentId);

  /// \brief Unlinks a document from a file
  /// \param sAbsolutePath Path to the document. Must be in the model.
  /// \return Returns PL_SUCCESS if the file existed.
  ...
  /// \return Returns PL_SUCCESS if the file existed and could be opened. Returns PL_FAILURE if the file is not in the model or the file can't be opened for read access. On read failure, the file will be marked as locked.
  ...
  /// \return Returns PL_SUCCESS if the file existed and could be opened. On failure, the file will be marked as locked.
