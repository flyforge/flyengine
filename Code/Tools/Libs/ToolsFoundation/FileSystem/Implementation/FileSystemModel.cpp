...
PLASMA_IMPLEMENT_SINGLETON(plFileSystemModel);

// clang-format off
PLASMA_BEGIN_SUBSYSTEM_DECLARATION(ToolsFoundation, FileSystemModel)
  ...
  {
    PLASMA_DEFAULT_NEW(plFileSystemModel);
  }
...
namespace
{
  thread_local plHybridArray<plFileChangedEvent, 2, plStaticAllocatorWrapper> g_PostponedFiles;
  thread_local bool g_bInFileBroadcast = false;
  thread_local plHybridArray<plFolderChangedEvent, 2, plStaticAllocatorWrapper> g_PostponedFolders;
...
{
#  if (PLASMA_ENABLED(PLASMA_SUPPORTS_CASE_INSENSITIVE_PATHS))
...
{
  {
    ...
    m_pWatcher = PLASMA_DEFAULT_NEW(plFileSystemWatcher, m_FileSystemConfig);
...
void plFileSystemModel::NotifyOfChange(plStringView sAbsolutePath)
{
  ...
  PLASMA_ASSERT_DEV(plPathUtils::IsAbsolutePath(sAbsolutePath), "Only absolute paths are supported for directory iteration.");
...
plResult plFileSystemModel::HashFile(plStringView sAbsolutePath, plFileStatus& out_stat)
{
  ...
  PLASMA_ASSERT_DEV(plPathUtils::IsAbsolutePath(sAbsolutePath), "Only absolute paths are supported for hashing.");
...
plUInt64 plFileSystemModel::HashFile(plStreamReader& ref_inputStream, plStreamWriter* pPassThroughStream)
{
  ...
  while (true)
  {
    const plUInt64 uiRead = ref_inputStream.ReadBytes(cachedBytes, PLASMA_ARRAY_SIZE(cachedBytes));
...
plResult plFileSystemModel::ReadDocument(plStringView sAbsolutePath, const plDelegate<void(const plFileStatus&, plStreamReader&)>& callback)
{
  ...
  plFileReader file;
  if (file.Open(sAbsolutePath2) == PLASMA_FAILURE)
  {
    MarkFileLocked(sAbsolutePath2);
  ...
  bool bFileChanged = false;
  {
    ...
    {
      PLASMA_REPORT_FAILURE("A file was removed from the model while we had a lock on it.");
    }
...
void plFileSystemModel::CheckFolder(plStringView sAbsolutePath)
{
  plStringBuilder sAbsolutePath2 = sAbsolutePath;
  sAbsolutePath2.MakeCleanPath();
  PLASMA_ASSERT_DEV(plPathUtils::IsAbsolutePath(sAbsolutePath2), "Only absolute paths are supported for directory iteration.");
  sAbsolutePath2.Trim(nullptr, "/");
...
void plFileSystemModel::FireFileChangedEvent(const plDataDirPath& file, plFileStatus fileStatus, plFileChangedEvent::Type type)
{
  ...
  PLASMA_SCOPE_EXIT(g_bInFileBroadcast = false);
...
void plFileSystemModel::FireFolderChangedEvent(const plDataDirPath& file, plFolderChangedEvent::Type type)
{
  ...
  PLASMA_SCOPE_EXIT(g_bInFolderBroadcast = false);
