...
void plFileSystemWatcher::Initialize()
{
  ...
  for (auto& dd : m_FileSystemConfig.m_DataDirs)
  {
    ...
    plDirectoryWatcher* pWatcher = PL_DEFAULT_NEW(plDirectoryWatcher);
  ...
  m_pWatcherTask = PL_DEFAULT_NEW(plDelegateTask<void>, "Watcher Changes", plTaskNesting::Never, [this]() {
  ...
  m_pNotifyTask = PL_DEFAULT_NEW(plDelegateTask<void>, "Watcher Notify", plTaskNesting::Never, [this]() { NotifyChanges(); });
}
...
{
  PL_PROFILE_SCOPE("plAssetWatcherTick");
  PL_LOCK(m_WatcherMutex);
...
void plFileSystemWatcher::HandleWatcherChange(const WatcherResult& res)
{
  switch (res.m_Action)
  {
    case plDirectoryWatcherAction::None:
      PL_ASSERT_DEV(false, "None event should never happen");
      break;
