...
#define PL_REGISTER_ACTION_0(ActionName, Scope, CategoryName, ShortCut, ActionClass)                                  \
  plActionManager::RegisterAction(plActionDescriptor(plActionType::Action, Scope, ActionName, CategoryName, ShortCut, \
    [](const plActionContext& context) -> plAction* { return PL_DEFAULT_NEW(ActionClass, context, ActionName); }));

/// \brief Registers an plAction whose constructor takes one argument.
#define PL_REGISTER_ACTION_1(ActionName, Scope, CategoryName, ShortCut, ActionClass, Param1)                          \
  plActionManager::RegisterAction(plActionDescriptor(plActionType::Action, Scope, ActionName, CategoryName, ShortCut, \
    [](const plActionContext& context) -> plAction* { return PL_DEFAULT_NEW(ActionClass, context, ActionName, Param1); }));

/// \brief Registers an plAction whose constructor takes two arguments.
#define PL_REGISTER_ACTION_2(ActionName, Scope, CategoryName, ShortCut, ActionClass, Param1, Param2)                  \
  plActionManager::RegisterAction(plActionDescriptor(plActionType::Action, Scope, ActionName, CategoryName, ShortCut, \
    [](const plActionContext& context) -> plAction* { return PL_DEFAULT_NEW(ActionClass, context, ActionName, Param1, Param2); }));

/// \brief Registers an plDynamicMenuAction
#define PL_REGISTER_DYNAMIC_MENU(ActionName, ActionClass, IconPath)                                                  \
  plActionManager::RegisterAction(plActionDescriptor(plActionType::Menu, plActionScope::Default, ActionName, "", "", \
    [](const plActionContext& context) -> plAction* { return PL_DEFAULT_NEW(ActionClass, context, ActionName, IconPath); }));

/// \brief Registers an plDynamicActionAndMenuAction.
#define PL_REGISTER_ACTION_AND_DYNAMIC_MENU_1(ActionName, Scope, CategoryName, ShortCut, ActionClass, Param1)                \
...
#define PL_REGISTER_MENU(ActionName)                                                                                 \
  plActionManager::RegisterAction(plActionDescriptor(plActionType::Menu, plActionScope::Default, ActionName, "", "", \
    [](const plActionContext& context) -> plAction* { return PL_DEFAULT_NEW(plMenuAction, context, ActionName, ""); }));

/// \brief Registers a category that should be treated as a sub-menu and specifies a custom QIcon path.
#define PL_REGISTER_MENU_WITH_ICON(ActionName, IconPath)                                                             \
  plActionManager::RegisterAction(plActionDescriptor(plActionType::Menu, plActionScope::Default, ActionName, "", "", \
    [](const plActionContext& context) -> plAction* { return PL_DEFAULT_NEW(plMenuAction, context, ActionName, IconPath); }));

/// \brief Registers a category that should just be a grouped area in a menu, but no dedicated sub-menu.
#define PL_REGISTER_CATEGORY(CategoryName)                                                                                 \
  plActionManager::RegisterAction(plActionDescriptor(plActionType::Category, plActionScope::Default, CategoryName, "", "", \
    [](const plActionContext& context) -> plAction* { return PL_DEFAULT_NEW(plCategoryAction, context); }));
...
/// These functions are usually not called directly, but rather the macros at the top of this file are used (see PL_REGISTER_CATEGORY, PL_REGISTER_MENU, PL_REGISTER_ACTION_X, ...).
///
...
class PL_GUIFOUNDATION_DLL plActionManager
{
public:
...
private:
  PL_MAKE_SUBSYSTEM_STARTUP_FRIEND(GuiFoundation, ActionManager);
