...
plQtDoubleSpinBox::plQtDoubleSpinBox(QWidget* pParent, bool bIntMode)
...
void plQtDoubleSpinBox::setDefaultValue(double value)
{
  m_fDefaultValue = value;
}


void plQtDoubleSpinBox::setDefaultValue(const plVariant& val)
{
  double fValue = 0;
  if (plToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setDefaultValue(fValue);
}

void plQtDoubleSpinBox::setMinimum(const plVariant& val)
{
  double fValue = 0;
  if (plToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setMinimum(fValue);
...
void plQtDoubleSpinBox::setMaximum(const plVariant& val)
{
  double fValue = 0;
  if (plToolsReflectionUtils::GetFloatFromVariant(val, fValue))
    setMaximum(fValue);
...
void plQtDoubleSpinBox::setValueInvalid()
{
  m_bInvalid = true;
...
{
  PL_ASSERT_DEBUG(plMath::IsFinite(fVal), "Spin box value must be finite!");
  m_bInvalid = false;
  m_fDisplayedValue = plMath::NaN<float>();
...
double plQtDoubleSpinBox::value() const
{
  if (m_bInvalid)
    return 0.0;

  PL_ASSERT_DEBUG(!plMath::IsNaN(QDoubleSpinBox::value()), "Spin box valid value should never be NaN!");
...
void plQtDoubleSpinBox::focusOutEvent(QFocusEvent* event)
{
  QDoubleSpinBox::focusOutEvent(event);
}

void plQtDoubleSpinBox::mousePressEvent(QMouseEvent* event)
{
  if (!isReadOnly())
  {
...
void plQtDoubleSpinBox::mouseReleaseEvent(QMouseEvent* event)
{
  if (!isReadOnly())
  {
...
void plQtDoubleSpinBox::mouseMoveEvent(QMouseEvent* event)
{
  if (!isReadOnly())
  {
    if (m_bDragging)
...
void plQtDoubleSpinBox::keyPressEvent(QKeyEvent* event)
{
...
bool plQtDoubleSpinBox::event(QEvent* event)
{
...
void plQtDoubleSpinBox::onCustomContextMenuRequested()
{
  if (!isReadOnly())
  {
