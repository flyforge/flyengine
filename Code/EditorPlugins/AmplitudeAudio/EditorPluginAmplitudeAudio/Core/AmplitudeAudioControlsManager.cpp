...
PLASMA_IMPLEMENT_SINGLETON(plAmplitudeAudioControlsManager);
...
plResult plAmplitudeAudioControlsManager::CreateTriggerControl(const char* szControlName, const plAudioSystemTriggerData* pControlData)
{
  ...
  sbOutputFile.Format(":atl/Triggers/{0}.plAudioSystemControl", szControlName);
...
{
  plStringBuilder sbOutputFile;
  sbOutputFile.Format(":atl/Rtpcs/{0}.plAudioSystemControl", szControlName);
...
{
  plStringBuilder sbOutputFile;
  sbOutputFile.Format(":atl/SwitchStates/{0}.plAudioSystemControl", szControlName);
...
{
  plStringBuilder sbOutputFile;
  sbOutputFile.Format(":atl/Environments/{0}.plAudioSystemControl", szControlName);
...
{
  plStringBuilder sbOutputFile;
  sbOutputFile.Format(":atl/SoundBanks/{0}.plAudioSystemControl", szControlName);
...
plResult plAmplitudeAudioControlsManager::LoadSoundBanks(const char* sRootFolder, const char* sSubPath)
{
  ...
  for (fsIt.StartSearch(searchPath, plFileSystemIteratorFlags::ReportFilesRecursive); fsIt.IsValid(); fsIt.Next())
  {
    ...
    if (json.Parse(reader).Succeeded())
    {
      ...
      plAmplitudeAudioSoundBankData* control = PLASMA_AUDIOSYSTEM_NEW(plAmplitudeAudioSoundBankData, id->ConvertTo<plUInt64>(), fsIt.GetStats().m_sName);
      const plResult result = CreateSoundBankControl(controlName, control);
      PLASMA_AUDIOSYSTEM_DELETE(control);
      if (result.Failed())
        return PLASMA_FAILURE;
    }
    else
...
plResult plAmplitudeAudioControlsManager::LoadControl(const plVariantDictionary& json, const plEnum<plAmplitudeAudioControlType>& eType)
{
  if (json.Contains("name"))
  {
    ...
    switch (eType)
    {
      ...
      {
        plAmplitudeAudioTriggerData* control = PLASMA_AUDIOSYSTEM_NEW(plAmplitudeAudioTriggerData, id->ConvertTo<plUInt64>());
      ...
      {
        plAmplitudeAudioRtpcData* control = PLASMA_AUDIOSYSTEM_NEW(plAmplitudeAudioRtpcData, id->ConvertTo<plUInt64>());
      ...
      case plAmplitudeAudioControlType::Switch:
      {
        if (json.Contains("states"))
        {
          ...
          for (const auto& state : states->Get<plVariantArray>())
          {
            ...
            plAmplitudeAudioSwitchStateData* control = PLASMA_AUDIOSYSTEM_NEW(plAmplitudeAudioSwitchStateData, id->ConvertTo<plUInt64>(), value.GetValue("id")->ConvertTo<plUInt64>());
      ...
      {
        plAmplitudeAudioEnvironmentData* control = PLASMA_AUDIOSYSTEM_NEW(plAmplitudeAudioEnvironmentData, id->ConvertTo<plUInt64>(), json.GetValue("effect")->ConvertTo<plUInt64>());
...
PLASMA_STATICLINK_FILE(EditorPluginAmplitudeAudio, EditorPluginAmplitudeAudio_AmplitudeAudioControlsManager);
