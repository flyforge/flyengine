...
plUInt32 plQtDuplicateDlg::s_uiNumberOfCopies = 1;
bool plQtDuplicateDlg::s_bGroupCopies = false;
plVec3 plQtDuplicateDlg::s_vTranslationStep(0, 0, 0);
plVec3 plQtDuplicateDlg::s_vRotationStep(0, 0, 0);
plVec3 plQtDuplicateDlg::s_vRandomTranslation(0, 0, 0);
plVec3 plQtDuplicateDlg::s_vRandomRotation(0, 0, 0);
int plQtDuplicateDlg::s_iRevolveAxis = 0;
int plQtDuplicateDlg::s_iRevolveStartAngle = 0;
int plQtDuplicateDlg::s_iRevolveAngleStep = 0;
  ...
  : QDialog(parent)
{
  ...
  m_vBoundingBoxSize.x = plMath::RoundToMultiple(m_vBoundingBoxSize.x, 0.01f);
  m_vBoundingBoxSize.y = plMath::RoundToMultiple(m_vBoundingBoxSize.y, 0.01f);
  m_vBoundingBoxSize.z = plMath::RoundToMultiple(m_vBoundingBoxSize.z, 0.01f);
...
void plQtDuplicateDlg::on_DefaultButtons_clicked(QAbstractButton* button)
{
  ...
  if (button == DefaultButtons->button(QDialogButtonBox::Ok))
  {
    ...
    s_fRevolveRadius = plMath::Clamp((float)SpinBoxRadius->value(), 0.0f, 1000.0f);

    accept();
  }
}

void plQtDuplicateDlg::on_toolButtonTransX_clicked()
{
  const float f = plMath::RoundToMultiple((float)SpinBoxTransX->value(), 0.01f);
...
void plQtDuplicateDlg::on_toolButtonTransY_clicked()
{
  const float f = plMath::RoundToMultiple((float)SpinBoxTransY->value(), 0.01f);
...
void plQtDuplicateDlg::on_toolButtonTransZ_clicked()
{
  const float f = plMath::RoundToMultiple((float)SpinBoxTransZ->value(), 0.01f);
...
void plQtDuplicateDlg::on_RevolveNone_clicked()
{
...
void plQtDuplicateDlg::on_RevolveX_clicked()
{
...
void plQtDuplicateDlg::on_RevolveY_clicked()
{
...
void plQtDuplicateDlg::on_RevolveZ_clicked()
{
