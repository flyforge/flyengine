...
plQtFileserveAllFilesModel::plQtFileserveAllFilesModel(QWidget* parent)
...
QVariant plQtFileserveAllFilesModel::data(const QModelIndex& index, int role /*= Qt::DisplayRole*/) const
{
  if (!index.isValid())
    return QVariant();
...
QVariant plQtFileserveAllFilesModel::headerData(int section, Qt::Orientation orientation, int role /*= Qt::DisplayRole*/) const
{
  if (role == Qt::DisplayRole)
  {
    if (section == 0)
