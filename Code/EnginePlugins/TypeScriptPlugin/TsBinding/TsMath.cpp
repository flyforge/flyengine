...
void plTypeScriptBinding::PushVec2(duk_context* pDuk, const plVec2& value)
{
  ...
  PL_VERIFY(duk.PushLocalObject("__Vec2").Succeeded(), ""); // [ global __Vec2 ]
...
void plTypeScriptBinding::SetVec2Property(duk_context* pDuk, const char* szPropertyName, plInt32 iObjIdx, const plVec2& value)
{
  ...
  PL_VERIFY(duk.PushLocalObject(szPropertyName, iObjIdx).Succeeded(), "invalid property");
...
plVec2 plTypeScriptBinding::GetVec2(duk_context* pDuk, plInt32 iObjIdx, const plVec2& vFallback /*= plVec2::MakeZero()*/)
{
  ...
  PL_VERIFY(duk_get_prop_string(pDuk, iObjIdx, "x"), "");
  res.x = static_cast<float>(duk_get_number_default(pDuk, -1, vFallback.x));
  duk_pop(pDuk);
  PL_VERIFY(duk_get_prop_string(pDuk, iObjIdx, "y"), "");
...
plVec2 plTypeScriptBinding::GetVec2Property(
  duk_context* pDuk, const char* szPropertyName, plInt32 iObjIdx, const plVec2& vFallback /*= plVec2::MakeZero()*/)
{
  plDuktapeHelper duk(pDuk);

  if (duk.PushLocalObject(szPropertyName, iObjIdx).Failed()) // [ prop ]
  {
    PL_DUK_RETURN_AND_VERIFY_STACK(duk, vFallback, 0);
  }

  const plVec2 res = GetVec2(pDuk, -1, vFallback);
  duk.PopStack(); // [ ]
  PL_DUK_RETURN_AND_VERIFY_STACK(duk, res, 0);
}
...
void plTypeScriptBinding::PushVec3(duk_context* pDuk, const plVec3& value)
{
  ...
  duk.PushGlobalObject();                                   // [ global ]
  PL_VERIFY(duk.PushLocalObject("__Vec3").Succeeded(), ""); // [ global __Vec3 ]
...
plVec3 plTypeScriptBinding::GetVec3(duk_context* pDuk, plInt32 iObjIdx, const plVec3& vFallback /*= plVec3::MakeZero()*/)
{
  if (duk_is_null_or_undefined(pDuk, iObjIdx))
    return vFallback;
  ...
  res.y = static_cast<float>(duk_get_number_default(pDuk, -1, vFallback.y));
  duk_pop(pDuk);
  PL_VERIFY(duk_get_prop_string(pDuk, iObjIdx, "z"), "");
...
plVec3 plTypeScriptBinding::GetVec3Property(
  duk_context* pDuk, const char* szPropertyName, plInt32 iObjIdx, const plVec3& vFallback /*= plVec3::MakeZero()*/)
{
  plDuktapeHelper duk(pDuk);
  ...
  const plVec3 res = GetVec3(pDuk, -1, vFallback);
  duk.PopStack(); // [ ]
...
void plTypeScriptBinding::PushMat3(duk_context* pDuk, const plMat3& value)
{
  ...
  duk.PushGlobalObject();                                   // [ global ]
  PL_VERIFY(duk.PushLocalObject("__Mat3").Succeeded(), ""); // [ global __Mat3 ]
...
void plTypeScriptBinding::SetMat3(duk_context* pDuk, plInt32 iObjIdx, const plMat3& value)
{
  ...
  PL_VERIFY(duk.PushLocalObject("m_ElementsCM", iObjIdx).Succeeded(), "invalid property");
...
plMat3 plTypeScriptBinding::GetMat3(duk_context* pDuk, plInt32 iObjIdx, const plMat3& mFallback /*= plMat3::MakeZero()*/)
{
  if (duk_is_null_or_undefined(pDuk, iObjIdx))
    return mFallback;
...
plMat3 plTypeScriptBinding::GetMat3Property(
  duk_context* pDuk, const char* szPropertyName, plInt32 iObjIdx, const plMat3& mFallback /*= plMat3::MakeZero()*/)
{
  plDuktapeHelper duk(pDuk);

  if (duk.PushLocalObject(szPropertyName, iObjIdx).Failed()) // [ prop ]
  {
    PL_DUK_RETURN_AND_VERIFY_STACK(duk, mFallback, 0);
  }

  const plMat3 res = GetMat3(pDuk, -1, mFallback);
  duk.PopStack(); // [ ]
...
void plTypeScriptBinding::PushMat4(duk_context* pDuk, const plMat4& value)
{
  ...
  duk.PushGlobalObject();                                   // [ global ]
  PL_VERIFY(duk.PushLocalObject("__Mat4").Succeeded(), ""); // [ global __Mat4 ]
...
plMat4 plTypeScriptBinding::GetMat4(duk_context* pDuk, plInt32 iObjIdx, const plMat4& mFallback /*= plMat4::MakeZero()*/)
{
  if (duk_is_null_or_undefined(pDuk, iObjIdx))
    return mFallback;
...
plMat4 plTypeScriptBinding::GetMat4Property(
  duk_context* pDuk, const char* szPropertyName, plInt32 iObjIdx, const plMat4& mFallback /*= plMat4::MakeZero()*/)
{
  plDuktapeHelper duk(pDuk);
  ...
  const plMat4 res = GetMat4(pDuk, -1, mFallback);
  duk.PopStack(); // [ ]
...
void plTypeScriptBinding::PushQuat(duk_context* pDuk, const plQuat& value)
{
  ...
  duk.PushGlobalObject();                                   // [ global ]
  PL_VERIFY(duk.PushLocalObject("__Quat").Succeeded(), ""); // [ global __Quat ]
...
plQuat plTypeScriptBinding::GetQuat(duk_context* pDuk, plInt32 iObjIdx, plQuat qFallback /*= plQuat::MakeIdentity()*/)
{
  if (duk_is_null_or_undefined(pDuk, iObjIdx))
    return qFallback;
  ...
  duk_pop(pDuk);
  PL_VERIFY(duk_get_prop_string(pDuk, iObjIdx, "w"), "");
...
plQuat plTypeScriptBinding::GetQuatProperty(
  duk_context* pDuk, const char* szPropertyName, plInt32 iObjIdx, plQuat qFallback /*= plQuat::MakeIdentity()*/)
{
  plDuktapeHelper duk(pDuk);

  if (duk.PushLocalObject(szPropertyName, iObjIdx).Failed()) // [ prop ]
  {
    PL_DUK_RETURN_AND_VERIFY_STACK(duk, qFallback, 0);
  }

  const plQuat res = GetQuat(pDuk, -1, qFallback);
  duk.PopStack(); // [ ]
  PL_DUK_RETURN_AND_VERIFY_STACK(duk, res, 0);
}
...
void plTypeScriptBinding::PushColor(duk_context* pDuk, const plColor& value)
{
  ...
  duk.PushGlobalObject();                                    // [ global ]
  PL_VERIFY(duk.PushLocalObject("__Color").Succeeded(), ""); // [ global __Color ]
  ...
  duk_context* pDuk, const char* szPropertyName, plInt32 iObjIdx, const plColor& fallback /*= plColor::White*/)
{
  ...
  if (duk.PushLocalObject(szPropertyName, iObjIdx).Failed()) // [ prop ]
  {
    PL_DUK_RETURN_AND_VERIFY_STACK(duk, fallback, 0);
  }
...
void plTypeScriptBinding::PushTransform(duk_context* pDuk, const plTransform& value)
{
  ...
  duk.PushGlobalObject();                                        // [ global ]
  PL_VERIFY(duk.PushLocalObject("__Transform").Succeeded(), ""); // [ global __Transform ]
...
plTransform plTypeScriptBinding::GetTransform(duk_context* pDuk, plInt32 iObjIdx, const plTransform& fallback /*= plTransform::MakeIdentity()*/)
{
  if (duk_is_null_or_undefined(pDuk, iObjIdx))
    return fallback;
...
plTransform plTypeScriptBinding::GetTransformProperty(
  duk_context* pDuk, const char* szPropertyName, plInt32 iObjIdx, const plTransform& fallback /*= plTransform::MakeIdentity()*/)
{
  plDuktapeHelper duk(pDuk);
...
plVariant plTypeScriptBinding::GetVariant(duk_context* pDuk, plInt32 iObjIdx, const plRTTI* pType)
{
  ...
  switch (pType->GetVariantType())
  {
    ...
    case plVariant::Type::Time:
      return plTime::MakeFromSeconds(duk.GetFloatValue(iObjIdx));
    ...
    case plVariant::Type::StringView:
      return plVariant(plStringView(duk.GetStringValue(iObjIdx)), false);
