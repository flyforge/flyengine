...
{
  plStringBuilder sAbsPathToPlTemplate, sAbsPathToData;
  PL_SUCCEED_OR_RETURN(plFileSystem::ResolvePath(":plugins/TypeScript/pl-template.ts", &sAbsPathToPlTemplate, nullptr));

  sAbsPathToData = sAbsPathToPlTemplate;
  sAbsPathToData.PathParentDirectory();

  plStringBuilder sPlFileContent;

  // read pl.ts
  {
    plFileReader filePl;
    PL_SUCCEED_OR_RETURN(filePl.Open(sAbsPathToPlTemplate));
    sPlFileContent.ReadAll(filePl);
  }
  ...
  RemoveAutoGeneratedCode(sPlFileContent);

  GenerateComponentsFile(":project/TypeScript/pl/AllComponents.ts");
  InjectComponentImportExport(sPlFileContent, "./pl/AllComponents");

  GenerateMessagesFile(":project/TypeScript/pl/AllMessages.ts");
  InjectMessageImportExport(sPlFileContent, "./pl/AllMessages");
  ...
  InjectEnumImportExport(sPlFileContent, "./pl/AllEnums");
  InjectFlagsImportExport(sPlFileContent, "./pl/AllFlags");

  {
    plDeferredFileWriter fileOut;
    fileOut.SetOutput(":project/TypeScript/pl.ts", true);
    PL_SUCCEED_OR_RETURN(fileOut.WriteBytes(sPlFileContent.GetData(), sPlFileContent.GetElementCount()));
    PL_SUCCEED_OR_RETURN(fileOut.Close());
  }
