...
// clang-format off
PL_BEGIN_COMPONENT_TYPE(plRcAgentComponent, 2, plComponentMode::Dynamic)
{
  PL_BEGIN_PROPERTIES
  {
    PL_MEMBER_PROPERTY("WalkSpeed",m_fWalkSpeed)->AddAttributes(new plDefaultValueAttribute(4.0f)),
  }
  PL_END_PROPERTIES;
}
...
plResult plRcAgentComponent::InitializeRecast()
{
  ...
  m_pQuery = PL_DEFAULT_NEW(dtNavMeshQuery);
  m_pCorridor = PL_DEFAULT_NEW(dtPathCorridor);
...
void plRcAgentComponentManager::Initialize()
{
  ...
  auto desc = PL_CREATE_MODULE_UPDATE_FUNCTION_DESC(plRcAgentComponentManager::Update, this);
