...
// clang-format off
PLASMA_IMPLEMENT_WORLD_MODULE(plRecastWorldModule);
PLASMA_BEGIN_DYNAMIC_REFLECTED_TYPE(plRecastWorldModule, 1, plRTTINoAllocator)
...
void plRecastWorldModule::Initialize()
{
  ...
  {
    auto updateDesc = PLASMA_CREATE_MODULE_UPDATE_FUNCTION_DESC(plRecastWorldModule::UpdateNavMesh, this);
...
void plRecastWorldModule::UpdateNavMesh(const UpdateContext& ctxt)
{
  if (m_pDetourNavMesh == nullptr && m_hNavMesh.IsValid())
  {
    ...
    if (m_pDetourNavMesh)
    {
      m_pNavMeshPointsOfInterest = PLASMA_DEFAULT_NEW(plNavMeshPointOfInterestGraph);
