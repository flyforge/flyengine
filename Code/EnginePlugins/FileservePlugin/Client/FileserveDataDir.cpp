...
{
  return PLASMA_DEFAULT_NEW(FileserveDataDirectoryReader, 0);
}

plDataDirectory::FolderWriter* plDataDirectory::FileserveType::CreateFolderWriter() const
{
  return PLASMA_DEFAULT_NEW(FileserveDataDirectoryWriter);
}
...
plResult plDataDirectory::FileserveType::GetFileStats(plStringView sFileOrFolder, bool bOneSpecificDataDir, plFileStats& out_Stats)
{
  ...
  plStringBuilder sFullPath;
  PLASMA_SUCCEED_OR_RETURN(plFileserveClient::GetSingleton()->DownloadFile(m_uiDataDirID, sRedirected, bOneSpecificDataDir, &sFullPath));
...
plDataDirectoryType* plDataDirectory::FileserveType::Factory(plStringView sDataDirectory, plStringView sGroup, plStringView sRootName, plFileSystem::DataDirUsage Usage)
{
  ...
  plDataDirectory::FileserveType* pDataDir = PLASMA_DEFAULT_NEW(plDataDirectory::FileserveType);
  pDataDir->m_uiDataDirID = plFileserveClient::GetSingleton()->MountDataDirectory(sDataDirectory, sRootName);

  if (pDataDir->m_uiDataDirID < 0xffff && pDataDir->InitializeDataDirectory(sDataDirectory) == PLASMA_SUCCESS)
    return pDataDir;
...
PLASMA_STATICLINK_FILE(FileservePlugin, FileservePlugin_Client_FileserveDataDir);