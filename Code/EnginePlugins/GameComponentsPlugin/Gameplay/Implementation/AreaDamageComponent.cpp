...
// clang-format off
PL_BEGIN_COMPONENT_TYPE(plAreaDamageComponent, 1, plComponentMode::Static)
{
  PL_BEGIN_PROPERTIES
  {
    PL_MEMBER_PROPERTY("OnCreation", m_bTriggerOnCreation)->AddAttributes(new plDefaultValueAttribute(true)),
    PL_MEMBER_PROPERTY("Radius", m_fRadius)->AddAttributes(new plDefaultValueAttribute(5.0f), new plClampValueAttribute(0.0f, plVariant())),
    PL_MEMBER_PROPERTY("CollisionLayer", m_uiCollisionLayer)->AddAttributes(new plDynamicEnumAttribute("PhysicsCollisionLayer")),
    PL_MEMBER_PROPERTY("Damage", m_fDamage)->AddAttributes(new plDefaultValueAttribute(10.0f)),
    PL_MEMBER_PROPERTY("Impulse", m_fImpulse)->AddAttributes(new plDefaultValueAttribute(100.0f)),
  }
  PL_END_PROPERTIES;
  PL_BEGIN_FUNCTIONS
  {
    PL_SCRIPT_FUNCTION_PROPERTY(ApplyAreaDamage),
  }
  PL_END_FUNCTIONS;
  PL_BEGIN_ATTRIBUTES
  {
...
void plAreaDamageComponent::ApplyAreaDamage()
{
  ...
  PL_PROFILE_SCOPE("ApplyAreaDamage");
    ...
    if (!hit.m_hActorObject.IsInvalidated())
    {
      ...
      if (GetWorld()->TryGetObject(hit.m_hActorObject, pObject))
      {
        ...
        if (m_fDamage != 0.0f)
        {
          ...
          pObject->PostEventMessage(msg, this, plTime::MakeFromMilliseconds(200));
        }
      }
    }
  }
