...
void DrawMeshTilePolygons(const dtMeshTile& meshTile, plDynamicArray<plDebugRenderer::Triangle>& out_triangles, plArrayPtr<plColor> areaColors)
{
  for (int polyIdx = 0; polyIdx < meshTile.header->polyCount; ++polyIdx)
  {
    ...
    {
      const plUInt8* pTriangle = &meshTile.detailTris[(polyDetail.triBase + triIdx) * 4]; // 3 indices, one byte flags

      plDebugRenderer::Triangle& tri = out_triangles.ExpandAndGetRef();

      plUInt8 uiArea = poly.getArea();
      tri.m_color = areaColors[uiArea];

      for (int vtxIdx = 0; vtxIdx < 3; ++vtxIdx)
      {
        const plUInt8 vtxValue = pTriangle[vtxIdx];
      ...
      plMath::Swap(tri.m_position[1], tri.m_position[2]); // fix the triangle winding
    }
  }
}
...
void DrawMeshTileEdges(const dtMeshTile& meshTile, bool bOuterEdges, bool bInnerEdges, bool bInnerDetailEdges, plDynamicArray<plDebugRenderer::Line>& out_lines)
{
  ...
  for (int polyIdx = 0; polyIdx < meshTile.header->polyCount; ++polyIdx)
  {
    ...
    for (int vtxIdx = 0; vtxIdx < numPolyVertices; ++vtxIdx)
    {
      ...
      plColor edgeColor = plColor::White;
      ...
      if (bIsInnerEdge)
      {
        if (p->neis[vtxIdx] & DT_EXT_LINK) // Border between tiles.
        {
          ...
          for (plUInt32 linkIdx = p->firstLink; linkIdx != DT_NULL_LINK; linkIdx = meshTile.links[linkIdx].next)
          {
          ...
          if (bIsConnected)
            edgeColor = plColor::Blue; // border with a neighboring tile that is loaded
          else
            edgeColor = plColor::Red; // border with a neighboring tile that is not loaded
        }
        else
        {
          ...
          edgeColor = plColor::DarkGrey; // inner detail region boundary
        }
      }
      else
      {
        edgeColor = plColor::Yellow; // outer boundary color
      }
      ...
      {
        const plUInt8* t = &meshTile.detailTris[(detailMesh.triBase + k) * 4];
        const float* tv[3];

        for (plInt32 m = 0; m < 3; ++m)
        {
          if (t[m] < p->vertCount)
