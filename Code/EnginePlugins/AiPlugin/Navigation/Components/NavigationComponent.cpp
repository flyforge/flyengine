...
// clang-format off
PLASMA_BEGIN_COMPONENT_TYPE(plAiNavigationComponent, 1, plComponentMode::Static)
{
...
void plAiNavigationComponent::Steer(plTransform& transform)
{
  ...
  m_Steering.m_MinTurnSpeed = plAngle::Degree(180);
...
void plAiNavigationComponent::PlaceOnGround(plTransform& transform)
{
  ...
  const plVec3 vDown = -plVec3::UnitZAxis();
  ...
  plDebugRenderer::DrawLineSphere(GetWorld(), plBoundingSphere(vStartPos, fRadius), plColor::CadetBlue);
  plDebugRenderer::DrawLineSphere(GetWorld(), plBoundingSphere(vEndPos, fRadius), plColor::RebeccaPurple);
