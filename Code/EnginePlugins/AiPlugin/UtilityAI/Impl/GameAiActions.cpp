...
PLASMA_IMPLEMENT_AICMD(plAiActionWait);
...
PLASMA_IMPLEMENT_AICMD(plAiActionLerpRotation);
...
PLASMA_IMPLEMENT_AICMD(plAiActionLerpPosition);
...
{
  m_fSpeed = 0.0f;
  m_vLocalSpaceSlide = plVec3::ZeroVector();
}
...
PLASMA_IMPLEMENT_AICMD(plAiActionLerpRotationTowards);
...
{
  m_vTargetPosition = plVec3::ZeroVector();
...
plAiActionResult plAiActionLerpRotationTowards::Execute(plGameObject& owner, plTime tDiff, plLogInterface* pLog)
{
  ...
  if (vCurDir.NormalizeIfNotZero(plVec3::ZeroVector()).Failed())
  {
    plLog::Error(pLog, "plAiActionLerpRotationTowards: Invalid current direction {}/{}/{}", vCurDir.x, vCurDir.y, vCurDir.z);
    return plAiActionResult::Failed;
  }

  if (vTargetDir.NormalizeIfNotZero(plVec3::ZeroVector()).Failed())
  {
...
//////////////////////////////////////////////////////////////////////////
//
// PLASMA_IMPLEMENT_AICMD(plAiActionFollowPath);
//
...
PLASMA_IMPLEMENT_AICMD(plAiActionBlackboardSetEntry);
...
plAiActionResult plAiActionBlackboardSetEntry::Execute(plGameObject& owner, plTime tDiff, plLogInterface* pLog)
{
  ...
  pBlackboard->SetEntryValue(m_sEntryName, m_Value).IgnoreResult();
  return plAiActionResult::Finished;
}
...
PLASMA_IMPLEMENT_AICMD(plAiActionBlackboardWait);
...
PLASMA_IMPLEMENT_AICMD(plAiActionBlackboardSetAndWait);
...
plAiActionResult plAiActionBlackboardSetAndWait::Execute(plGameObject& owner, plTime tDiff, plLogInterface* pLog)
{
  ...
  {
    pBlackboard->SetEntryValue(m_sEntryName, m_SetValue).IgnoreResult();
    m_SetValue = {};
  }
...
PLASMA_IMPLEMENT_AICMD(plAiActionCCMoveTo);
...
plAiActionResult plAiActionCCMoveTo::Execute(plGameObject& owner, plTime tDiff, plLogInterface* pLog)
{
  ...
  plQuat qRot;
  qRot.SetShortestRotation(plVec3::UnitXAxis(), vDir.GetNormalized());
...
PLASMA_IMPLEMENT_AICMD(plAiActionSpawn);
...
PLASMA_IMPLEMENT_AICMD(plAiActionQuip);
...
PLASMA_IMPLEMENT_AICMD(plAiActionNavigateTo);
...
{
  inout_sText.Format("NavTo: {}/{}/{} - @{}/sec", m_pTargetPosition->x, m_pTargetPosition->y, m_pTargetPosition->z, m_fSpeed);
}
