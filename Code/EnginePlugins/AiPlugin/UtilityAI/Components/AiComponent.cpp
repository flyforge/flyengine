...
PLASMA_BEGIN_COMPONENT_TYPE(plAiComponent, 1, plComponentMode::Dynamic)
{
  PLASMA_BEGIN_PROPERTIES
  {
    PLASMA_MEMBER_PROPERTY("DebugInfo", m_bDebugInfo),
  }
...
void plAiComponent::OnSimulationStarted()
{
  ...
  m_SensorManager.AddSensor("Sensor_See", PLASMA_DEFAULT_NEW(plAiSensorSpatial, plTempHashedString("Sensor_POI")));
  m_PerceptionManager.AddGenerator(PLASMA_DEFAULT_NEW(plAiPerceptionGenPOI));
  m_PerceptionManager.AddGenerator(PLASMA_DEFAULT_NEW(plAiPerceptionGenWander));
  m_PerceptionManager.AddGenerator(PLASMA_DEFAULT_NEW(plAiPerceptionGenCheckpoint));
  m_BehaviorManager.AddBehavior(PLASMA_DEFAULT_NEW(plAiBehaviorGoToPOI));
  //m_BehaviorManager.AddBehavior(PLASMA_DEFAULT_NEW(plAiBehaviorWander));
  m_BehaviorManager.AddBehavior(PLASMA_DEFAULT_NEW(plAiBehaviorGoToCheckpoint));
  // m_BehaviorManager.AddBehavior(PLASMA_DEFAULT_NEW(plAiBehaviorShoot));
  m_BehaviorManager.AddBehavior(PLASMA_DEFAULT_NEW(plAiBehaviorQuip));
}
