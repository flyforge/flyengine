...
plParticleQuadRenderer::plParticleQuadRenderer()
{
  ...
  CreateParticleDataBuffer(m_hTangentDataBuffer, sizeof(plTangentQuadParticleShaderData), s_uiParticlesPerBatch);

  m_hShader = plResourceManager::LoadResource<plShaderResource>("Shaders/Particles/QuadParticle.plShader");
}
...
void plParticleQuadRenderer::RenderBatch(const plRenderViewContext& renderViewContext, const plRenderPipelinePass* pPass, const plRenderDataBatch& batch) const
{
  ...
  {
    pRenderContext->BindMeshBuffer(plGALBufferHandle(), plGALBufferHandle(), nullptr, plGALPrimitiveTopology::Triangles, s_uiParticlesPerBatch * 2);
  ...
  for (auto it = batch.GetIterator<plParticleQuadRenderData>(0, batch.GetCount()); it.IsValid(); ++it)
  {
    ...
    while (uiNumParticles > 0)
    {
      ...
      {
        pGALCommandEncoder->UpdateBuffer(m_hTangentDataBuffer, 0, plMakeArrayPtr(pParticleTangentData, uiNumParticlesInBatch).ToByteArray());
