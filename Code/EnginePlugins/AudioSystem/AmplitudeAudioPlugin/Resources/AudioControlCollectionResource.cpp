...
PLASMA_BEGIN_DYNAMIC_REFLECTED_TYPE(plAmplitudeAudioControlCollectionResource, 1, plRTTIDefaultAllocator<plAmplitudeAudioControlCollectionResource>)
PLASMA_END_DYNAMIC_REFLECTED_TYPE;

PLASMA_RESOURCE_IMPLEMENT_COMMON_CODE(plAmplitudeAudioControlCollectionResource);
...
void plAmplitudeAudioControlCollectionResourceDescriptor::Save(plStreamWriter& stream) const
{
  ...
  for (plUInt32 i = 0; i < uiNumResources; ++i)
  {
...

    {
      ...
      while (true)
      {
        const plUInt64 uiRead = file.ReadBytes(Temp, PLASMA_ARRAY_SIZE(Temp));
...
void plAmplitudeAudioControlCollectionResourceDescriptor::Load(plStreamReader& stream)
{
  ...
  PLASMA_ASSERT_DEV(uiIdentifier == 0xAC, "File does not contain a valid plAmplitudeAudioControlCollectionResourceDescriptor");
  ...
  for (plUInt32 i = 0; i < uiNumResources; ++i)
  {
    ...
    m_Entries[i].m_pControlBufferStorage = PLASMA_AUDIOSYSTEM_NEW(plDefaultMemoryStreamStorage, uiSize, plAudioSystemAllocatorWrapper::GetAllocator());
    m_Entries[i].m_pControlBufferStorage->ReadAll(stream, uiSize);
  }
}


PLASMA_RESOURCE_IMPLEMENT_CREATEABLE(plAmplitudeAudioControlCollectionResource, plAmplitudeAudioControlCollectionResourceDescriptor)
{
...

  {
    ...
    for (auto& entry : m_Collection.m_Entries)
    {
      if (entry.m_pControlBufferStorage != nullptr)
      {
        ...
        PLASMA_AUDIOSYSTEM_DELETE(entry.m_pControlBufferStorage);
      }
    }
...
PLASMA_STATICLINK_FILE(AmplitudeAudioPlugin, AmplitudeAudioPlugin_Resources_AudioControlCollectionResource);
