...
class PLASMA_AUDIOSYSTEMPLUGIN_DLL plAudioMiddleware
{
public:
  ...
  /// \return PLASMA_SUCCESS when the audio middleware was successfully initialized,
  ...
  /// \return PLASMA_SUCCESS when the audio middleware was successfully stopped,
  /// PLASMA_FAILURE otherwise.
  ...
  /// \return PLASMA_SUCCESS when audio middleware resources was successfully released,
  /// PLASMA_FAILURE otherwise.
  virtual plResult Release() = 0;

  /// \brief Stops all the sounds actually played by the audio middleware.
  /// \return PLASMA_SUCCESS when the operation is successful, PLASMA_FAILURE otherwise.
  ...
  PLASMA_NODISCARD virtual const char* GetMiddlewareName() const = 0;

  /// \brief Gets the audio middleware's master gain.
  /// \return The master gain.
  PLASMA_NODISCARD virtual float GetMasterGain() const = 0;

  /// \brief Gets the audio middleware's muted state.
  /// \return The muted state.
  PLASMA_NODISCARD virtual bool GetMute() const = 0;
