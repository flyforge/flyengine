...
plResult plAudioTranslationLayer::Startup()
{
  ...
  PL_SUCCEED_OR_RETURN_CUSTOM_LOG(m_pAudioMiddleware->Startup(), "Unable to load the ATL. An error occurred while loading the audio middleware.");
...
bool plAudioTranslationLayer::ProcessRequest(plVariant&& request, bool bSync)
{
  ...
  if (request.IsA<plAudioSystemRequestRegisterEntity>())
  {
    ...
    audioRequest.m_eStatus = {PL_FAILURE};
    ...
    m_mEntities[audioRequest.m_uiEntityId] = PL_AUDIOSYSTEM_NEW(plATLEntity, audioRequest.m_uiEntityId, entity);
  ...
  else if (request.IsA<plAudioSystemRequestUnregisterEntity>())
  {
    ...
    {
      PL_AUDIOSYSTEM_DELETE(m_mEntities[audioRequest.m_uiEntityId]);
  ...
  else if (request.IsA<plAudioSystemRequestRegisterListener>())
  {
    ...
    m_mListeners[audioRequest.m_uiListenerId] = PL_AUDIOSYSTEM_NEW(plATLListener, audioRequest.m_uiListenerId, pListenerData);
  ...
  else if (request.IsA<plAudioSystemRequestUnregisterListener>())
  {
    ...
    if (audioRequest.m_eStatus.Succeeded())
    {
      PL_AUDIOSYSTEM_DELETE(m_mListeners[audioRequest.m_uiListenerId]);
  ...
  else if (request.IsA<plAudioSystemRequestShutdown>())
  {
    ...
    for (auto&& bank : m_mSoundBanks)
    {
      ...
      PL_AUDIOSYSTEM_DELETE(bank.Value());
    }
    ...
    for (auto&& environment : m_mEnvironments)
    {
      ...
      PL_AUDIOSYSTEM_DELETE(environment.Value());
    }
    ...
    for (auto&& switchState : m_mSwitchStates)
    {
      ...
      PL_AUDIOSYSTEM_DELETE(switchState.Value());
    }
    ...
    for (auto&& rtpc : m_mRtpcs)
    {
      ...
      PL_AUDIOSYSTEM_DELETE(rtpc.Value());
    }
    ...
    for (auto&& trigger : m_mTriggers)
    {
      ...
      PL_AUDIOSYSTEM_DELETE(trigger.Value());
    }
    ...
    for (auto&& listener : m_mListeners)
    {
      ...
      PL_AUDIOSYSTEM_DELETE(listener.Value());
    }
    ...
    for (auto&& entity : m_mEntities)
    {
      ...
      PL_AUDIOSYSTEM_DELETE(entity.Value());
    }

    audioRequest.m_eStatus = {PL_SUCCESS};
  }
...
PL_STATICLINK_FILE(AudioSystemPlugin, AudioSystemPlugin_Implementation_ATL_AudioTranslationLayer);
