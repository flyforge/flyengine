...
// clang-format off
PLASMA_IMPLEMENT_SINGLETON(plAudioSystem);
// clang-format on
...
void plAudioSystem::UpdateSound()
{
  ...
  // PLASMA_ASSERT_ALWAYS(gMainThreadId == plThreadUtils::GetCurrentThreadID(), "AudioSystem::UpdateSound not called from main thread.");
  ...
  {
    PLASMA_LOCK(m_BlockingRequestCallbacksMutex);
  ...
  if (!handleBlockingRequest)
  {
    ...
    {
      PLASMA_LOCK(m_PendingRequestCallbacksMutex);
...
plAudioSystem::~plAudioSystem()
{
  PLASMA_ASSERT_DEV(!m_bInitialized, "You should shutdown the AudioSystem before to call the dtor.");
}

bool plAudioSystem::Startup()
{
  PLASMA_ASSERT_ALWAYS(gMainThreadId == plThreadUtils::GetCurrentThreadID(), "AudioSystem::Startup not called from main thread.");
...
{
  PLASMA_ASSERT_ALWAYS(gMainThreadId == plThreadUtils::GetCurrentThreadID(), "AudioSystem::Shutdown not called from main thread.");
...
void plAudioSystem::SendRequest(plVariant&& request)
{
  ...
  PLASMA_LOCK(m_PendingRequestsMutex);
...
void plAudioSystem::SendRequestSync(plVariant&& request)
{
  ...
  {
    PLASMA_LOCK(m_BlockingRequestsMutex);
...
void plAudioSystem::RegisterTrigger(plAudioSystemDataID uiId, plAudioSystemTriggerData* pTriggerData)
{
  ...
  m_AudioTranslationLayer.m_mTriggers[uiId] = PLASMA_AUDIOSYSTEM_NEW(plATLTrigger, uiId, pTriggerData);
}
...
void plAudioSystem::RegisterRtpc(plAudioSystemDataID uiId, plAudioSystemRtpcData* pRtpcData)
{
  ...
  m_AudioTranslationLayer.m_mRtpcs[uiId] = PLASMA_AUDIOSYSTEM_NEW(plATLRtpc, uiId, pRtpcData);
}
...
void plAudioSystem::RegisterSwitchState(plAudioSystemDataID uiId, plAudioSystemSwitchStateData* pSwitchStateData)
{
  ...
  m_AudioTranslationLayer.m_mSwitchStates[uiId] = PLASMA_AUDIOSYSTEM_NEW(plATLSwitchState, uiId, pSwitchStateData);
}
...
void plAudioSystem::RegisterEnvironment(plAudioSystemDataID uiId, plAudioSystemEnvironmentData* pEnvironmentData)
{
  ...
  m_AudioTranslationLayer.m_mEnvironments[uiId] = PLASMA_AUDIOSYSTEM_NEW(plATLEnvironment, uiId, pEnvironmentData);
}
...
void plAudioSystem::RegisterSoundBank(plAudioSystemDataID uiId, plAudioSystemBankData* pSoundBankData)
{
  ...
  m_AudioTranslationLayer.m_mSoundBanks[uiId] = PLASMA_AUDIOSYSTEM_NEW(plATLSoundBank, uiId, pSoundBankData);
}
...
void plAudioSystem::UnregisterEntity(const plAudioSystemDataID uiId)
{
  ...
  PLASMA_AUDIOSYSTEM_DELETE(m_AudioTranslationLayer.m_mEntities[uiId]);
...
void plAudioSystem::UnregisterListener(const plAudioSystemDataID uiId)
{
  ...
  PLASMA_AUDIOSYSTEM_DELETE(m_AudioTranslationLayer.m_mListeners[uiId]);
...
void plAudioSystem::UnregisterTrigger(const plAudioSystemDataID uiId)
{
  ...
  PLASMA_AUDIOSYSTEM_DELETE(m_AudioTranslationLayer.m_mTriggers[uiId]);
...
void plAudioSystem::UnregisterRtpc(const plAudioSystemDataID uiId)
{
  ...
  PLASMA_AUDIOSYSTEM_DELETE(m_AudioTranslationLayer.m_mRtpcs[uiId]);
...
void plAudioSystem::UnregisterSwitchState(plAudioSystemDataID uiId)
{
  ...
  PLASMA_AUDIOSYSTEM_DELETE(m_AudioTranslationLayer.m_mSwitchStates[uiId]);
...
void plAudioSystem::UnregisterEnvironment(plAudioSystemDataID uiId)
{
  ...
  PLASMA_AUDIOSYSTEM_DELETE(m_AudioTranslationLayer.m_mEnvironments[uiId]);
...
void plAudioSystem::UnregisterSoundBank(plAudioSystemDataID uiId)
{
  ...
  PLASMA_AUDIOSYSTEM_DELETE(m_AudioTranslationLayer.m_mSoundBanks[uiId]);
  m_AudioTranslationLayer.m_mSoundBanks.Remove(uiId);
}

void plAudioSystem::UpdateInternal()
{
  PLASMA_ASSERT_ALWAYS(m_pAudioThread->GetThreadID() == plThreadUtils::GetCurrentThreadID(), "AudioSystem::UpdateInternal not called from audio thread.");
...
void plAudioSystem::StartAudioThread()
{
  ...
  {
    m_pAudioThread = PLASMA_AUDIOSYSTEM_NEW(plAudioThread);
...
void plAudioSystem::StopAudioThread()
{
  if (m_pAudioThread != nullptr)
  {
    ...
    PLASMA_AUDIOSYSTEM_DELETE(m_pAudioThread);
...
PLASMA_STATICLINK_FILE(AudioSystemPlugin, AudioSystemPlugin_Implementation_AudioSystem);
