...
class PL_AUDIOSYSTEMPLUGIN_DLL plAudioSystemComponent : public plComponent
{
  PL_DECLARE_ABSTRACT_COMPONENT_TYPE(plAudioSystemComponent, plComponent);
...
class PL_AUDIOSYSTEMPLUGIN_DLL plAudioSystemProxyDependentComponent : public plAudioSystemComponent
{
  PL_DECLARE_ABSTRACT_COMPONENT_TYPE(plAudioSystemProxyDependentComponent, plAudioSystemComponent);
...
protected:
  ...
  PL_NODISCARD plAudioSystemDataID GetEntityId() const;

  class plAudioProxyComponent* m_pProxyComponent{nullptr};
};

/// \brief Base class for audio system environment components.
class PL_AUDIOSYSTEMPLUGIN_DLL plAudioSystemEnvironmentComponent : public plAudioSystemProxyDependentComponent
{
  PL_DECLARE_ABSTRACT_COMPONENT_TYPE(plAudioSystemEnvironmentComponent, plAudioSystemProxyDependentComponent);
...
public:
  ...
  PL_NODISCARD virtual float GetEnvironmentAmount(plAudioProxyComponent* pProxyComponent) const = 0;

  /// \brief Gets the ID of the environment in the Audio System.
  PL_NODISCARD plAudioSystemDataID GetEnvironmentId() const;

  /// \brief Gets the distance from the sphere's origin
  /// at which the environment amount will slightly start to decrease.
  PL_NODISCARD virtual float GetMaxDistance() const;
