...
class PLASMA_AUDIOSYSTEMPLUGIN_DLL plAudioListenerComponent : public plAudioSystemComponent
{
  PLASMA_DECLARE_COMPONENT_TYPE(plAudioListenerComponent, plAudioSystemComponent, plAudioListenerComponentManager);
...
public:
  ...
  PLASMA_NODISCARD plVec3 GetListenerPosition() const;

  /// \brief Gets the current velocity of this listener.
  PLASMA_NODISCARD plVec3 GetListenerVelocity() const;

  /// \brief Gets the current orientation of this listener.
  PLASMA_NODISCARD plQuat GetListenerRotation() const;

  /// \brief Gets whether this listener is the default one or not.
  PLASMA_NODISCARD bool IsDefault() const;
