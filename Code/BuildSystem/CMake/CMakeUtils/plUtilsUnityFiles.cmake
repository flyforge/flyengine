...
function(pl_generate_folder_unity_file TARGET_NAME PROJECT_DIRECTORY SUB_FOLDER_PATH FILE_LIST)
	...
	pl_add_pvs_header(UNITY_FILE_CONTENTS "// generated unity cpp file\n\n")

	# find the PCH and include it as the first thing in the unity file
	pl_retrieve_target_pch(${TARGET_NAME} PCH_FILE)

	if(PCH_FILE)
		string(APPEND UNITY_FILE_CONTENTS "#include <${PCH_FILE}.h>\n\n")

		pl_pch_use("${PCH_FILE}.h" "${FOLDER_UNITY_FILE}")
	endif()
...
# #####################################
# ## pl_generate_folder_unity_files_for_target(<target> <project-path> [<folder-to-exclude> ...])
# #####################################
function(pl_generate_folder_unity_files_for_target TARGET_NAME TARGET_DIRECTORY)
	pl_gather_subfolders(${TARGET_DIRECTORY} ALL_FOLDERS)
	...
	foreach(CUR_FOLDER ${ALL_FOLDERS})
		...
		pl_generate_folder_unity_file(${TARGET_NAME} "${TARGET_DIRECTORY}" "${CUR_FOLDER}" "${FILES_IN_FOLDER}")
	endforeach()
endfunction()