  ...
  : QTreeWidget(pParent)
{
  ...
  PLASMA_VERIFY(connect(pDelegate, &plFolderNameDelegate::editingFinished, this, &eqQtAssetBrowserFolderView::OnFolderEditingFinished, Qt::QueuedConnection), "signal/slot connection failed");
  ...
  PLASMA_VERIFY(connect(this, SIGNAL(itemSelectionChanged()), this, SLOT(OnItemSelectionChanged())) != nullptr, "signal/slot connection failed");
...
{
  PLASMA_LOCK(m_FolderStructureMutex);
...
plStatus eqQtAssetBrowserFolderView::canDrop(QDropEvent* e, plDynamicArray<plString>& out_files, plString& out_sTargetFolder)
{
  ...
  if (dropIndex.isValid())
  {
    ...
    out_sTargetFolder = qtToPlasmaString(sAbsTarget);

    for (const QString& sFile : files)
    {
      plString sFileToMove = qtToPlasmaString(sFile);
