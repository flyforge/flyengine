...
class PL_EDITORFRAMEWORK_DLL plQtAssetFilter : public QObject
{
  Q_OBJECT
public:
...
struct PL_EDITORFRAMEWORK_DLL plAssetBrowserItemFlags
{
  using StorageType = plUInt8;

  enum Enum
  {
    Folder = PL_BIT(0),        // Any folder inside a data directory
    DataDirectory = PL_BIT(1), // mutually exclusive with Folder
    File = PL_BIT(2),          // any file, could also be an Asset
    Asset = PL_BIT(3),         // main asset: mutually exclusive with SubAsset
    SubAsset = PL_BIT(4),      // sub-asset (imaginary, not a File or Asset)
    Default = 0
  };
...
PL_DECLARE_FLAGS_OPERATORS(plAssetBrowserItemFlags);

/// \brief Model of the item view in the asset browser.
class PL_EDITORFRAMEWORK_DLL plQtAssetBrowserModel : public QAbstractItemModel
{
  Q_OBJECT
public:
  enum UserRoles
