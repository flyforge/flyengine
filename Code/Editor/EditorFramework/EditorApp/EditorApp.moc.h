...
struct PLASMA_EDITORFRAMEWORK_DLL plEditorAppEvent
{
  enum class Type
  {
...
public:
  struct StartupFlags
  {
    ...
    enum Enum
    {
      ...
      UnitTest = PLASMA_BIT(3),   ///< Specified when the process is running as a unit test
      Background = PLASMA_BIT(4), ///< This process is an editor processor background process handling IPC tasks of the editor parent process.
      Default = 0,
    };
