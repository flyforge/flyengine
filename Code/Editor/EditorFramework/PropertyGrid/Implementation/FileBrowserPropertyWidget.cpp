...
plQtExternalFilePropertyWidget::plQtExternalFilePropertyWidget()
  : plQtStandardPropertyWidget()
{
  m_pLayout = new QHBoxLayout(this);
  m_pLayout->setContentsMargins(0, 0, 0, 0);
...
bool plQtExternalFilePropertyWidget::IsValidFileReference(plStringView sFile) const
{
  auto pAttr = m_pProp->GetAttributeByType<plExternalFileBrowserAttribute>();
...
void plQtExternalFilePropertyWidget::OnInit()
{
  auto pAttr = m_pProp->GetAttributeByType<plExternalFileBrowserAttribute>();
  PL_ASSERT_DEV(pAttr != nullptr, "plQtFilePropertyWidget was created without a plExternalFileBrowserAttribute!");
}

void plQtExternalFilePropertyWidget::InternalSetValue(const plVariant& value)
{
  plQtScopedBlockSignals b(m_pWidget);
  plQtScopedBlockSignals b2(m_pButton);
...
void plQtExternalFilePropertyWidget::on_TextFinished_triggered()
{
  plStringBuilder sText = m_pWidget->text().toUtf8().data();
...
void plQtExternalFilePropertyWidget::on_TextChanged_triggered(const QString& value)
{
  if (!hasFocus())
    on_TextFinished_triggered();
}

void plQtExternalFilePropertyWidget::OnOpenExplorer()
{
  plString sPath = m_pWidget->text().toUtf8().data();
  if (!plQtEditorApp::GetSingleton()->MakeDataDirectoryRelativePathAbsolute(sPath))
    return;
...
void plQtExternalFilePropertyWidget::OnOpenFile()
{
  plString sPath = m_pWidget->text().toUtf8().data();
  if (!plQtEditorApp::GetSingleton()->MakeDataDirectoryRelativePathAbsolute(sPath))
    return;
...
void plQtExternalFilePropertyWidget::on_BrowseFile_clicked()
{
  plString sFile = m_pWidget->text().toUtf8().data();
  const plExternalFileBrowserAttribute* pFileAttribute = m_pProp->GetAttributeByType<plExternalFileBrowserAttribute>();
  ...
  QString sResult = QFileDialog::getOpenFileName(this, plMakeQString(pFileAttribute->GetDialogTitle()), sStartDir.GetData(), plMakeQString(pFileAttribute->GetTypeFilter()), nullptr, QFileDialog::Option::DontResolveSymlinks);
