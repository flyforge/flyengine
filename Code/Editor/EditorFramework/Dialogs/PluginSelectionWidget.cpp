...
plQtPluginSelectionWidget::~plQtPluginSelectionWidget() = default;

void plQtPluginSelectionWidget::SetPluginSet(plPluginBundleSet* pPluginSet)
{
  m_pPluginSet = pPluginSet;
  m_States.Clear();

  plSet<plString> templates;
  plStringBuilder tmp;
  ...
  for (const plString tmp : templates)
  {
...
void plQtPluginSelectionWidget::SyncStateToSet()
{
...
void plQtPluginSelectionWidget::on_PluginsList_currentItemChanged(QListWidgetItem* current, QListWidgetItem* previous)
{
  if (previous)
  {
...
void plQtPluginSelectionWidget::on_PluginsList_itemChanged(QListWidgetItem* item)
{
...
void plQtPluginSelectionWidget::on_Template_currentIndexChanged(int index)
{
  if (index <= 0)
    return;

  const plString sTemplate = Template->currentText().toUtf8().data();
...
void plQtPluginSelectionWidget::UpdateInternalState()
{
  for (auto& s : m_States)
  {
  ...
  plSet<plString, plCompareString_NoCase> exclusiveFeatures;

  for (plUInt32 row = 0; row < m_States.GetCount(); ++row)
  {
...
void plQtPluginSelectionWidget::ApplyRequired(plArrayPtr<plString> required)
{
