...
plPerClusterData GetClusterData(float3 screenPosition)
{
...
float3 ComputeReflection(inout plMaterialData matData, float3 viewVector, plPerClusterData clusterData)
{
  ...
  [loop] for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    const plPerReflectionProbeData probeData = perPerReflectionProbeDataBuffer[probeIndex];
...
void ApplyDecals(inout plMaterialData matData, plPerClusterData clusterData, uint gameObjectId)
{
  uint firstItemIndex = clusterData.offset;
  ...
  for (uint i = firstItemIndex; i < lastItemIndex; ++i)
  {
    ...
    plPerDecalData decalData = perDecalDataBuffer[decalIndex];
...
void ApplyRefraction(inout plMaterialData matData, inout AccumulatedLight light)
{
