...
AccumulatedLight SphereLightShading(plPerLightData lightData, plMaterialData matData, float3 L, float3 V, float specularIntensity, float attenuation, out float luminance)
{
  ...
  float3 closestPointOnSphere = lightVector + centerToRay * saturate(lightData.size * invDistToRay);
  ...
  float sqrLightRadius = lightData.size * lightData.size;
...
AccumulatedLight TubeLightShading(plPerLightData lightData, plMaterialData matData, float3 L, float3 V, float specularIntensity, float attenuation, out float luminanceMultiplier, out float luminanceModifier)
{
  float3 N = matData.worldNormal;
  float3 lightVector = lightData.position - matData.worldPosition;

  float3 lightLeft = unpack(RGB10ToFloat3(lightData.upDirection));
  float lightWidth = lightData.length;

  float3 P0 = lightData.position - lightLeft * lightWidth * 0.5f;
  float3 P1 = lightData.position + lightLeft * lightWidth * 0.5f;
  ...
  float3 p0 = lightData.position - left * (0.5 * lightWidth) + lightData.size * up;
  float3 p1 = lightData.position - left * (0.5 * lightWidth) - lightData.size * up;
  float3 p2 = lightData.position + left * (0.5 * lightWidth) - lightData.size * up;
  float3 p3 = lightData.position + left * (0.5 * lightWidth) + lightData.size * up;
  ...
  luminanceModifier = PI * saturate(dot(sphereL, N)) * ((lightData.size * lightData.size) / sqrSphereDistance);
  ...
  float3 closestPoint = L + centerToRay * saturate(lightData.size / length(centerToRay));
