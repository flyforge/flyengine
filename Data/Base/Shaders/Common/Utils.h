...
/*------------------------------------------------------------------------------
    TRANSFORMATIONS
------------------------------------------------------------------------------*/
float3 WorldToView(float3 x, bool isPosition = true)
{
  return mul(float4(x, (float)isPosition), GetWorldToCameraMatrix()).xyz;
}

float3 WorldToNdc(float3 x, bool isPosition = true)
{
  float4 ndc = mul(float4(x, (float)isPosition), GetWorldToScreenMatrix());
  return ndc.xyz / ndc.w;
}

float3 WorldToNdc(float3 x, float4x4 transform)
{
  float4 ndc = mul(float4(x, 1.0f), transform);
  return ndc.xyz / ndc.w;
}

float3 ViewToNdc(float3 x, bool isPosition = true)
{
  float4 ndc = mul(float4(x, (float)isPosition), GetCameraToScreenMatrix());
  return ndc.xyz / ndc.w;
}

float2 WorldToUv(float3 x, bool isPosition = true)
{
  float4 uv = mul(float4(x, (float)isPosition), GetWorldToScreenMatrix());
  return (uv.xy / uv.w) * float2(0.5f, -0.5f) + 0.5f;
}

float2 ViewToUv(float3 x, bool isPosition = true)
{
  float4 uv = mul(float4(x, (float)isPosition), GetCameraToScreenMatrix());
  return (uv.xy / uv.w) * float2(0.5f, -0.5f) + 0.5f;
}

float3 GetPositionWSFromDepth(const float2 uv, const float depth)
{
...
/*------------------------------------------------------------------------------
    DIRECTION UV
------------------------------------------------------------------------------*/
float2 DirectionSphereUv(float3 direction)
{
...
float GetOffsetNonTemporal(uint2 screenPos)
{
  int2 position = (int2)(screenPos);
